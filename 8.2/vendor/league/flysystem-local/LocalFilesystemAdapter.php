...
namespace Odigos\League\Flysystem\Local;
...
use Generator;
use Odigos\League\Flysystem\ChecksumProvider;
use Odigos\League\Flysystem\Config;
use Odigos\League\Flysystem\DirectoryAttributes;
use Odigos\League\Flysystem\FileAttributes;
use Odigos\League\Flysystem\FilesystemAdapter;
use Odigos\League\Flysystem\PathPrefixer;
use Odigos\League\Flysystem\SymbolicLinkEncountered;
use Odigos\League\Flysystem\UnableToCopyFile;
use Odigos\League\Flysystem\UnableToCreateDirectory;
use Odigos\League\Flysystem\UnableToDeleteDirectory;
use Odigos\League\Flysystem\UnableToDeleteFile;
use Odigos\League\Flysystem\UnableToMoveFile;
use Odigos\League\Flysystem\UnableToProvideChecksum;
use Odigos\League\Flysystem\UnableToReadFile;
use Odigos\League\Flysystem\UnableToRetrieveMetadata;
use Odigos\League\Flysystem\UnableToSetVisibility;
use Odigos\League\Flysystem\UnableToWriteFile;
use Odigos\League\Flysystem\UnixVisibility\PortableVisibilityConverter;
use Odigos\League\Flysystem\UnixVisibility\VisibilityConverter;
use Odigos\League\MimeTypeDetection\FinfoMimeTypeDetector;
use Odigos\League\MimeTypeDetection\MimeTypeDetector;
...
class LocalFilesystemAdapter implements FilesystemAdapter, ChecksumProvider
{
    ...
    public function __construct(string $location, ?VisibilityConverter $visibility = null, private int $writeFlags = LOCK_EX, private int $linkHandling = self::DISALLOW_LINKS, ?MimeTypeDetector $mimeTypeDetector = null, bool $lazyRootCreation = \false, bool $useInconclusiveMimeTypeFallback = \false)
    {
        $this->prefixer = new PathPrefixer($location, DIRECTORY_SEPARATOR);
        $visibility ??= new PortableVisibilityConverter();
        $this->visibility = $visibility;
        $this->rootLocation = $location;
        $this->mimeTypeDetector = $mimeTypeDetector ?? new FallbackMimeTypeDetector(detector: new FinfoMimeTypeDetector(), useInconclusiveMimeTypeFallback: $useInconclusiveMimeTypeFallback);
        if (!$lazyRootCreation) {
    ...
    public function listContents(string $path, bool $deep): iterable
    {
        ...
        foreach ($iterator as $fileInfo) {
            ...
            try {
                ...
                yield $isDirectory ? new DirectoryAttributes(str_replace('\\', '/', $path), $visibility, $lastModified) : new FileAttributes(str_replace('\\', '/', $path), $fileInfo->getSize(), $visibility, $lastModified);
    ...
    public function copy(string $source, string $destination, Config $config): void
    {
        ...
        $visibility = $config->get(Config::OPTION_VISIBILITY, $config->get(Config::OPTION_RETAIN_VISIBILITY, \true) ? $this->visibility($source)->visibility() : null);
        if ($visibility) {
            $this->setVisibility($destination, (string) $visibility);
        }
    }
