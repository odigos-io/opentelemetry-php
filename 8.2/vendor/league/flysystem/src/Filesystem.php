...
use Generator;
use Odigos\League\Flysystem\UrlGeneration\PrefixPublicUrlGenerator;
use Odigos\League\Flysystem\UrlGeneration\PublicUrlGenerator;
use Odigos\League\Flysystem\UrlGeneration\ShardedPrefixPublicUrlGenerator;
use Odigos\League\Flysystem\UrlGeneration\TemporaryUrlGenerator;
use Throwable;
...
class Filesystem implements FilesystemOperator
{
    ...
    public function __construct(private FilesystemAdapter $adapter, array $config = [], ?PathNormalizer $pathNormalizer = null, private ?PublicUrlGenerator $publicUrlGenerator = null, private ?TemporaryUrlGenerator $temporaryUrlGenerator = null)
    {
        $this->config = new Config($config);
        $this->pathNormalizer = $pathNormalizer ?? new WhitespacePathNormalizer($this->config->get('allow_relative_path_traversal', \true));
    }
    ...
    {
        $this->publicUrlGenerator ??= $this->resolvePublicUrlGenerator() ?? throw UnableToGeneratePublicUrl::noGeneratorConfigured($path);
    ...
    public function temporaryUrl(string $path, DateTimeInterface $expiresAt, array $config = []): string
    {
        ...
        if ($generator instanceof TemporaryUrlGenerator) {
            return $generator->temporaryUrl($this->pathNormalizer->normalizePath($path), $expiresAt, $this->config->extend($config));
        }
    ...
    public function checksum(string $path, array $config = []): string
    {
        ...
        try {
            return $this->adapter->checksum($this->pathNormalizer->normalizePath($path), $config);
        } catch (ChecksumAlgoIsNotSupported) {
            return $this->calculateChecksumFromStream($this->pathNormalizer->normalizePath($path), $config);
        }
    }
    ...
    {
        $retainVisibility = $this->config->get(Config::OPTION_RETAIN_VISIBILITY, $config[Config::OPTION_RETAIN_VISIBILITY] ?? \true);
