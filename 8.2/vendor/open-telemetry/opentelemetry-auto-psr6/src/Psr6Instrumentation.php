...
namespace Odigos\OpenTelemetry\Contrib\Instrumentation\Psr6;
...
class Psr6Instrumentation
{
    ...
    {
        $instrumentation = new CachedInstrumentation('io.opentelemetry.contrib.php.psr6', InstalledVersions::getVersion('open-telemetry/opentelemetry-auto-psr6'), 'https://opentelemetry.io/schemas/1.32.0');
    ...
    private static function makeSpanBuilder(CachedInstrumentation $instrumentation, string $name, string $function, string $class, ?string $filename, ?int $lineno): SpanBuilderInterface
    {
        return $instrumentation->tracer()->spanBuilder($function)->setSpanKind(SpanKind::KIND_INTERNAL)->setAttribute(TraceAttributes::CODE_FUNCTION_NAME, sprintf('%s::%s', $class, $function))->setAttribute(TraceAttributes::CODE_FILE_PATH, $filename)->setAttribute(TraceAttributes::CODE_LINE_NUMBER, $lineno)->setAttribute('cache.operation', $name);
    }
