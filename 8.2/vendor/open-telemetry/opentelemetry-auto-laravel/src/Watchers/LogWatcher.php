...
class LogWatcher extends \OpenTelemetry\Contrib\Instrumentation\Laravel\Watchers\Watcher
{
    ...
    public function recordLog(MessageLogged $log): void
    {
        ...
        $attributes = ['context' => json_encode($contextToEncode), ...$exception !== null ? ['exception.type' => $exception::class, 'exception.message' => $exception->getMessage(), 'exception.stacktrace' => StackTraceFormatter::format($exception)] : []];
        $logger = $this->instrumentation->logger();
        $record = (new LogRecord($log->message))->setSeverityText($log->level)->setSeverityNumber(Severity::fromPsr3($log->level))->setAttributes($attributes);
        $logger->emit($record);
    }
