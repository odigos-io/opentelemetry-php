...
final class ExplicitBucketHistogramAggregation implements AggregationInterface
{
    ...
    #[\Override]
    public function initialize(): \OpenTelemetry\SDK\Metrics\Aggregation\ExplicitBucketHistogramSummary
    {
        return new \OpenTelemetry\SDK\Metrics\Aggregation\ExplicitBucketHistogramSummary(0, 0, +INF, -INF, array_fill(0, count($this->boundaries) + 1, 0));
    }
    /**
     ...
     */
    #[\Override]
    public function merge($left, $right): \OpenTelemetry\SDK\Metrics\Aggregation\ExplicitBucketHistogramSummary
    {
        ...
        return new \OpenTelemetry\SDK\Metrics\Aggregation\ExplicitBucketHistogramSummary($count, $sum, $min, $max, $buckets);
    }
    /**
     * @param ExplicitBucketHistogramSummary $left
     * @param ExplicitBucketHistogramSummary $right
     */
    #[\Override]
    public function diff($left, $right): \OpenTelemetry\SDK\Metrics\Aggregation\ExplicitBucketHistogramSummary
    {
        $count = -$left->count + $right->count;
        $sum = -$left->sum + $right->sum;
