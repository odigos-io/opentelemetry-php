...
{
    private readonly \OpenTelemetry\SDK\Metrics\Stream\Delta $head;
    public function __construct(private readonly AggregationInterface $aggregation)
    {
        $this->head = new \OpenTelemetry\SDK\Metrics\Stream\Delta(new \OpenTelemetry\SDK\Metrics\Stream\Metric([], [], 0), 0);
        /** @phan-suppress-next-line PhanTypeObjectUnsetDeclaredProperty */
        unset($this->head->metric);
    }
    public function add(\OpenTelemetry\SDK\Metrics\Stream\Metric $metric, int|GMP $readers): void
    {
        ...
        if (($this->head->prev->readers ?? null) != $readers) {
            $this->head->prev = new \OpenTelemetry\SDK\Metrics\Stream\Delta($metric, $readers, $this->head->prev);
        } else {
            assert($this->head->prev !== null);
            $this->mergeInto($this->head->prev->metric, $metric);
        }
    }
    public function collect(int $reader, bool $retain = \false): ?\OpenTelemetry\SDK\Metrics\Stream\Metric
    {
        $n = null;
    ...
    private function tryUnlink(\OpenTelemetry\SDK\Metrics\Stream\Delta $n): void
    {
        assert($n->prev !== null);
        /** @phpstan-ignore-next-line */
        if ($n->prev->readers == 0) {
    ...
    private function mergeInto(\OpenTelemetry\SDK\Metrics\Stream\Metric $into, \OpenTelemetry\SDK\Metrics\Stream\Metric $metric): void
    {
