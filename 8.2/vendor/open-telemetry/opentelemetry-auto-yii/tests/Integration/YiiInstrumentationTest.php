...
class YiiInstrumentationTest extends AbstractTest
{
    public function test_success()
    {
        ...
        $this->assertEquals('Odigos\yii\web\HeadersAlreadySentException', get_class($exception));
    ...
    public function test_non_inline_action()
    {
        ...
        $this->assertNotNull($exception);
        $this->assertEquals('Odigos\yii\base\ViewNotFoundException', get_class($exception));
        $attributes = $this->storage[0]->getAttributes();
        $this->assertCount(1, $this->storage);
    ...
    {
        $config = ['id' => 'basic', 'basePath' => dirname(__DIR__), 'bootstrap' => ['log'], 'components' => ['request' => [
            // !!! insert a secret key in the following (if it is empty) - this is required by cookie validation
            'cookieValidationKey' => 'ybQT_avit3_11aPJfJQ6zwbdjjwjOYbs',
        ], 'cache' => ['class' => 'Odigos\yii\caching\FileCache'], 'user' => ['identityClass' => 'Odigos\app\models\User', 'enableAutoLogin' => \true], 'errorHandler' => ['errorAction' => 'site/error'], 'log' => ['traceLevel' => 3, 'targets' => [['class' => 'Odigos\yii\log\FileTarget', 'levels' => ['error', 'warning']]]], 'db' => ['class' => 'Odigos\yii\db\Connection', 'dsn' => 'mysql:host=localhost;dbname=yii2basic', 'username' => 'root', 'password' => '', 'charset' => 'utf8'], 'urlManager' => ['enablePrettyUrl' => \true, 'showScriptName' => \false, 'rules' => ['<controller:\w+>' => '<controller>', '<controller:\w+>/<action:\w+>' => '<controller>/<action>']]], 'controllerMap' => ['site' => ['class' => 'Odigos\OpenTelemetry\Tests\Instrumentation\Yii\tests\Integration\SiteController']], 'params' => []];
...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $config = ['class' => 'Odigos\yii\web\Response', 'format' => \Odigos\yii\web\Response::FORMAT_RAW, 'content' => 'hello'];
        return \Odigos\Yii::createObject($config);
    }
    /** @psalm-suppress MoreSpecificReturnType */
    public function actions()
    {
        /** @psalm-suppress LessSpecificReturnStatement */
        return ['error' => ['class' => 'Odigos\yii\web\ErrorAction']];
    }
