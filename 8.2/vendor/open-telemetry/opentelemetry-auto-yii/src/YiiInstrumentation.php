...
class YiiInstrumentation
{
    ...
    public static function register(): void
    {
        ...
        hook(Application::class, 'handleRequest', pre: static function (Application $application, array $params, string $class, string $function, ?string $filename, ?int $lineno) use ($instrumentation): void {
            $request = $application->getRequest();
            $parent = Globals::propagator()->extract($request, \OpenTelemetry\Contrib\Instrumentation\Yii\RequestPropagationGetter::instance());
        ...
        }, post: static function (Application $application, array $params, ?Response $response, ?\Throwable $exception): void {
            ...
            if ($response) {
                ...
                $span->setAttribute(TraceAttributes::HTTP_RESPONSE_BODY_SIZE, \OpenTelemetry\Contrib\Instrumentation\Yii\YiiInstrumentation::getResponseLength($response));
                ...
                $prop->inject($response, \OpenTelemetry\Contrib\Instrumentation\Yii\ResponsePropagationSetter::instance(), $scope->context());
            }
            if ($exception) {
        ...
        hook(Controller::class, 'beforeAction', pre: static function (Controller $controller, array $params, string $class, string $function, ?string $filename, ?int $lineno): void {
            ...
            $route = \OpenTelemetry\Contrib\Instrumentation\Yii\YiiInstrumentation::normalizeRouteName(get_class($controller), $actionName);
