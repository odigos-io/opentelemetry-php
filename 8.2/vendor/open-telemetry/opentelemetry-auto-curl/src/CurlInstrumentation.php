...
class CurlInstrumentation
{
    ...
    public static function register(): void
    {
        ...
        hook(null, 'curl_setopt', pre: null, post: static function ($_obj, array $params, mixed $retVal) use ($curlHandleToAttributes, &$curlSetOptInstrumentationSuppressed) {
            if ($retVal != \true || $curlSetOptInstrumentationSuppressed) {
                return;
            }
        ...
        hook(null, 'curl_setopt_array', pre: null, post: static function ($_obj, array $params, mixed $retVal) use ($curlHandleToAttributes) {
            if ($retVal != \true) {
        ...
        hook(null, 'curl_copy_handle', pre: null, post: static function ($obj, array $params, mixed $retVal) use ($curlHandleToAttributes) {
        ...
        hook(null, 'curl_exec', pre: static function ($obj, array $params, ?string $class, ?string $function, ?string $filename, ?int $lineno) use ($instrumentation, $curlHandleToAttributes, &$curlSetOptInstrumentationSuppressed) {
            ...
            $builder = $instrumentation->tracer()->spanBuilder($spanName)->setParent($parent)->setSpanKind(SpanKind::KIND_CLIENT)->setAttribute(TraceAttributes::CODE_FUNCTION_NAME, $function)->setAttribute(TraceAttributes::CODE_FILE_PATH, $filename)->setAttribute(TraceAttributes::CODE_LINE_NUMBER, $lineno)->setAttributes($attributes);
            ...
            if (self::isRequestHeadersCapturingEnabled()) {
                if (!$curlHandleToAttributes[$params[0]]->isVerboseEnabled()) {
                    // we let go of captuing request headers because CURLINFO_HEADER_OUT is disabling CURLOPT_VERBOSE
                    curl_setopt($params[0], \CURLINFO_HEADER_OUT, \true);
                } else {
        ...
        hook(null, 'curl_multi_init', pre: null, post: static function ($obj, array $params, mixed $retVal) use ($curlMultiToHandle) {
        ...
        hook(null, 'curl_multi_add_handle', pre: null, post: static function ($obj, array $params, mixed $retVal) use ($curlMultiToHandle) {
            if ($retVal == 0) {
        ...
        hook(null, 'curl_multi_remove_handle', pre: null, post: static function ($obj, array $params, mixed $retVal) use ($curlMultiToHandle) {
            if ($retVal == 0) {
        ...
        hook(null, 'curl_multi_close', pre: static function ($obj, array $params) use ($curlMultiToHandle) {
        ...
        hook(null, 'curl_multi_exec', pre: null, post: static function ($obj, array $params, mixed $retVal) use ($curlMultiToHandle, $instrumentation, $curlHandleToAttributes, &$curlSetOptInstrumentationSuppressed) {
            if ($retVal == \CURLM_OK) {
                ...
                /** @psalm-suppress PossiblyNullArrayAccess */
                if (!$mHandle['started']) {
                    // on first call to curl_multi_exec we're marking it's a transfer start for all curl handles attached to multi handle
                    $parent = Context::getCurrent();
                    $propagator = Globals::propagator();
                    ...
                    foreach ($handles as $cHandle => &$metadata) {
                        ...
                        if (self::isRequestHeadersCapturingEnabled()) {
                            if (!$curlHandleToAttributes[$cHandle]->isVerboseEnabled()) {
                                // we let go of captuing request headers because CURLINFO_HEADER_OUT is disabling CURLOPT_VERBOSE
                                curl_setopt($cHandle, \CURLINFO_HEADER_OUT, \true);
                            }
                        }
                        $curlSetOptInstrumentationSuppressed = \false;
                        $metadata['span'] = $span;
                ...
                if ($isRunning == 0) {
                    ...
                    foreach ($handles as $cHandle => &$metadata) {
                        if ($metadata['finished'] == \false) {
                            $metadata['finished'] = \true;
                            self::finishMultiSpan(\CURLE_OK, $cHandle, $curlHandleToAttributes, $metadata['span']);
                            // there is no way to get information if it was OK or not without calling curl_multi_info_read
                        }
                    }
                    $mHandle['started'] = \false;
                    // reset multihandle started state in case it will be reused
        ...
        hook(null, 'curl_multi_info_read', pre: null, post: static function ($obj, array $params, mixed $retVal) use ($curlMultiToHandle, $curlHandleToAttributes) {
            $mHandle =& $curlMultiToHandle[$params[0]];
            if ($retVal != \false) {
