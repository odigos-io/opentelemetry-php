...
class Schema extends BaseSchema implements ConstraintFinderInterface
{
    ...
    protected function loadTableSchema($name)
    {
        ...
        foreach ($foreignKeys as $key) {
            if (isset($table->foreignKeys[$key['FK_NAME']])) {
                ...
                $table->foreignKeys[$key['FK_NAME']] = [$key['PKTABLE_NAME'], $key['FKCOLUMN_NAME'] => $key['PKCOLUMN_NAME']];
            }
        }
        return $table;
    }
    ...
    {
        $primaryKey = $this->db->getSlavePdo(\true)->cubrid_schema(\PDO::CUBRID_SCH_PRIMARY_KEY, $tableName);
        if (empty($primaryKey)) {
            return null;
        }
        ArrayHelper::multisort($primaryKey, 'KEY_SEQ', \SORT_ASC, \SORT_NUMERIC);
        return new Constraint(['name' => $primaryKey[0]['KEY_NAME'], 'columnNames' => ArrayHelper::getColumn($primaryKey, 'ATTR_NAME')]);
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        static $actionTypes = [0 => 'CASCADE', 1 => 'RESTRICT', 2 => 'NO ACTION', 3 => 'SET NULL'];
        $foreignKeys = $this->db->getSlavePdo(\true)->cubrid_schema(\PDO::CUBRID_SCH_IMPORTED_KEYS, $tableName);
        ...
        foreach ($foreignKeys as $name => $foreignKey) {
            $result[] = new ForeignKeyConstraint(['name' => $name, 'columnNames' => ArrayHelper::getColumn($foreignKey, 'FKCOLUMN_NAME'), 'foreignTableName' => $foreignKey[0]['PKTABLE_NAME'], 'foreignColumnNames' => ArrayHelper::getColumn($foreignKey, 'PKCOLUMN_NAME'), 'onDelete' => isset($actionTypes[$foreignKey[0]['DELETE_RULE']]) ? $actionTypes[$foreignKey[0]['DELETE_RULE']] : null, 'onUpdate' => isset($actionTypes[$foreignKey[0]['UPDATE_RULE']]) ? $actionTypes[$foreignKey[0]['UPDATE_RULE']] : null]);
        }
        return $result;
    }
    /**
    ...
    {
        $constraints = $this->db->getSlavePdo(\true)->cubrid_schema(\PDO::CUBRID_SCH_CONSTRAINT, $tableName);
        $constraints = ArrayHelper::index($constraints, null, ['TYPE', 'NAME']);
        ArrayHelper::multisort($constraints, 'KEY_ORDER', \SORT_ASC, \SORT_NUMERIC);
        $result = ['indexes' => [], 'uniques' => []];
        foreach ($constraints as $type => $names) {
            foreach ($names as $name => $constraint) {
                $isUnique = in_array((int) $type, [0, 2], \true);
                $result['indexes'][] = new IndexConstraint(['isPrimary' => (bool) $constraint[0]['PRIMARY_KEY'], 'isUnique' => $isUnique, 'name' => $name, 'columnNames' => ArrayHelper::getColumn($constraint, 'ATTR_NAME')]);
                if ($isUnique) {
                    $result['uniques'][] = new Constraint(['name' => $name, 'columnNames' => ArrayHelper::getColumn($constraint, 'ATTR_NAME')]);
                }
            }
        }
