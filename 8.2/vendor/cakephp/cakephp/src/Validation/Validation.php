...
class Validation
{
     ...
     * @var array<string>
     */
    protected const COMPARE_STRING = [self::COMPARE_EQUAL, self::COMPARE_NOT_EQUAL, self::COMPARE_SAME, self::COMPARE_NOT_SAME];
    /**
     ...
     */
    protected static array $_pattern = ['hostname' => '(?:[_\p{L}0-9][-_\p{L}0-9]*\.)*(?:[\p{L}0-9][-\p{L}0-9]{0,62})\.(?:(?:[a-z]{2}\.)?[a-z]{2,})', 'latitude' => '[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)', 'longitude' => '[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)'];
    /**
    ...
    {
        if (class_exists(ChronosDate::class) && $check instanceof ChronosDate || $check instanceof DateTimeInterface) {
            return \true;
        }
        if (is_object($check)) {
            return \false;
        ...
        $regex['dmy'] = '%^(?:(?:31(\/|-|\.|\x20)(?:0?[13578]|1[02]))\1|(?:(?:29|30)' . $separator . '(?:0?[13-9]|1[0-2])\2))' . $year . '$|^(?:29' . $separator . '0?2\3' . $leapYear . ')$|^(?:0?[1-9]|1\d|2[0-8])' . $separator . '(?:(?:0?[1-9])|(?:1[0-2]))\4' . $year . '$%';
        $regex['mdy'] = '%^(?:(?:(?:0?[13578]|1[02])(\/|-|\.|\x20)31)\1|(?:(?:0?[13-9]|1[0-2])' . $separator . '(?:29|30)\2))' . $year . '$|^(?:0?2' . $separator . '29\3' . $leapYear . ')$|^(?:(?:0?[1-9])|(?:1[0-2]))' . $separator . '(?:0?[1-9]|1\d|2[0-8])\4' . $year . '$%';
        $regex['ymd'] = '%^(?:(?:' . $leapYear . $separator . '(?:0?2\1(?:29)))|(?:' . $year . $separator . '(?:(?:(?:0?[13578]|1[02])\2(?:31))|(?:(?:0?[13-9]|1[0-2])\2(29|30))|(?:(?:0?[1-9])|(?:1[0-2]))\2(?:0?[1-9]|1\d|2[0-8]))))$%';
        ...
        $regex['My'] = '%^(Jan(uary)?|Feb(ruary)?|Ma(r(ch)?|y)|Apr(il)?|Ju((ly?)|(ne?))|Aug(ust)?|Oct(ober)?|(Sep(?=\b|t)t?|Nov|Dec)(ember)?)' . $separator . $fourDigitYear . '$%';
    ...
    {
        if (class_exists(ChronosTime::class) && $check instanceof ChronosTime || $check instanceof DateTimeInterface) {
            return \true;
        }
        if (is_array($check)) {
            $check = static::_getDateString($check);
    ...
    public static function localizedTime(mixed $check, string $type = 'datetime', string|int|null $format = null): bool
    {
        if (!class_exists(DateTime::class)) {
            throw new CakeException('The Cake\I18n\DateTime class is not available. Install the cakephp/i18n package.');
        }
        if (class_exists(ChronosTime::class) && $check instanceof ChronosTime || $check instanceof DateTimeInterface) {
            return \true;
        }
        ...
        static $methods = ['date' => 'parseDate', 'time' => 'parseTime', 'datetime' => 'parseDateTime'];
     ...
     * @return bool Success.
     */
    public static function boolean(mixed $check, array $booleanValues = [\true, \false, 0, 1, '0', '1']): bool
    {
        return in_array($check, $booleanValues, \true);
    }
    /**
     ...
     * @return bool Success.
     */
    public static function truthy(mixed $check, array $truthyValues = [\true, 1, '1']): bool
    {
        return in_array($check, $truthyValues, \true);
    }
    /**
    ...
    {
        return in_array($check, $falseyValues, \true);
    }
    /**
    ...
    {
        if ($check instanceof $enumClassName && $check instanceof BackedEnum) {
    ...
    protected static function isValidEnum(BackedEnum $enum, array $options): bool
    {
        ...
        if ($options['only']) {
            ...
            if (in_array($enum, $options['only'], \true)) {
                return \true;
            }
            return \false;
        }
        if ($options['except']) {
            ...
            if (in_array($enum, $options['except'], \true)) {
                return \false;
            }
        }
        return \true;
    ...
    public static function ipOrRange(mixed $check, string $type = 'both'): bool
    {
        ...
        if (in_array($type, ['both', 'ipv4', \true]) && filter_var($ip, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV4)) {
            return is_numeric($mask) && $mask >= 0 && $mask <= 32;
        }
        if (in_array($type, ['both', 'ipv6', \true]) && filter_var($ip, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV6)) {
    ...
    public static function url(mixed $check, bool $strict = \false): bool
    {
        ...
        // phpcs:disable Generic.Files.LineLength
        $regex = '/^(?:(?:https?|ftps?|sftp|file|news|gopher):\/\/)' . ($strict ? '' : '?') . '(?:' . static::$_pattern['IPv4'] . '|\[' . static::$_pattern['IPv6'] . '\]|' . static::$_pattern['hostname'] . ')(?::[1-9][0-9]{0,4})?' . '(?:\/' . $path . '*)?' . '(?:\?' . $fragmentAndQuery . '*)?' . '(?:#' . $fragmentAndQuery . '*)?$/iu';
        // phpcs:enable Generic.Files.LineLength
        return static::_check($check, $regex);
    }
    /**
    ...
    public static function inList(mixed $check, array $list, bool $caseInsensitive = \false): bool
    {
        ...
        return in_array((string) $check, $list, \true);
    }
    /**
    ...
    public static function mimeType(mixed $check, array|string $mimeTypes = []): bool
    {
        ...
        return in_array(strtolower($mime), $mimeTypes, \true);
    }
    /**
    ...
    public static function uploadError(mixed $check, bool $allowNoFile = \false): bool
    {
        ...
        if ($allowNoFile) {
            return in_array((int) $code, [\UPLOAD_ERR_OK, \UPLOAD_ERR_NO_FILE], \true);
        }
    ...
    public static function uploadedFile(mixed $file, array $options = []): bool
    {
        ...
        $options += ['minSize' => null, 'maxSize' => null, 'types' => null, 'optional' => \false];
    ...
    {
        return self::imageSize($file, ['width' => [$operator, $width]]);
    }
    /**
    ...
    {
        return self::imageSize($file, ['height' => [$operator, $height]]);
    }
    /**
    ...
    public static function geoCoordinate(mixed $value, array $options = []): bool
    {
        ...
        $options += ['format' => 'both', 'type' => 'latLong'];
        if ($options['type'] !== 'latLong') {
            throw new InvalidArgumentException(sprintf('Unsupported coordinate type `%s`. Use `latLong` instead.', $options['type']));
        }
    ...
    {
        if (!is_string($check) || !preg_match('/^[A-Z]{2}[0-9]{2}[A-Z0-9]{1,30}$/', $check)) {
            return \false;
        }
