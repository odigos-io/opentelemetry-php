...
class TableSchema implements \Cake\Database\Schema\TableSchemaInterface, \Cake\Database\Schema\SqlGeneratorInterface
{
     ...
     */
    public static array $columnLengths = ['tiny' => self::LENGTH_TINY, 'medium' => self::LENGTH_MEDIUM, 'long' => self::LENGTH_LONG];
    /**
     ...
     * @var array<string, mixed>
     */
    protected static array $_columnKeys = ['type' => null, 'baseType' => null, 'length' => null, 'precision' => null, 'null' => null, 'default' => null, 'comment' => null];
    /**
     ...
     * @var array<string, mixed>
     */
    protected static array $_indexKeys = ['type' => null, 'columns' => [], 'length' => [], 'references' => [], 'include' => null, 'update' => 'restrict', 'delete' => 'restrict', 'constraint' => null, 'deferrable' => null, 'expression' => null];
    /**
     * Names of the valid index types.
     *
     * @var array<string>
     */
    protected static array $_validIndexTypes = [self::INDEX_INDEX, self::INDEX_FULLTEXT];
    /**
     * Names of the valid constraint types.
     *
     * @var array<string>
     */
    protected static array $_validConstraintTypes = [self::CONSTRAINT_PRIMARY, self::CONSTRAINT_UNIQUE, self::CONSTRAINT_FOREIGN, self::CONSTRAINT_CHECK];
    /**
     * Names of the valid foreign key actions.
     *
     * @var array<string>
     */
    protected static array $_validForeignKeyActions = [self::ACTION_CASCADE, self::ACTION_SET_NULL, self::ACTION_SET_DEFAULT, self::ACTION_NO_ACTION, self::ACTION_RESTRICT];
    /**
    ...
    public function setColumnType(string $name, string $type)
    {
        ...
        $this->_columns[$name]->setType($type)->setBaseType(null);
    ...
    public function isNullable(string $name): bool
    {
        ...
        return $this->_columns[$name]->getNull() === \true;
    }
    /**
     * @inheritDoc
     */
    public function defaultValues(): array
    {
        ...
        foreach ($this->_columns as $column) {
            ...
            if ($default === null && $column->getNull() !== \true && $column->getName()) {
                continue;
            }
    ...
    public function addConstraint(string $name, array|string $attrs)
    {
        ...
        if ($type === static::CONSTRAINT_FOREIGN) {
            ...
            if ($constraint instanceof \Cake\Database\Schema\ForeignKey) {
                ...
                $constraint->setColumns(array_unique(array_merge((array) $constraint->getColumns(), $attrs['columns'])));
                if ($constraint->getReferencedTable()) {
                    $constraint->setColumns(array_unique(array_merge((array) $constraint->getReferencedColumns(), [$attrs['references'][1]])));
                }
                return $this;
            }
        }
    ...
    public function __unserialize(array $data): void
    {
        ...
        $this->_typeMap = $data["\x00*\x00_typeMap"] ?? [];
