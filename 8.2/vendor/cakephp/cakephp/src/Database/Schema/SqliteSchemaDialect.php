...
class SqliteSchemaDialect extends SchemaDialect
{
    ...
    public function describeForeignKeys(string $tableName): array
    {
        ...
        foreach ($statement->fetchAll('assoc') as $row) {
            $id = $row['id'];
            if (!isset($keys[$id])) {
                $keys[$id] = ['name' => $id, 'type' => TableSchema::CONSTRAINT_FOREIGN, 'columns' => [], 'references' => [$row['table'], []], 'update' => $this->_convertOnClause($row['on_update'] ?? ''), 'delete' => $this->_convertOnClause($row['on_delete'] ?? ''), 'deferrable' => null];
            }
    ...
    public function describeCheckConstraints(string $tableName): array
    {
        ...
        if (preg_match_all($pattern, $createSql, $matches, \PREG_SET_ORDER)) {
            foreach ($matches as $index => $match) {
                ...
                $constraints[] = ['name' => $name, 'type' => TableSchema::CONSTRAINT_CHECK, 'expression' => $expression];
            }
        }
        return $constraints;
    }
    ...
    public function columnDefinitionSql(array $column): string
    {
        ...
        $hasLength = [TableSchemaInterface::TYPE_BINARY, TableSchemaInterface::TYPE_STRING, TableSchemaInterface::TYPE_CHAR, TableSchemaInterface::TYPE_TINYINTEGER, TableSchemaInterface::TYPE_SMALLINTEGER, TableSchemaInterface::TYPE_INTEGER];
