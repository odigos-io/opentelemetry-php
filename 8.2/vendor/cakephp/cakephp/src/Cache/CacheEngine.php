...
abstract class CacheEngine implements CacheInterface, \Cake\Cache\CacheEngineInterface, EventDispatcherInterface
{
     ...
     */
    protected array $_defaultConfig = ['duration' => 3600, 'groups' => [], 'prefix' => 'cake_', 'warnOnWriteFailures' => \true];
    /**
    ...
    public function add(string $key, mixed $value): bool
    {
        ...
        $this->dispatchEvent(CacheBeforeAddEvent::NAME, ['key' => $prefixedKey, 'value' => $value, 'ttl' => $duration]);
        if ($cachedValue === null) {
            $success = $this->set($key, $value);
            $this->_eventClass = CacheAfterAddEvent::class;
            $this->dispatchEvent(CacheAfterAddEvent::NAME, ['key' => $prefixedKey, 'value' => $value, 'success' => $success, 'ttl' => $duration]);
            return $success;
        }
        $this->_eventClass = CacheAfterAddEvent::class;
        $this->dispatchEvent(CacheAfterAddEvent::NAME, ['key' => $prefixedKey, 'value' => $value, 'success' => \false, 'ttl' => $duration]);
        return \false;
    }
    /**
