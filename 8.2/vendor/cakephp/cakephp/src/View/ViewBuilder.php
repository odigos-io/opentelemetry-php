...
class ViewBuilder implements JsonSerializable
{
     ...
     * @var bool
     */
    protected bool $_autoLayout = \true;
    /**
     ...
     * @return $this
     */
    public function setVars(array $data, bool $merge = \true)
    {
        if ($merge) {
     ...
     * @return $this
     */
    public function setOptions(array $options, bool $merge = \true)
    {
        if ($merge) {
    ...
    {
        if (!in_array($strategy, [self::MERGE_DEEP, self::MERGE_SHALLOW], \true)) {
    ...
    public function build(?ServerRequest $request = null, ?Response $response = null, ?EventManagerInterface $events = null): \Cake\View\View
    {
        ...
        $data = ['name' => $this->_name, 'templatePath' => $this->_templatePath, 'template' => $this->_template, 'plugin' => $this->_plugin, 'theme' => $this->_theme, 'layout' => $this->_layout, 'autoLayout' => $this->_autoLayout, 'layoutPath' => $this->_layoutPath, 'helpers' => $this->_helpers, 'viewVars' => $this->_vars, 'configMergeStrategy' => $this->_configMergeStrategy];
        $data += $this->_options;
        /** @var \Cake\View\View */
    ...
    {
        $properties = ['_templatePath', '_template', '_plugin', '_theme', '_layout', '_autoLayout', '_layoutPath', '_name', '_className', '_options', '_helpers', '_vars', '_configMergeStrategy'];
        $array = [];
    ...
    protected function _checkViewVars(mixed &$item, string $key): void
    {
        ...
        if (is_resource($item) || $item instanceof Closure || $item instanceof PDO) {
            throw new InvalidArgumentException(sprintf('Failed serializing the `%s` %s in the `%s` view var', is_resource($item) ? get_resource_type($item) : $item::class, is_resource($item) ? 'resource' : 'object', $key));
        }
    }
    /**
