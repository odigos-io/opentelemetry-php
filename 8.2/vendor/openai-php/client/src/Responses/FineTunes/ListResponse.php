...
final class ListResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    {
        $data = array_map(fn(array $result): \OpenAI\Responses\FineTunes\RetrieveResponse => \OpenAI\Responses\FineTunes\RetrieveResponse::from($result, $meta), $attributes['data']);
    ...
    {
        return ['object' => $this->object, 'data' => array_map(static fn(\OpenAI\Responses\FineTunes\RetrieveResponse $response): array => $response->toArray(), $this->data)];
    }
}
