...
final class Psr4DirectoryLoader extends Loader implements DirectoryAwareLoaderInterface
{
    ...
    private function loadFromDirectory(string $directory, string $psr4Prefix, array $excluded = []): RouteCollection
    {
        ...
        $files = iterator_to_array(new \RecursiveIteratorIterator(new \RecursiveCallbackFilterIterator(new \RecursiveDirectoryIterator($directory, \FilesystemIterator::SKIP_DOTS | \FilesystemIterator::FOLLOW_SYMLINKS), static fn(\SplFileInfo $current) => !str_starts_with($current->getBasename(), '.')), \RecursiveIteratorIterator::SELF_FIRST));
