...
final class LookupStage implements StageInterface, OperatorInterface
{
    ...
    public const PROPERTIES = ['as' => 'as', 'from' => 'from', 'localField' => 'localField', 'foreignField' => 'foreignField', 'let' => 'let', 'pipeline' => 'pipeline'];
     ...
     * The pipeline cannot directly access the joined document fields. Instead, define variables for the joined document fields using the let option and then reference the variables in the pipeline stages.
     */
    public function __construct(string $as, Optional|string $from = Optional::Undefined, Optional|string $localField = Optional::Undefined, Optional|string $foreignField = Optional::Undefined, Optional|Document|Serializable|stdClass|array $let = Optional::Undefined, Optional|PackedArray|Pipeline|BSONArray|array $pipeline = Optional::Undefined)
    {
        $this->as = $as;
        $this->from = $from;
