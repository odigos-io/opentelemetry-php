...
class DataTablePrompt extends \Laravel\Prompts\Prompt
{
     ...
     */
    public function __construct(array|Collection $headers = [], array|Collection|null $rows = null, public int $scroll = 10, public string $label = '', public string $hint = '', public bool|string $required = \false, public mixed $validate = null, public ?Closure $transform = null, public ?Closure $filter = null)
    {
        if ($rows === null) {
            $rows = $headers;
            $headers = [];
        ...
        $this->trackTypedValue(submit: \false, ignore: fn($key) => $this->state !== 'search');
    ...
    public function filteredRows(): array
    {
        ...
        if ($this->filter !== null) {
            return $this->filteredCache = array_filter($this->rows, fn($row) => ($this->filter)($row, $this->typedValue));
        }
        return $this->filteredCache = array_filter($this->rows, fn($row) => str_contains(mb_strtolower(implode(' ', $row)), mb_strtolower($this->typedValue)));
    }
    /**
    ...
    {
        return array_slice($this->filteredRows(), $this->firstVisible, $this->scroll, preserve_keys: \true);
    }
    /**
