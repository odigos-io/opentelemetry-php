...
class AutoCompletePrompt extends \Laravel\Prompts\Prompt
{
     ...
     */
    public function __construct(public string $label, array|Collection|Closure $options = [], public string $placeholder = '', public string $default = '', public bool|string $required = \false, public mixed $validate = null, public string $hint = '', public ?Closure $transform = null)
    {
    ...
    public function valueWithCursor(int $maxWidth): string
    {
        ...
        if ($ghostText !== '' && $this->cursorPosition >= mb_strlen($this->value())) {
            ...
            return $this->value() . $this->inverse($cursorChar) . $this->dim($remainingGhost);
        }
        return $this->addCursor($this->value(), $this->cursorPosition, $maxWidth) . $this->dim($ghostText);
    }
    /**
    ...
    public function matches(): array
    {
        ...
        return $this->matches = array_values(array_filter($this->options, fn($option) => str_starts_with(strtolower($option), strtolower($this->value()))));
    }
    /**
