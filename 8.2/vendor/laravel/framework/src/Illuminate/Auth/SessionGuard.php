...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
     ...
     */
    public function __construct($name, UserProvider $provider, Session $session, ?Request $request = null, ?Timebox $timebox = null, bool $rehashOnLogin = \true, int $timeboxDuration = 200000, ?string $hashKey = null)
    {
        $this->name = $name;
    ...
    public function user()
    {
        ...
        if (is_null($this->user) && !is_null($recaller = $this->recaller())) {
            ...
            if ($this->user) {
                ...
                $this->fireLoginEvent($this->user, \true);
            }
        }
        return $this->user;
    }
    ...
    {
        $this->getCookieJar()->queue($this->createRecaller($user->getAuthIdentifier() . '|' . $user->getRememberToken() . '|' . $this->hashPasswordForCookie($user->getAuthPassword())));
    }
    /**
    ...
    protected function rehashUserPasswordForDeviceLogout($password)
    {
        ...
        $this->provider->rehashPasswordIfRequired($user, ['password' => $password], force: \true);
    }
    /**
