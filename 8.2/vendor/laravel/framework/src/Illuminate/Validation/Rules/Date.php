...
class Date implements Stringable
{
    ...
    {
        return $date instanceof DateTimeInterface ? $date->format($this->format ?? 'Y-m-d') : $date;
    }
    /**
    ...
    {
        return implode('|', [$this->format === null ? 'date' : 'date_format:' . $this->format, ...$this->constraints]);
    }
}
