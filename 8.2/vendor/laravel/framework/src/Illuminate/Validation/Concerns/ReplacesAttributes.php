...
trait ReplacesAttributes
{
    ...
    {
        return str_replace([':values', ':VALUES', ':Values'], [implode(' / ', $this->getAttributeList($parameters)), Str::upper(implode(' / ', $this->getAttributeList($parameters))), implode(' / ', array_map(Str::ucfirst(...), $this->getAttributeList($parameters)))], $message);
    }
    /**
    ...
    protected function replaceIn($message, $attribute, $rule, $parameters)
    {
        ...
        return str_replace([':values', ':VALUES', ':Values'], [implode(', ', $parameters), Str::upper(implode(', ', $parameters)), implode(', ', array_map(Str::ucfirst(...), $parameters))], $message);
    }
    /**
    ...
    protected function replaceRequiredUnless($message, $attribute, $rule, $parameters)
    {
        ...
        return str_replace([':other', ':OTHER', ':Other', ':values', ':VALUES', ':Values'], [$other, Str::upper($other), Str::ucfirst($other), implode(', ', $values), Str::upper(implode(', ', $values)), implode(', ', array_map(Str::ucfirst(...), $values))], $message);
    }
    /**
    ...
    {
        return str_replace([':other', ':OTHER', ':Other'], [implode(' / ', $this->getAttributeList($parameters)), Str::upper(implode(' / ', $this->getAttributeList($parameters))), implode(' / ', array_map(Str::ucfirst(...), $this->getAttributeList($parameters)))], $message);
    }
    /**
    ...
    private function replaceWhileKeepingCase(string $message, array $mapping): string
    {
        ...
        $cases = array_reduce(array_keys($mapping), fn(array $carry, string $placeholder) => [...$carry, ...array_map(fn(callable $fn) => ':' . $fn($placeholder), $fn)], []);
        $replacements = array_reduce(array_values($mapping), fn(array $carry, string $parameter) => [...$carry, ...array_map(fn(callable $fn) => $fn($parameter), $fn)], []);
