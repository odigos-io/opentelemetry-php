...
class Deserializer
{
        ...
        if (isset($schema['properties']) && is_array($schema['properties'])) {
            ...
            foreach ($schema['properties'] as $key => $definition) {
                if (!is_array($definition)) {
                    throw new InvalidArgumentException("Unable to represent the schema for property [{$key}]; boolean schemas are not supported.");
                }
                $property = $this->build($definition, $refs);
                if (in_array((string) $key, $required, \true)) {
                    $property->required();
                }
    ...
    protected function resolveType(array $schema): array
    {
        ...
        $nullable = \false;
        if (is_array($type)) {
            $nullable = in_array('null', $type, \true);
            $names = array_values(array_unique(array_map('strval', array_filter($type, static fn($value) => $value !== 'null'))));
            if (count($names) > 1) {
    ...
    {
        $keywords = ['minLength', 'maxLength', 'pattern', 'format', 'minimum', 'maximum', 'multipleOf', 'items', 'minItems', 'maxItems', 'uniqueItems', 'properties', 'required', 'additionalProperties'];
        $unsupported = array_values(array_intersect($keywords, array_keys($schema)));
        if ($unsupported !== []) {
            throw new InvalidArgumentException('Type-specific keywords [' . implode(', ', $unsupported) . '] are not supported on a multi-type JSON Schema union.');
        }
    }
    /**
    ...
    protected function inferEnumType(array $enum): ?string
    {
        ...
        foreach ($enum as $value) {
            ...
            if (in_array($resolved, ['integer', 'number'], \true) && in_array($current, ['integer', 'number'], \true)) {
                $resolved = 'number';
                continue;
            }
            return null;
    ...
    protected function normalizeUnions(array $schema, array $refs = []): array
    {
        foreach (['anyOf', 'oneOf'] as $key) {
            ...
            if (!$nullable || count($branches) !== 1) {
                throw new InvalidArgumentException("Only a nullable \"{$key}\" (a single schema plus a \"null\" branch) is supported.");
            }
            ...
            foreach ($siblings as $siblingKey => $value) {
                if (array_key_exists($siblingKey, $branch) && $branch[$siblingKey] !== $value) {
                    throw new InvalidArgumentException("Conflicting [{$siblingKey}] between a \"{$key}\" branch and its sibling keys.");
                }
            }
            return [array_merge($siblings, $branch), \true, $branchRefs];
        }
