...
class JsonApiRequest extends Request
{
    ...
    public function sparseFields(string $key): array
    {
        if (is_null($this->cachedSparseFields)) {
            $this->cachedSparseFields = (new Collection($this->array('fields')))->transform(fn($fieldsets) => empty($fieldsets) ? [] : explode(',', $fieldsets))->all();
        }
    ...
    public function sparseIncluded(?string $key = null): ?array
    {
        if (is_null($this->cachedSparseIncluded)) {
            ...
            $this->cachedSparseIncluded = (new Collection(empty($included) ? [] : explode(',', $included)))->transform(function ($item) {
                $with = null;
            ...
            })->mapToGroups(fn($item) => [$item['relation'] => $item['with']])->toArray();
        }
        if (is_null($key)) {
