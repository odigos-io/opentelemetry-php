...
class BroadcastingInstallCommand extends Command
{
     ...
     * @var array
     */
    protected $frameworkPackages = ['react' => '@laravel/echo-react', 'vue' => '@laravel/echo-vue'];
    /**
    ...
    public function handle()
    {
        ...
        Env::writeVariable('BROADCAST_CONNECTION', $this->driver, $this->laravel->basePath('.env'), \true);
        ...
        if ($this->isUsingSupportedFramework()) {
            ...
            if (file_exists($bootstrapScriptPath = $this->laravel->resourcePath('js/bootstrap.js'))) {
                ...
                if (!str_contains($bootstrapScript, './echo')) {
                    file_put_contents($bootstrapScriptPath, trim($bootstrapScript . \PHP_EOL . file_get_contents(__DIR__ . '/stubs/echo-bootstrap-js.stub')) . \PHP_EOL);
                }
                ...
                if (!str_contains($appScript, './echo')) {
                    file_put_contents($appScriptPath, trim($appScript . \PHP_EOL . file_get_contents(__DIR__ . '/stubs/echo-bootstrap-js.stub')) . \PHP_EOL);
                }
            }
        }
        $this->installReverb();
    ...
    protected function uncommentChannelsRoutesFile()
    {
        ...
        if (str_contains($content, '// channels: ')) {
            (new Filesystem())->replaceInFile('// channels: ', 'channels: ', $appBootstrapPath);
        } elseif (str_contains($content, 'channels: ')) {
            return;
        } elseif (str_contains($content, 'commands: __DIR__.\'/../routes/console.php\',')) {
            (new Filesystem())->replaceInFile('commands: __DIR__.\'/../routes/console.php\',', 'commands: __DIR__.\'/../routes/console.php\',' . \PHP_EOL . '        channels: __DIR__.\'/../routes/channels.php\',', $appBootstrapPath);
        } elseif (str_contains($content, '->withRouting(')) {
            (new Filesystem())->replaceInFile('->withRouting(', '->withRouting(' . \PHP_EOL . '        channels: __DIR__.\'/../routes/channels.php\',', $appBootstrapPath);
        } else {
    ...
    protected function enableBroadcastServiceProvider()
    {
        ...
        if (str_contains($config, '// App\Providers\BroadcastServiceProvider::class')) {
            $filesystem->replaceInFile('// App\Providers\BroadcastServiceProvider::class', 'Odigos\App\Providers\BroadcastServiceProvider::class', app()->configPath('app.php'));
        }
    }
    /**
    ...
    protected function collectPusherConfig()
    {
        ...
        Env::writeVariables(['PUSHER_APP_ID' => $appId, 'PUSHER_APP_KEY' => $key, 'PUSHER_APP_SECRET' => $secret, 'PUSHER_APP_CLUSTER' => $cluster, 'PUSHER_PORT' => 443, 'PUSHER_SCHEME' => 'https', 'VITE_PUSHER_APP_KEY' => '${PUSHER_APP_KEY}', 'VITE_PUSHER_APP_CLUSTER' => '${PUSHER_APP_CLUSTER}', 'VITE_PUSHER_HOST' => '${PUSHER_HOST}', 'VITE_PUSHER_PORT' => '${PUSHER_PORT}', 'VITE_PUSHER_SCHEME' => '${PUSHER_SCHEME}'], $this->laravel->basePath('.env'));
    }
    /**
    ...
    protected function collectAblyConfig()
    {
        ...
        Env::writeVariables(['ABLY_KEY' => $key, 'ABLY_PUBLIC_KEY' => $publicKey, 'VITE_ABLY_PUBLIC_KEY' => '${ABLY_PUBLIC_KEY}'], $this->laravel->basePath('.env'));
    }
    /**
    ...
    protected function injectFrameworkSpecificConfiguration()
    {
        if ($this->appUsesVue()) {
            ...
            $filePaths = [$this->laravel->resourcePath('js/app.ts'), $this->laravel->resourcePath('js/app.js')];
        } else {
            $importPath = $this->frameworkPackages['react'];
            $filePaths = [$this->laravel->resourcePath('js/app.tsx'), $this->laravel->resourcePath('js/app.jsx')];
        }
...
JS;
    ...
    protected function installReverb()
    {
        ...
        if (!confirm('Would you like to install Laravel Reverb?', default: \true)) {
            return;
        }
        $this->requireComposerPackages($this->option('composer'), ['laravel/reverb:^1.0']);
    ...
    {
        if ($this->option('without-node') || !confirm('Would you like to install and build the Node dependencies required for broadcasting?', default: \true)) {
            return;
        }
    ...
    protected function packageDependenciesInclude(string $package): bool
    {
        ...
        $packageJson = json_decode(file_get_contents($packageJsonPath), \true);
