...
class VendorPublishCommand extends Command
{
     ...
     * @var bool
     */
    protected static $updateMigrationDates = \true;
    /**
    ...
    {
        return array_merge(['All providers and tags'], preg_filter('/^/', '<fg=gray>Provider:</> ', Arr::sort(ServiceProvider::publishableProviders())), preg_filter('/^/', '<fg=gray>Tag:</> ', Arr::sort(ServiceProvider::publishableGroups())));
    }
    /**
    ...
    protected function publishFile($from, $to)
    {
        if (!$this->option('existing') && (!$this->files->exists($to) || $this->option('force')) || $this->option('existing') && $this->files->exists($to)) {
            ...
            $this->components->twoColumnDetail(sprintf('File [%s] does not exist', str_replace(base_path() . '/', '', $to)), '<fg=yellow;options=bold>SKIPPED</>');
        } else {
            $this->components->twoColumnDetail(sprintf('File [%s] already exists', str_replace(base_path() . '/', '', realpath($to))), '<fg=yellow;options=bold>SKIPPED</>');
        }
    }
    /**
    ...
    protected function publishDirectory($from, $to)
    {
        ...
        $this->moveManagedFiles($from, new MountManager(['from' => new Flysystem(new LocalAdapter($from)), 'to' => new Flysystem(new LocalAdapter($to, $visibility))]));
    ...
    {
        foreach ($manager->listContents('from://', \true)->sortByPath() as $file) {
            $path = Str::after($file['path'], 'from://');
            if ($file['type'] === 'file' && (!$this->option('existing') && (!$manager->fileExists('to://' . $path) || $this->option('force')) || $this->option('existing') && $manager->fileExists('to://' . $path))) {
