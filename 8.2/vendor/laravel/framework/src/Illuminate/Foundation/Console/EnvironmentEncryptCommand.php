...
class EnvironmentEncryptCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $keyPassed = $key !== null;
        $environmentFile = $this->option('env') ? Str::finish(dirname($this->laravel->environmentFilePath()), \DIRECTORY_SEPARATOR) . '.env.' . $this->option('env') : $this->laravel->environmentFilePath();
        ...
        try {
            ...
            $encrypted = $this->option('readable') ? $this->encryptReadableFormat($contents, $encrypter) : $encrypter->encrypt($contents);
