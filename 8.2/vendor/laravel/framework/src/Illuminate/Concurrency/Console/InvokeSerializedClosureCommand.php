...
class InvokeSerializedClosureCommand extends Command
{
    ...
    {
        try {
            $this->output->write(json_encode(['successful' => \true, 'result' => serialize($this->laravel->call(match (\true) {
                !is_null($this->argument('code')) => unserialize($this->argument('code')),
                isset($_SERVER['LARAVEL_INVOKABLE_CLOSURE']) => unserialize(base64_decode($_SERVER['LARAVEL_INVOKABLE_CLOSURE'])),
                default => fn() => null,
            }))]));
        } catch (Throwable $e) {
            report($e);
            ...
            $this->output->write(json_encode(['successful' => \false, 'exception' => get_class($e), 'message' => $e->getMessage(), 'file' => $e->getFile(), 'line' => $e->getLine(), 'parameters' => $parameters]));
        }
    }
}
