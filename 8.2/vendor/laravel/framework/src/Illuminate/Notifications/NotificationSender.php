...
class NotificationSender
{
    ...
    public function __construct($manager, $bus, $events, $locale = null)
    {
        ...
        $this->events->listen(NotificationFailed::class, fn() => $this->failedEventWasDispatched = \true);
    }
    /**
    ...
    protected function sendToNotifiable($notifiable, $id, $notification, $channel)
    {
        ...
        try {
            ...
            if (!$this->failedEventWasDispatched) {
                ...
                $this->events->dispatch(new NotificationFailed($notifiable, $notification, $channel, ['exception' => $exception]));
            }
    ...
    {
        if (method_exists($notification, 'shouldSend') && $notification->shouldSend($notifiable, $channel) === \false) {
            return \false;
        }
    ...
    protected function formatNotifiables($notifiables)
    {
        if (!$notifiables instanceof Collection && !is_array($notifiables)) {
            return $notifiables instanceof Model ? new EloquentCollection([$notifiables]) : [$notifiables];
        }
        return $notifiables;
    }
}
