...
trait ValidatesAttributes
{
    ...
    public function validateBetween($attribute, $value, $parameters)
    {
        ...
        return with(BigNumber::of($this->getSize($attribute, $value)), fn($size) => $size->isGreaterThanOrEqualTo($this->trim($parameters[0])) && $size->isLessThanOrEqualTo($this->trim($parameters[1])));
    }
    /**
    ...
    public function validateEmail($attribute, $value, $parameters)
    {
        ...
        $validations = collect($parameters)->unique()->map(fn($validation) => match (\true) {
