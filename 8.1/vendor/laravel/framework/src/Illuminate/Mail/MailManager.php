...
class MailManager implements FactoryContract
{
    ...
    protected function resolve($name)
    {
        ...
        $mailer = new \Illuminate\Mail\Mailer($name, $this->app['view'], $this->createSymfonyTransport($config), $this->app['events']);
    ...
    protected function createSmtpTransport(array $config)
    {
        ...
        if (!$scheme) {
            $scheme = !empty($config['encryption']) && $config['encryption'] === 'tls' ? $config['port'] == 465 ? 'smtps' : 'smtp' : '';
        }
    ...
    protected function createPostmarkTransport(array $config)
    {
        ...
        return $factory->create(new Dsn('postmark+api', 'default', $config['token'] ?? $this->app['config']->get('services.postmark.token'), null, null, $options));
    }
    /**
