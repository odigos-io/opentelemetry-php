...
class Mailable implements MailableContract, Renderable
{
    ...
    public function send($mailer)
    {
        return $this->withLocale($this->locale, function () use ($mailer) {
            ...
            $mailer = $mailer instanceof MailFactory ? $mailer->mailer($this->mailer) : $mailer;
            return $mailer->send($this->buildView(), $this->buildViewData(), function ($message) {
                $this->buildFrom($message)->buildRecipients($message)->buildSubject($message)->buildTags($message)->buildMetadata($message)->runCallbacks($message)->buildAttachments($message);
            });
        });
    }
    /**
    ...
    public function queue(Queue $queue)
    {
        ...
        return $queue->connection($connection)->pushOn($queueName ?: null, $this->newQueuedJob());
    }
    /**
    ...
    {
        return Container::getInstance()->make(\Illuminate\Mail\SendQueuedMailable::class, ['mailable' => $this])->through(array_merge(method_exists($this, 'middleware') ? $this->middleware() : [], $this->middleware ?? []));
    }
    /**
    ...
    {
        if (isset($this->html)) {
            return array_filter(['html' => new HtmlString($this->html), 'text' => $this->textView ?? null]);
        }
    ...
    protected function buildMarkdownView()
    {
        ...
        return ['html' => $this->buildMarkdownHtml($data), 'text' => $this->buildMarkdownText($data)];
    }
    /**
    ...
    {
        return fn($data) => $this->markdownRenderer()->render($this->markdown, array_merge($data, $viewData));
    }
    /**
    ...
    protected function buildDiskAttachments($message)
    {
        foreach ($this->diskAttachments as $attachment) {
            ...
            $message->attachData($storage->get($attachment['path']), $attachment['name'] ?? basename($attachment['path']), array_merge(['mime' => $storage->mimeType($attachment['path'])], $attachment['options']));
        }
    }
    /**
    ...
    protected function setAddress($address, $name = null, $property = 'to')
    {
        ...
        foreach ($this->addressesToArray($address, $name) as $recipient) {
            ...
            $this->{$property}[] = ['name' => $recipient->name ?? null, 'address' => $recipient->email];
        }
        $this->{$property} = collect($this->{$property})->reverse()->unique('address')->reverse()->values()->all();
        return $this;
    }
    /**
    ...
    protected function hasRecipient($address, $name = null, $property = 'to')
    {
        ...
        $expected = ['name' => $expected->name ?? null, 'address' => $expected->email];
    ...
    {
        return $this->subject === $subject || method_exists($this, 'envelope') && $this->envelope()->hasSubject($subject);
    }
    /**
    ...
    public function attach($file, array $options = [])
    {
        ...
        $this->attachments = collect($this->attachments)->push(compact('file', 'options'))->unique('file')->all();
        return $this;
    }
    /**
    ...
    public function hasAttachment($file, array $options = [])
    {
        ...
        if ($file instanceof \Illuminate\Mail\Attachment) {
            ...
            [$file, $options] = $parts === \false ? [null, []] : $parts;
        }
        return collect($this->attachments)->contains(fn($attachment) => $attachment['file'] === $file && array_filter($attachment['options']) === array_filter($options));
    }
    /**
    ...
    private function hasEnvelopeAttachment($attachment, $options = [])
    {
        ...
        return Collection::make(is_object($attachments) ? [$attachments] : $attachments)->map(fn($attached) => $attached instanceof Attachable ? $attached->toMailAttachment() : $attached)->contains(fn($attached) => $attached->isEquivalent($attachment, $options));
    }
    /**
    ...
    {
        $this->diskAttachments = collect($this->diskAttachments)->push(['disk' => $disk, 'path' => $path, 'name' => $name ?? basename($path), 'options' => $options])->unique(function ($file) {
    ...
    {
        return collect($this->diskAttachments)->contains(fn($attachment) => $attachment['disk'] === $disk && $attachment['path'] === $path && $attachment['name'] === ($name ?? basename($path)) && $attachment['options'] === $options);
    }
    /**
    ...
    {
        $this->rawAttachments = collect($this->rawAttachments)->push(compact('data', 'name', 'options'))->unique(function ($file) {
    ...
    {
        return collect($this->rawAttachments)->contains(fn($attachment) => $attachment['data'] === $data && $attachment['name'] === $name && array_filter($attachment['options']) === array_filter($options));
    }
    /**
    ...
    {
        return in_array($value, $this->tags) || method_exists($this, 'envelope') && in_array($value, $this->envelope()->tags);
    }
    /**
    ...
    {
        return isset($this->metadata[$key]) && $this->metadata[$key] === $value || method_exists($this, 'envelope') && $this->envelope()->hasMetadata($key, $value);
    }
    /**
    ...
    public function assertFrom($address, $name = null)
    {
        ...
        PHPUnit::assertTrue($this->hasFrom($address, $name), "Email was not from expected address [{$recipient}].");
        return $this;
    }
    /**
    ...
    public function assertTo($address, $name = null)
    {
        $this->renderForAssertions();
        $recipient = $this->formatAssertionRecipient($address, $name);
        PHPUnit::assertTrue($this->hasTo($address, $name), "Did not see expected recipient [{$recipient}] in email 'to' recipients.");
        return $this;
    }
    /**
     * Assert that the mailable has the given recipient.
    ...
    public function assertHasCc($address, $name = null)
    {
        $this->renderForAssertions();
        $recipient = $this->formatAssertionRecipient($address, $name);
        PHPUnit::assertTrue($this->hasCc($address, $name), "Did not see expected recipient [{$recipient}] in email 'cc' recipients.");
        return $this;
    }
    /**
     * Assert that the mailable has the given recipient.
    ...
    public function assertHasBcc($address, $name = null)
    {
        $this->renderForAssertions();
        $recipient = $this->formatAssertionRecipient($address, $name);
        PHPUnit::assertTrue($this->hasBcc($address, $name), "Did not see expected recipient [{$recipient}] in email 'bcc' recipients.");
        return $this;
    }
    /**
    ...
    public function assertHasReplyTo($address, $name = null)
    {
        ...
        PHPUnit::assertTrue($this->hasReplyTo($address, $name), "Did not see expected address [{$replyTo}] as email 'reply to' recipient.");
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasSubject($subject), "Did not see expected text [{$subject}] in email subject.");
        return $this;
    }
    /**
    ...
    {
        [$html, $text] = $this->renderForAssertions();
        PHPUnit::assertStringContainsString($string, $text, "Did not see expected text [{$string}] within text email body.");
        return $this;
    }
    /**
    ...
    {
        [$html, $text] = $this->renderForAssertions();
        PHPUnit::assertStringNotContainsString($string, $text, "Saw unexpected text [{$string}] within text email body.");
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasAttachment($file, $options), 'Did not find the expected attachment.');
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasAttachedData($data, $name, $options), 'Did not find the expected attachment.');
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasAttachmentFromStorage($path, $name, $options), 'Did not find the expected attachment.');
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasAttachmentFromStorageDisk($disk, $path, $name, $options), 'Did not find the expected attachment.');
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasTag($tag), "Did not see expected tag [{$tag}] in email tags.");
        return $this;
    }
    /**
    ...
    {
        $this->renderForAssertions();
        PHPUnit::assertTrue($this->hasMetadata($key, $value), "Did not see expected key [{$key}] and value [{$value}] in email metadata.");
        return $this;
    }
    /**
    ...
    protected function renderForAssertions()
    {
        ...
        return $this->assertionableRenderStrings = $this->withLocale($this->locale, function () {
            $this->prepareMailableForDelivery();
            $html = Container::getInstance()->make('mailer')->render($view = $this->buildView(), $this->buildViewData());
            if (is_array($view) && isset($view[1])) {
                $text = $view[1];
            }
            $text ??= $view['text'] ?? '';
            if (!empty($text) && !$text instanceof Htmlable) {
                $text = Container::getInstance()->make('mailer')->render($text, $this->buildViewData());
            }
    ...
    private function ensureAttachmentsAreHydrated()
    {
        ...
        $attachments = $this->attachments();
        Collection::make(is_object($attachments) ? [$attachments] : $attachments)->each(function ($attachment) {
