...
class PendingRequest
{
    ...
    public function __construct(?\Illuminate\Http\Client\Factory $factory = null, $middleware = [])
    {
        ...
        $this->asJson();
        $this->options = ['connect_timeout' => 10, 'http_errors' => \false, 'timeout' => 30];
        $this->beforeSendingCallbacks = collect([function (\Illuminate\Http\Client\Request $request, array $options, \Illuminate\Http\Client\PendingRequest $pendingRequest) {
    ...
    public function withCookies(array $cookies, string $domain)
    {
        return tap($this, function () use ($cookies, $domain) {
            $this->options = array_merge_recursive($this->options, ['cookies' => CookieJar::fromArray($cookies, $domain)]);
        });
    }
    /**
     ...
     * @return $this
     */
    public function retry(array|int $times, Closure|int $sleepMilliseconds = 0, ?callable $when = null, bool $throw = \true)
    {
        $this->tries = $times;
    ...
    public function withOptions(array $options)
    {
        return tap($this, function () use ($options) {
            $this->options = array_replace_recursive(array_merge_recursive($this->options, Arr::only($options, $this->mergableOptions)), $options);
        });
    }
    /**
            ...
            try {
                return tap($this->newResponse($this->sendRequest($method, $url, $options)), function ($response) use ($attempt, &$shouldRetry) {
                    ...
                    if (!$response->successful()) {
                        try {
                            $shouldRetry = $this->retryWhenCallback ? call_user_func($this->retryWhenCallback, $response->toException(), $this) : \true;
                ...
                $this->dispatchConnectionFailedEvent(new \Illuminate\Http\Client\Request($e->getRequest()));
                throw new \Illuminate\Http\Client\ConnectionException($e->getMessage(), 0, $e);
            }
        }, $this->retryDelay ?? 100, function ($exception) use (&$shouldRetry) {
            $result = $shouldRetry ?? ($this->retryWhenCallback ? call_user_func($this->retryWhenCallback, $exception, $this) : \true);
            $shouldRetry = null;
            return $result;
        });
    }
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $mergedOptions = $this->normalizeRequestOptions($this->mergeOptions(['laravel_data' => $laravelData, 'on_stats' => $onStats], $options));
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair((new \Illuminate\Http\Client\Request($request))->withData($options['laravel_data']), $this->newResponse($response));
                    return $response;
                });
            };
        };
    ...
    public function buildStubHandler()
    {
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())->map->__invoke((new \Illuminate\Http\Client\Request($request))->withData($options['laravel_data']), $options)->filter()->first();
                if (is_null($response)) {
    ...
    public function runBeforeSendingCallbacks($request, array $options)
    {
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func($callback, (new \Illuminate\Http\Client\Request($request))->withData($options['laravel_data']), $options, $this);
     ...
     * @return $this
     */
    public function preventStrayRequests($prevent = \true)
    {
     ...
     * @return void
     */
    protected function dispatchConnectionFailedEvent(\Illuminate\Http\Client\Request $request)
    {
        if ($dispatcher = $this->factory?->getDispatcher()) {
