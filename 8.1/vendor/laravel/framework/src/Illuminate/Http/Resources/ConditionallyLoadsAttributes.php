...
trait ConditionallyLoadsAttributes
{
    ...
    protected function filter($data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_numeric($key) && $value instanceof \Illuminate\Http\Resources\MergeValue) {
    ...
    protected function removeMissingValues($data)
    {
        $numericKeys = \true;
        foreach ($data as $key => $value) {
            if ($value instanceof \Illuminate\Http\Resources\PotentiallyMissing && $value->isMissing() || $value instanceof self && $value->resource instanceof \Illuminate\Http\Resources\PotentiallyMissing && $value->isMissing()) {
                unset($data[$key]);
            } else {
    ...
    {
        if ($condition) {
            return new \Illuminate\Http\Resources\MergeValue(value($value));
        }
        return func_num_args() === 3 ? new \Illuminate\Http\Resources\MergeValue(value($default)) : new \Illuminate\Http\Resources\MissingValue();
    }
    /**
    ...
    {
        return new \Illuminate\Http\Resources\MergeValue(Arr::only($this->resource->toArray(), $attributes));
    }
    /**
    ...
    protected function whenPivotLoadedAs($accessor, $table, $value, $default = null)
    {
        ...
        return $this->when($this->hasPivotLoadedAs($accessor, $table), ...[$value, $default]);
    }
    /**
    ...
    {
        return isset($this->resource->{$accessor}) && ($this->resource->{$accessor} instanceof $table || $this->resource->{$accessor}->getTable() === $table);
    }
    /**
    ...
    {
        return transform($value, $callback, func_num_args() === 3 ? $default : new \Illuminate\Http\Resources\MissingValue());
    }
}
