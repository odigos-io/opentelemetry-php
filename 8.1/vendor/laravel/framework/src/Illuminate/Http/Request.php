...
{
    use \Illuminate\Http\Concerns\CanBePrecognitive, \Illuminate\Http\Concerns\InteractsWithContentTypes, \Illuminate\Http\Concerns\InteractsWithFlashData, \Illuminate\Http\Concerns\InteractsWithInput, Macroable;
    /**
    ...
    {
        if (!isset($this->json)) {
            $this->json = new InputBag((array) json_decode($this->getContent() ?: '[]', \true));
        }
        if (is_null($key)) {
            return $this->json;
        }
    ...
    {
        $newRequest = new static($request->query->all(), $request->request->all(), $request->attributes->all(), $request->cookies->all(), (new static())->filterFiles($request->files->all()) ?? [], $request->server->all());
    ...
    {
        return $this->hasSession() ? new SymfonySessionDecorator($this->session()) : throw new SessionNotFoundException();
    }
    /**
