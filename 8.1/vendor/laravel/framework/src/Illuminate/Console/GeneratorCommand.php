...
abstract class GeneratorCommand extends \Illuminate\Console\Command implements PromptsForMissingInput
{
    ...
    protected function possibleModels()
    {
        ...
        return collect(Finder::create()->files()->depth(0)->in($modelPath))->map(fn($file) => $file->getBasename('.php'))->sort()->values()->all();
    }
    /**
    ...
    protected function possibleEvents()
    {
        ...
        return collect(Finder::create()->files()->depth(0)->in($eventPath))->map(fn($file) => $file->getBasename('.php'))->sort()->values()->all();
    }
    /**
    ...
    {
        return in_array(strtolower($name), collect($this->reservedNames)->transform(fn($name) => strtolower($name))->all());
    }
    /**
