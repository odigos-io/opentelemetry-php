...
class Task extends \Illuminate\Console\View\Components\Component
{
    ...
    public function render($description, $task = null, $verbosity = OutputInterface::VERBOSITY_NORMAL)
    {
        ...
        try {
            ...
            $runTime = $task ? ' ' . number_format((microtime(\true) - $startTime) * 1000) . 'ms' : '';
            ...
            $this->output->writeln($result !== \false ? ' <fg=green;options=bold>DONE</>' : ' <fg=red;options=bold>FAIL</>', $verbosity);
        }
    }
}
