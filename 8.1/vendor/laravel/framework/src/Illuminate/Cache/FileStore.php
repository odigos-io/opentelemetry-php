...
class FileStore implements Store, LockProvider
{
    ...
    public function put($key, $value, $seconds)
    {
        ...
        $result = $this->files->put($path, $this->expiration($seconds) . serialize($value), \true);
    ...
    protected function ensureCacheDirectoryExists($path)
    {
        ...
        if (!$this->files->exists($directory)) {
            $this->files->makeDirectory($directory, 0777, \true, \true);
    ...
    public function lock($name, $seconds = 0, $owner = null)
    {
        ...
        return new \Illuminate\Cache\FileLock(new static($this->files, $this->lockDirectory ?? $this->directory, $this->filePermission), $name, $seconds, $owner);
    }
    /**
    ...
    protected function getPayload($key)
    {
        ...
        try {
            if (is_null($contents = $this->files->get($path, \true))) {
