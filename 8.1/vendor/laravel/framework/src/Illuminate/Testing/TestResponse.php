...
{
    use \Illuminate\Testing\Concerns\AssertsStatusCodes, Conditionable, Tappable, Macroable {
        __call as macroCall;
    }
    /**
    ...
    {
        PHPUnit::assertTrue($this->isSuccessful(), $this->statusMessageWithDetails('>=200, <300', $this->getStatusCode()));
        return $this;
    }
    /**
    ...
    public function assertSuccessfulPrecognition()
    {
        ...
        PHPUnit::assertTrue($this->headers->has('Precognition-Success'), 'Header [Precognition-Success] not present on response.');
        PHPUnit::assertSame('true', $this->headers->get('Precognition-Success'), 'The Precognition-Success header was found, but the value is not `true`.');
        return $this;
    }
    /**
    ...
    {
        PHPUnit::assertTrue($this->isServerError(), $this->statusMessageWithDetails('>=500, < 600', $this->getStatusCode()));
        return $this;
    }
    /**
    ...
    {
        PHPUnit::assertTrue($this->isRedirect(), $this->statusMessageWithDetails('201, 301, 302, 303, 307, 308', $this->getStatusCode()));
        if (!is_null($uri)) {
    ...
    {
        PHPUnit::assertTrue($this->isRedirect(), $this->statusMessageWithDetails('201, 301, 302, 303, 307, 308', $this->getStatusCode()));
        PHPUnit::assertTrue(Str::contains($this->headers->get('Location'), $uri), 'Redirect location [' . $this->headers->get('Location') . '] does not contain [' . $uri . '].');
        return $this;
    }
    /**
    ...
    public function assertRedirectToSignedRoute($name = null, $parameters = [])
    {
        ...
        PHPUnit::assertTrue($request->hasValidSignature(), 'The response is not a redirect to a signed route.');
        if (!is_null($name)) {
            $expectedUri = rtrim($request->fullUrlWithQuery(['signature' => null, 'expires' => null]), '?');
            PHPUnit::assertEquals(app('url')->to($uri), $expectedUri);
        }
        return $this;
    }
    /**
    ...
    public function assertDownload($filename = null)
    {
        ...
        if (trim($contentDisposition[0]) !== 'attachment') {
            PHPUnit::fail('Response does not offer a file download.' . \PHP_EOL . 'Disposition [' . trim($contentDisposition[0]) . '] found in header, [attachment] expected.');
        }
        if (!is_null($filename)) {
            if (isset($contentDisposition[1]) && trim(explode('=', $contentDisposition[1])[0]) !== 'filename') {
                PHPUnit::fail('Unsupported Content-Disposition header provided.' . \PHP_EOL . 'Disposition [' . trim(explode('=', $contentDisposition[1])[0]) . '] found in header, [filename] expected.');
            }
            ...
            if (!isset($contentDisposition[1])) {
                ...
                PHPUnit::assertSame($filename, isset(explode('=', $contentDisposition[1])[1]) ? trim(explode('=', $contentDisposition[1])[1], " \"'") : '', $message);
                return $this;
            }
        } else {
    ...
    {
        PHPUnit::assertNotNull($cookie = $this->getCookie($cookieName, $encrypted && !is_null($value), $unserialize), "Cookie [{$cookieName}] not present on response.");
        if (!$cookie || is_null($value)) {
            return $this;
        }
        $cookieValue = $cookie->getValue();
        PHPUnit::assertEquals($value, $cookieValue, "Cookie [{$cookieName}] was found, but value [{$cookieValue}] does not match [{$value}].");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::assertNotNull($cookie = $this->getCookie($cookieName, \false), "Cookie [{$cookieName}] not present on response.");
        $expiresAt = Carbon::createFromTimestamp($cookie->getExpiresTime());
        PHPUnit::assertTrue($cookie->getExpiresTime() !== 0 && $expiresAt->lessThan(Carbon::now()), "Cookie [{$cookieName}] is not expired, it expires at [{$expiresAt}].");
        return $this;
    }
    /**
    ...
    public function assertCookieNotExpired($cookieName)
    {
        PHPUnit::assertNotNull($cookie = $this->getCookie($cookieName, \false), "Cookie [{$cookieName}] not present on response.");
        $expiresAt = Carbon::createFromTimestamp($cookie->getExpiresTime());
        PHPUnit::assertTrue($cookie->getExpiresTime() === 0 || $expiresAt->greaterThan(Carbon::now()), "Cookie [{$cookieName}] is expired, it expired at [{$expiresAt}].");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::assertNull($this->getCookie($cookieName, \false), "Cookie [{$cookieName}] is present on response.");
        return $this;
    }
    /**
    ...
    public function assertJsonValidationErrors($errors, $responseKey = 'errors')
    {
        ...
        foreach ($errors as $key => $value) {
            ...
            if ($errorMissing) {
                PHPUnit::fail("Failed to find a validation error in the response for key and message: '{$key}' => '{$expectedMessage}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
            }
        }
        return $this;
    }
    ...
    public function assertJsonValidationErrorFor($key, $responseKey = 'errors')
    {
        $jsonErrors = Arr::get($this->json(), $responseKey) ?? [];
        $errorMessage = $jsonErrors ? 'Response has the following JSON validation errors:' . \PHP_EOL . \PHP_EOL . json_encode($jsonErrors, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE) . \PHP_EOL : 'Response does not have JSON validation errors.';
        PHPUnit::assertArrayHasKey($key, $jsonErrors, "Failed to find a validation error in the response for key: '{$key}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
        return $this;
    }
    /**
    ...
    public function assertJsonMissingValidationErrors($keys = null, $responseKey = 'errors')
    {
        ...
        if (is_null($keys) && count($errors) > 0) {
            PHPUnit::fail('Response has unexpected validation errors: ' . \PHP_EOL . \PHP_EOL . json_encode($errors, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE));
        }
        foreach (Arr::wrap($keys) as $key) {
            PHPUnit::assertFalse(isset($errors[$key]), "Found unexpected validation error for key: '{$key}'");
        }
        return $this;
    }
    /**
    ...
    public function assertJsonIsArray($key = null)
    {
        ...
        PHPUnit::assertTrue(is_array($data) && str_starts_with($encodedData, '[') && str_ends_with($encodedData, ']'));
        return $this;
    }
    /**
    ...
    public function assertInvalid($errors = null, $errorBag = 'default', $responseKey = 'errors')
    {
        ...
        foreach (Arr::wrap($errors) as $key => $value) {
            PHPUnit::assertArrayHasKey($resolvedKey = is_int($key) ? $value : $key, $sessionErrors, "Failed to find a validation error in session for key: '{$resolvedKey}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
            foreach (Arr::wrap($value) as $message) {
                if (!is_int($key)) {
                    ...
                    if (!$hasError) {
                        PHPUnit::fail("Failed to find a validation error for key and message: '{$key}' => '{$message}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
                    }
                }
            }
        }
    ...
    public function assertSessionHasInput($key, $value = null)
    {
        ...
        if (is_null($value)) {
            PHPUnit::assertTrue($this->session()->hasOldInput($key), "Session is missing expected key [{$key}].");
        } elseif ($value instanceof Closure) {
    ...
    public function assertSessionHasNoErrors()
    {
        ...
        PHPUnit::assertFalse($hasErrors, 'Session has unexpected errors: ' . \PHP_EOL . \PHP_EOL . json_encode((function () use ($hasErrors) {
            $errors = [];
