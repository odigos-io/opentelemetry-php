...
class AssertableJsonString implements ArrayAccess, Countable
{
    ...
    public function __construct($jsonable)
    {
        ...
        if ($jsonable instanceof JsonSerializable) {
            ...
            $this->decoded = json_decode($jsonable->toJson(), \true);
        } elseif (is_array($jsonable)) {
            $this->decoded = $jsonable;
        } else {
            $this->decoded = json_decode($jsonable, \true);
        }
    }
    /**
    ...
    {
        if (!is_null($key)) {
            PHPUnit::assertCount($count, data_get($this->decoded, $key), "Failed to assert that the response count matched the expected {$count}");
            return $this;
        }
        PHPUnit::assertCount($count, $this->decoded, "Failed to assert that the response count matched the expected {$count}");
        return $this;
    }
    /**
    ...
    public function assertExact(array $data)
    {
        ...
        PHPUnit::assertEquals(json_encode($expected, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES), json_encode($actual, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES));
        return $this;
    }
    /**
    ...
    {
        $actual = json_encode(Arr::sortRecursive((array) $this->decoded), \JSON_UNESCAPED_UNICODE);
    ...
    public function assertFragment(array $data)
    {
        ...
        foreach (Arr::sortRecursive($data) as $key => $value) {
            ...
            PHPUnit::assertTrue(Str::contains($actual, $expected), 'Unable to find JSON fragment: ' . \PHP_EOL . \PHP_EOL . '[' . json_encode([$key => $value], \JSON_UNESCAPED_UNICODE) . ']' . \PHP_EOL . \PHP_EOL . 'within' . \PHP_EOL . \PHP_EOL . "[{$actual}].");
        }
        return $this;
    }
    /**
    ...
    public function assertMissing(array $data, $exact = \false)
    {
        ...
        foreach (Arr::sortRecursive($data) as $key => $value) {
            ...
            PHPUnit::assertFalse(Str::contains($actual, $unexpected), 'Found unexpected JSON fragment: ' . \PHP_EOL . \PHP_EOL . '[' . json_encode([$key => $value], \JSON_UNESCAPED_UNICODE) . ']' . \PHP_EOL . \PHP_EOL . 'within' . \PHP_EOL . \PHP_EOL . "[{$actual}].");
        }
        return $this;
    }
    /**
    ...
    public function assertMissingExact(array $data)
    {
        ...
        PHPUnit::fail('Found unexpected JSON fragment: ' . \PHP_EOL . \PHP_EOL . '[' . json_encode($data, \JSON_UNESCAPED_UNICODE) . ']' . \PHP_EOL . \PHP_EOL . 'within' . \PHP_EOL . \PHP_EOL . "[{$actual}].");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::assertArraySubset($data, $this->decoded, $strict, $this->assertJsonMessage($data));
        return $this;
    }
    /**
