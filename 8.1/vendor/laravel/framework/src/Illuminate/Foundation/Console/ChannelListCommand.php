...
class ChannelListCommand extends Command
{
    ...
    public function handle(Broadcaster $broadcaster)
    {
        ...
        if (!$this->laravel->providerIsLoaded('Odigos\App\Providers\BroadcastServiceProvider') && file_exists($this->laravel->path('Providers/BroadcastServiceProvider.php'))) {
    ...
    protected function forCli($channels)
    {
        ...
        return $channels->map(function ($channel, $channelName) use ($maxChannelName, $terminalWidth) {
            ...
            $dots = str_repeat('.', max($terminalWidth - mb_strlen($channelName . $spaces . $resolver) - 6, 0));
            $dots = empty($dots) ? $dots : " {$dots}";
            return sprintf('  <fg=blue;options=bold>%s</> %s<fg=white>%s</><fg=#6C7280>%s</>', $channelName, $spaces, $resolver, $dots);
        })->filter()->sort()->prepend('')->push('')->push($channelCount)->push('')->toArray();
    }
    /**
