...
class ConfigShowCommand extends Command
{
    ...
    public function render($name)
    {
        ...
        foreach (Arr::dot($data) as $key => $value) {
            $this->components->twoColumnDetail($this->formatKey($key), $this->formatValue($value));
        }
    }
    /**
     * Render the title.
    ...
    {
        $this->components->twoColumnDetail("<fg=green;options=bold>{$title}</>", $subtitle);
    }
    /**
    ...
    {
        return preg_replace_callback('/(.*)\.(.*)$/', fn($matches) => sprintf('<fg=gray>%s ⇁</> %s', str_replace('.', ' ⇁ ', $matches[1]), $matches[2]), $key);
    }
    /**
