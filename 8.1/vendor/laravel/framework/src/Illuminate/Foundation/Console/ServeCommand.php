...
class ServeCommand extends Command
{
     ...
     * @var string[]
     */
    public static $passthroughVariables = ['APP_ENV', 'HERD_PHP_81_INI_SCAN_DIR', 'HERD_PHP_82_INI_SCAN_DIR', 'HERD_PHP_83_INI_SCAN_DIR', 'IGNITION_LOCAL_SITES_PATH', 'LARAVEL_SAIL', 'PATH', 'PHP_CLI_SERVER_WORKERS', 'PHP_IDE_CONFIG', 'SYSTEMROOT', 'XDEBUG_CONFIG', 'XDEBUG_MODE', 'XDEBUG_SESSION'];
    /**
    ...
    protected function serverCommand()
    {
        ...
        return [(new PhpExecutableFinder())->find(\false), '-S', $this->host() . ':' . $this->port(), $server];
    }
    /**
    ...
    protected function handleProcessOutput()
    {
        return fn($type, $buffer) => str($buffer)->explode("\n")->each(function ($line) {
            if (str($line)->contains('Development Server (http')) {
                ...
                $this->requestsPool[$requestPort] = [$this->getDateFromLine($line), \false];
