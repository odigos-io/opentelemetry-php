...
class MultiSelectPromptRenderer extends \Laravel\Prompts\Themes\Default\Renderer implements Scrolling
{
    ...
    protected function renderOptions(MultiSelectPrompt $prompt): string
    {
        ...
        })->values(), $prompt->firstVisible, $prompt->scroll, count($prompt->options), min($this->longest($prompt->options, padding: 6), $prompt->terminal()->cols() - 6), $prompt->state === 'cancel' ? 'dim' : 'cyan')->implode(\PHP_EOL);
    }
    /**
