 ...
 */
namespace Odigos\cebe\markdown\block;
...
trait HeadlineTrait
{
    ...
    {
        return $line[0] === '#' && !preg_match('/^#\d+/', $line) || !empty($lines[$current + 1]) && (($l = $lines[$current + 1][0]) === '=' || $l === '-') && preg_match('/^(\-+|=+)\s*$/', $lines[$current + 1]);
    }
    /**
    ...
    protected function consumeHeadline($lines, $current)
    {
        if ($lines[$current][0] === '#') {
            ...
            $block = ['headline', 'content' => $this->parseInline(trim($lines[$current], "# \t")), 'level' => $level];
            return [$block, $current];
        } else {
            // underlined headline
            $block = ['headline', 'content' => $this->parseInline($lines[$current]), 'level' => $lines[$current + 1][0] === '=' ? 1 : 2];
