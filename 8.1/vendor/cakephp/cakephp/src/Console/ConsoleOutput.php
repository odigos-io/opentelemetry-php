...
class ConsoleOutput
{
     ...
     */
    protected static array $_foregroundColors = ['black' => 30, 'red' => 31, 'green' => 32, 'yellow' => 33, 'blue' => 34, 'magenta' => 35, 'cyan' => 36, 'white' => 37];
    /**
     * background colors used in colored output.
     *
     * @var array<string, int>
     */
    protected static array $_backgroundColors = ['black' => 40, 'red' => 41, 'green' => 42, 'yellow' => 43, 'blue' => 44, 'magenta' => 45, 'cyan' => 46, 'white' => 47];
    /**
     * Formatting options for colored output.
     *
     * @var array<string, int>
     */
    protected static array $_options = ['bold' => 1, 'underline' => 4, 'blink' => 5, 'reverse' => 7];
    /**
     ...
     */
    protected static array $_styles = ['emergency' => ['text' => 'red'], 'alert' => ['text' => 'red'], 'critical' => ['text' => 'red'], 'error' => ['text' => 'red'], 'error.bg' => ['background' => 'red', 'text' => 'black'], 'warning' => ['text' => 'yellow'], 'warning.bg' => ['background' => 'yellow', 'text' => 'black'], 'info' => ['text' => 'cyan'], 'info.bg' => ['background' => 'white', 'text' => 'cyan'], 'debug' => ['text' => 'yellow'], 'success' => ['text' => 'green'], 'success.bg' => ['background' => 'green', 'text' => 'black'], 'notice' => ['text' => 'cyan'], 'notice.bg' => ['background' => 'cyan', 'text' => 'black'], 'comment' => ['text' => 'blue'], 'question' => ['text' => 'magenta']];
    /**
    ...
    public function __construct($stream = 'php://stdout')
    {
        ...
        $this->_output = $stream;
        if (\DIRECTORY_SEPARATOR === '\\' && !str_contains(strtolower(php_uname('v')), 'windows 10') && !str_contains(strtolower((string) env('SHELL')), 'bash.exe') && !(bool) env('ANSICON') && env('ConEmuANSI') !== 'ON' || function_exists('posix_isatty') && !posix_isatty($this->_output) || env('NO_COLOR') !== null) {
    ...
    public function styleText(string $text): string
    {
        ...
        if ($this->_outputAs !== static::PLAIN) {
            ...
            $output = preg_replace_callback('/<(?P<tag>[a-z0-9-_.]+)>(?P<text>.*?)<\/(\1)>/ims', $replaceTags, $text);
            if ($output !== null) {
                return $output;
            }
        }
    ...
    protected function _replaceTags(array $matches): string
    {
        ...
        return "\x1b[" . implode(';', $styleInfo) . 'm' . $matches['text'] . "\x1b[0m";
    }
    /**
    ...
    {
        if (!in_array($type, [self::RAW, self::PLAIN, self::COLOR], \true)) {
