...
class View implements EventDispatcherInterface
{
     ...
     * @var array<string>
     */
    protected array $_passedVars = ['viewVars', 'autoLayout', 'helpers', 'template', 'layout', 'name', 'theme', 'layoutPath', 'templatePath', 'plugin'];
    /**
     ...
     */
    public function __construct(?ServerRequest $request = null, ?Response $response = null, ?EventManagerInterface $eventManager = null, array $viewOptions = [])
    {
        ...
        $this->setConfig(array_diff_key($viewOptions, array_flip($this->_passedVars)));
     ...
     * @return $this
     */
    public function enableAutoLayout(bool $enable = \true)
    {
    ...
    public function element(string $name, array $data = [], array $options = []): string
    {
        ...
        if (isset($options['cache'])) {
            $options['cache'] = $this->_elementCache($name, $data, array_diff_key($options, ['callbacks' => \false, 'plugin' => null, 'ignoreMissing' => null]));
        }
    ...
    public function render(?string $template = null, string|false|null $layout = null): string
    {
        ...
        if ($this->autoLayout) {
            if (!$this->layout) {
                throw new CakeException('View::$layout must be a non-empty string.' . 'To disable layout rendering use method `View::disableAutoLayout()` instead.');
            }
    ...
    public function set(array|string $name, mixed $value = null)
    {
        if (is_array($name)) {
            if (is_array($value)) {
                ...
                if ($data === \false) {
                    throw new CakeException('Invalid data provided for array_combine() to work: Both $name and $value require same count.');
                }
            } else {
                $data = $name;
            }
        ...
        switch ($type) {
            case static::TYPE_ELEMENT:
                ...
                if (!$parent) {
                    ...
                    throw new LogicException(sprintf('You cannot extend an element which does not exist (%s).', $defaultPath . $name . $this->_ext));
                }
                break;
    ...
    protected function _render(string $templateFile, array $data = []): string
    {
        ...
        if ($initialBlocks !== $remainingBlocks) {
            throw new LogicException(sprintf('The `%s` block was left open. Blocks are not allowed to cross files.', (string) $this->Blocks->active()));
        }
        return $content;
    }
    /**
     ...
     */
    protected function _getElementFileName(string $name, bool $pluginCheck = \true): string|false
    {
    ...
    protected function _getSubPaths(string $basePath): array
    {
        ...
        if ($this->request->getParam('prefix')) {
            ...
            foreach ($prefixPath as $prefixPart) {
                ...
                array_unshift($paths, $path . $basePath);
            }
        }
        return $paths;
    }
     ...
     */
    protected function _paths(?string $plugin = null, bool $cached = \true): array
    {
        if ($cached) {
        ...
        if ($plugin) {
            foreach ($templatePaths as $templatePath) {
                $pluginPaths[] = $templatePath . static::PLUGIN_TEMPLATE_FOLDER . \DIRECTORY_SEPARATOR . $plugin . \DIRECTORY_SEPARATOR;
            }
        ...
        if ($this->theme) {
            ...
            if ($plugin) {
                $themePaths[] = $themePath . static::PLUGIN_TEMPLATE_FOLDER . \DIRECTORY_SEPARATOR . $plugin . \DIRECTORY_SEPARATOR;
            }
            $themePaths[] = $themePath;
        }
        $paths = array_merge($themePaths, $pluginPaths, $templatePaths, App::core('templates'));
        if ($plugin !== null) {
    ...
    protected function _elementCache(string $name, array $data, array $options): array
    {
        ...
        $cache = $options['cache'];
        unset($options['cache']);
        $keys = array_merge([$pluginKey, $elementKey], array_keys($options), array_keys($data));
        $config = ['config' => $this->elementCache, 'key' => implode('_', $keys)];
        if (is_array($cache)) {
            $config = $cache + $config;
        }
