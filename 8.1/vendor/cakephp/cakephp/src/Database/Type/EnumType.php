...
class EnumType extends \Cake\Database\Type\BaseType
{
    ...
    public function toDatabase(mixed $value, Driver $driver): string|int|null
    {
        ...
        if ($value instanceof BackedEnum) {
            if (!$value instanceof $this->enumClassName) {
                throw new InvalidArgumentException(sprintf('Given value type `%s` does not match associated `%s` backed enum in `%s`', get_debug_type($value), $this->backingType, $this->enumClassName));
            }
            return $value->value;
        }
        if (!is_string($value) && !is_int($value)) {
            throw new InvalidArgumentException(sprintf('Cannot convert value `%s` of type `%s` to string or int', print_r($value, \true), get_debug_type($value)));
        }
    ...
    public function marshal(mixed $value): ?BackedEnum
    {
        ...
        if (!is_string($value) && !is_int($value)) {
            throw new InvalidArgumentException(sprintf('Unable to marshal value `%s` of type `%s` to `%s`', print_r($value, \true), get_debug_type($value), $this->enumClassName));
        }
        if ($this->backingType === 'int') {
            if ($value === '') {
                return null;
