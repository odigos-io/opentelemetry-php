...
class Route
{
        ...
        $checker = function () use ($defaults): bool {
            foreach (['plugin', 'prefix', 'controller', 'action'] as $key) {
                if (isset($defaults[$key]) && !is_string($defaults[$key])) {
                    throw new CakeException('Value for `' . $key . '` in $defaults when connecting routes' . ' must be of type `string` or `null`');
                }
            }
            return \true;
        };
    ...
    protected function normalizeAndValidateMethods(array|string $methods): array|string
    {
        ...
        if ($diff !== []) {
            throw new InvalidArgumentException(sprintf('Invalid HTTP method received. `%s` is invalid.', implode(', ', $diff)));
        }
        return $methods;
    }
    /**
    ...
    public function setPatterns(array $patterns)
    {
        ...
        if (mb_strlen($patternValues) < strlen($patternValues)) {
            $this->options['multibytePattern'] = \true;
        }
    ...
    public function parse(string $url, string $method): ?array
    {
        ...
        if (isset($this->defaults['_method']) && !in_array($method, (array) $this->defaults['_method'], \true)) {
            return null;
        }
        array_shift($route);
    ...
    public function match(array $url, array $context = []): ?string
    {
        ...
        if (isset($keyNames['controller']) && !isset($url['controller']) || isset($keyNames['action']) && !isset($url['action'])) {
            return null;
        }
    ...
    protected function _matchMethod(array $url): bool
    {
        ...
        foreach ($methods as $value) {
            if (in_array($value, $defaults, \true)) {
                return \true;
            }
        }
        return \false;
    ...
    {
        $matched = preg_match(static::PLACEHOLDER_REGEX, $this->template, $namedElements, \PREG_OFFSET_CAPTURE);
        if ($matched) {
