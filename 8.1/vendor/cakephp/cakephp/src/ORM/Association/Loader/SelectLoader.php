...
class SelectLoader
{
    ...
    {
        return ['foreignKey' => $this->foreignKey, 'conditions' => [], 'strategy' => $this->strategy, 'nestKey' => $this->alias, 'sort' => $this->sort];
    }
    /**
    ...
    protected function _buildQuery(array $options): SelectQuery
    {
        ...
        $fetchQuery = $query->select($options['fields'])->where($options['conditions'])->eagerLoaded(\true)->enableHydration($selectQuery->isHydrationEnabled())->setConnectionRole($selectQuery->getConnectionRole());
    ...
    protected function _assertFieldsPresent(SelectQuery $fetchQuery, array $key): void
    {
        ...
        $missingKey = function ($fieldList, $key): bool {
            foreach ($key as $keyField) {
                if (!in_array($keyField, $fieldList, \true)) {
                    return \true;
                }
            }
            return \false;
        ...
        if ($missingFields) {
            throw new InvalidArgumentException(sprintf('You are required to select the "%s" field(s)', implode(', ', $key)));
        }
    }
    /**
     ...
     */
    protected function _createTupleCondition(SelectQuery $query, array $keys, mixed $filter, string $operator): TupleComparison
    {
        $types = [];
    ...
    protected function _linkField(array $options): array|string
    {
        ...
        $keys = in_array($this->associationType, [Association::ONE_TO_ONE, Association::ONE_TO_MANY], \true) ? $this->foreignKey : $this->bindingKey;
    ...
    protected function _buildSubquery(SelectQuery $query): SelectQuery
    {
        ...
        $filterQuery->mapReduce(null, null, \true);
        $filterQuery->formatResults(null, \true);
        $filterQuery->contain([], \true);
        ...
        if (!$hasLimit) {
            ...
            if ($hasOrder) {
                $filterQuery->orderBy([], \true);
            }
        }
        $fields = $this->_subqueryFields($query);
        $filterQuery->select($fields['select'], \true)->groupBy($fields['group']);
        return $filterQuery;
    }
    /**
    ...
    {
        $resultMap = [];
        $singleResult = in_array($this->associationType, [Association::MANY_TO_ONE, Association::ONE_TO_ONE], \true);
        $keys = in_array($this->associationType, [Association::ONE_TO_ONE, Association::ONE_TO_MANY], \true) ? $this->foreignKey : $this->bindingKey;
        $key = (array) $keys;
    ...
    {
        $keys = $this->associationType === Association::MANY_TO_ONE ? $this->foreignKey : $this->bindingKey;
        $sourceKeys = [];
        foreach ((array) $keys as $key) {
