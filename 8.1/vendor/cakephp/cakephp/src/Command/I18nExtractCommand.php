...
class I18nExtractCommand extends \Cake\Command\Command
{
    ...
    {
        $defaultPaths = array_merge([APP], array_values(App::path('templates')), ['D']);
        $defaultPathIndex = 0;
        while (\true) {
    ...
    public function execute(Arguments $args, ConsoleIo $io): ?int
    {
        ...
        if ($args->hasOption('extract-core')) {
            ...
            $response = $io->askChoice('Would you like to extract the messages from the CakePHP core?', ['y', 'n'], 'n');
        ...
        if ($args->hasOption('output')) {
            ...
            while (\true) {
                $response = $io->ask($message, $localePaths[0]);
                if (strtoupper($response) === 'Q') {
                    $io->err('Extract Aborted');
                ...
                $io->err('');
                $io->err('<error>The directory path you supplied was ' . 'not found. Please try again.</error>');
                $io->err('');
            }
        }
        if ($args->hasOption('merge')) {
            ...
            $io->out();
            $response = $io->askChoice('Would you like to merge all domain strings into the default.pot file?', ['y', 'n'], 'n');
    ...
    {
        $parser->setDescription([static::getDescription(), 'Source files are parsed and string literal format strings ' . 'provided to the <info>__</info> family of functions are extracted.'])->addOption('app', ['help' => 'Directory where your application is located.'])->addOption('paths', ['help' => 'Comma separated list of paths that are searched for source files.'])->addOption('merge', ['help' => 'Merge all domain strings into a single default.po file.', 'default' => 'no', 'choices' => ['yes', 'no']])->addOption('output', ['help' => 'Full path to output directory.'])->addOption('files', ['help' => 'Comma separated list of files to parse.'])->addOption('exclude-plugins', ['boolean' => \true, 'default' => \true, 'help' => 'Ignores all files in plugins if this command is run inside from the same app directory.'])->addOption('plugin', ['help' => 'Extracts tokens only from the plugin specified and ' . "puts the result in the plugin's `locales` directory.", 'short' => 'p'])->addOption('exclude', ['help' => 'Comma separated list of directories to exclude.' . ' Any path containing a path segment with the provided values will be skipped. E.g. test,vendors'])->addOption('overwrite', ['boolean' => \true, 'default' => \false, 'help' => 'Always overwrite existing .pot files.'])->addOption('extract-core', ['help' => 'Extract messages from the CakePHP core libraries.', 'choices' => ['yes', 'no']])->addOption('no-location', ['boolean' => \true, 'default' => \false, 'help' => 'Do not write file locations for each extracted message.'])->addOption('marker-error', ['boolean' => \true, 'default' => \false, 'help' => 'Do not display marker error.']);
        return $parser;
    }
    /**
    ...
    protected function _writeFiles(Arguments $args, ConsoleIo $io): void
    {
        ...
        foreach ($this->_storage as $domain => $sentences) {
            ...
            while ($overwriteAll === \false && file_exists($outputPath) && strtoupper($response) !== 'Y') {
                $io->out();
                $response = $io->askChoice(sprintf('Error: %s already exists in this location. Overwrite? [Y]es, [N]o, [A]ll', $filename), ['y', 'n', 'a'], 'y');
