...
class ServerCommand extends \Cake\Command\Command
{
    ...
    public function execute(Arguments $args, ConsoleIo $io): ?int
    {
        ...
        $command = sprintf('%s -S %s:%d -t %s', $phpBinary, $this->_host, $this->_port, escapeshellarg($this->_documentRoot));
        if ($this->_iniPath) {
    ...
    {
        $parser->setDescription([static::getDescription(), "<warning>[WARN] Don't use this in a production environment</warning>"])->addOption('host', ['short' => 'H', 'help' => 'ServerHost'])->addOption('port', ['short' => 'p', 'help' => 'ListenPort'])->addOption('ini_path', ['short' => 'I', 'help' => 'php.ini path'])->addOption('document_root', ['short' => 'd', 'help' => 'DocumentRoot']);
        return $parser;
    }
}
