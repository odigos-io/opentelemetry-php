...
final class TraceContextPropagator implements TextMapPropagatorInterface
{
    ...
    private static function extractImpl($carrier, PropagationGetterInterface $getter): SpanContextInterface
    {
         ...
         */
        if (!\OpenTelemetry\API\Trace\Propagation\TraceContextValidator::isValidTraceVersion($version) || !SpanContextValidator::isValidTraceId($traceId) || !SpanContextValidator::isValidSpanId($spanId) || !\OpenTelemetry\API\Trace\Propagation\TraceContextValidator::isValidTraceFlag($traceFlags)) {
            return SpanContext::getInvalid();
        }
