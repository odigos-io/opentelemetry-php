...
class Bucket
{
    ...
    {
        return ['bucketName' => $this->bucketName, 'codec' => $this->codec, 'databaseName' => $this->databaseName, 'manager' => $this->manager, 'chunkSizeBytes' => $this->chunkSizeBytes, 'readConcern' => $this->readConcern, 'readPreference' => $this->readPreference, 'typeMap' => $this->typeMap, 'writeConcern' => $this->writeConcern];
    }
    /**
    ...
    public function openUploadStream(string $filename, array $options = [])
    {
        ...
        $context = stream_context_create([self::STREAM_WRAPPER_PROTOCOL => ['collectionWrapper' => $this->collectionWrapper, 'filename' => $filename, 'options' => $options]]);
    ...
    private function createPathForFile(object $file): string
    {
        ...
        return sprintf('%s://%s/%s.files/%s', self::STREAM_WRAPPER_PROTOCOL, urlencode($this->databaseName), urlencode($this->bucketName), urlencode($id));
    }
    /**
    ...
    {
        return sprintf('%s://%s/%s.files', self::STREAM_WRAPPER_PROTOCOL, urlencode($this->databaseName), urlencode($this->bucketName));
    }
    /**
    ...
    private function openDownloadStreamByFile(object $file)
    {
        ...
        $context = stream_context_create([self::STREAM_WRAPPER_PROTOCOL => ['collectionWrapper' => $this->collectionWrapper, 'file' => $file]]);
    ...
    private function resolveStreamContext(string $path, string $mode, array $context): array
    {
        ...
        if ($mode === 'r' || $mode === 'rb') {
            ...
            return ['collectionWrapper' => $this->collectionWrapper, 'file' => $file];
        }
        if ($mode === 'w' || $mode === 'wb') {
            return ['collectionWrapper' => $this->collectionWrapper, 'filename' => $filename, 'options' => $context + ['chunkSizeBytes' => $this->chunkSizeBytes]];
        }
