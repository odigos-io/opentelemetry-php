...
use Odigos\FastRoute\RouteCollector;
...
abstract class DispatcherTest extends TestCase
{
    ...
    {
        return ['dataGenerator' => $this->getDataGeneratorClass(), 'dispatcher' => $this->getDispatcherClass()];
    }
    /**
    ...
    {
        $dispatcher = \Odigos\FastRoute\simpleDispatcher($callback, $this->generateDispatcherOptions());
    ...
    public function testNotFoundDispatches($method, $uri, $callback)
    {
        ...
        $this->assertArrayNotHasKey(1, $routeInfo, 'NOT_FOUND result must only contain a single element in the returned info array');
    ...
    public function testMethodNotAllowedDispatches($method, $uri, $callback, $availableMethods)
    {
        $dispatcher = \Odigos\FastRoute\simpleDispatcher($callback, $this->generateDispatcherOptions());
        $routeInfo = $dispatcher->dispatch($method, $uri);
        $this->assertArrayHasKey(1, $routeInfo, 'METHOD_NOT_ALLOWED result must return an array of allowed methods at index 1');
    ...
    {
        \Odigos\FastRoute\simpleDispatcher(function (RouteCollector $r) {
