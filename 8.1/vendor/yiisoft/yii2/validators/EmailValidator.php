...
class EmailValidator extends \yii\validators\Validator
{
    ...
    {
        return $this->hasDNSRecord($domain, \true) || $this->hasDNSRecord($domain, \false);
    }
    ...
    {
        $options = ['pattern' => new JsExpression($this->pattern), 'fullPattern' => new JsExpression($this->fullPattern), 'allowName' => $this->allowName, 'message' => $this->formatMessage($this->message, ['attribute' => $model->getAttributeLabel($attribute)]), 'enableIDN' => (bool) $this->enableIDN];
        if ($this->skipOnEmpty) {
