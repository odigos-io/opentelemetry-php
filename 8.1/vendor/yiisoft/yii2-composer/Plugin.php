...
class Plugin implements PluginInterface, EventSubscriberInterface
{
    ...
    {
        $this->_installer = new \yii\composer\Installer($io, $composer);
    ...
    public function checkPackageUpdates(PackageEvent $event)
    {
        ...
        if ($operation instanceof UpdateOperation) {
            $this->_packageUpdates[$operation->getInitialPackage()->getName()] = ['from' => $operation->getInitialPackage()->getVersion(), 'fromPretty' => $operation->getInitialPackage()->getPrettyVersion(), 'to' => $operation->getTargetPackage()->getVersion(), 'toPretty' => $operation->getTargetPackage()->getPrettyVersion(), 'direction' => $this->_isUpgrade($event, $operation) ? 'up' : 'down'];
        }
    }
    /**
     * @param PackageEvent $event
    ...
    private function _isUpgrade(PackageEvent $event, UpdateOperation $operation)
    {
        ...
        if (method_exists('Composer\Package\Version\VersionParser', 'isUpgrade')) {
            return VersionParser::isUpgrade($operation->getInitialPackage()->getVersion(), $operation->getTargetPackage()->getVersion());
        }
        return $event->getPolicy()->versionCompare($operation->getInitialPackage(), $operation->getTargetPackage(), '<');
    }
    /**
    ...
    {
        $io->write("\n  <fg=yellow;options=bold>Seems you have " . ($package['direction'] === 'up' ? 'upgraded' : 'downgraded') . ' Yii Framework from version ' . $package['fromPretty'] . ' to ' . $package['toPretty'] . '.</>');
