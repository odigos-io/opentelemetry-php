...
final class MetaInformation implements MetaInformationContract
{
     ...
     */
    use ArrayAccessible;
    private function __construct(public ?string $requestId, public readonly \OpenAI\Responses\Meta\MetaInformationOpenAI $openai, public readonly ?\OpenAI\Responses\Meta\MetaInformationRateLimit $requestLimit, public readonly ?\OpenAI\Responses\Meta\MetaInformationRateLimit $tokenLimit)
    {
    }
    /**
    ...
    public static function from(array $headers): self
    {
        ...
        $openai = \OpenAI\Responses\Meta\MetaInformationOpenAI::from(['model' => $headers['openai-model'][0] ?? null, 'organization' => $headers['openai-organization'][0] ?? null, 'version' => $headers['openai-version'][0] ?? null, 'processingMs' => isset($headers['openai-processing-ms'][0]) ? (int) $headers['openai-processing-ms'][0] : null]);
