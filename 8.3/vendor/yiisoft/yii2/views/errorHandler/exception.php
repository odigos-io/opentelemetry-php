...
echo $handler->htmlEncode($exception);
?></textarea>
...
echo \http_build_query(['q' => $exception->getMessage()]);
...
?>" title="Search error on Google" target="_blank">
        ...
        <?php 
if ($exception instanceof \Odigos\yii\base\ErrorException) {
    ?>
            ...
            <h1>
                <span><?php 
    echo $handler->htmlEncode($exception->getName());
    ?></span>
                &ndash; <?php 
...
?>
    ...
    <div class="call-stack">
        <?php 
echo $handler->renderCallStack($exception);
?>
    </div>
...
?></p>
        <p><?php 
echo $handler->createServerInformationLink();
?></p>
        <p><a href="https://yiiframework.com/">Yii Framework</a>/<?php 
echo $handler->createFrameworkVersionLink();
?></p>
    </div>
