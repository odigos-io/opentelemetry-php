...
class Schema extends BaseSchema implements ConstraintFinderInterface
{
    ...
    protected function loadTableForeignKeys($tableName)
    {
        ...
        $foreignKeys = $this->normalizePdoRowKeyCase($foreignKeys, \true);
        $foreignKeys = ArrayHelper::index($foreignKeys, null, 'table');
        ArrayHelper::multisort($foreignKeys, 'seq', \SORT_ASC, \SORT_NUMERIC);
        $result = [];
        foreach ($foreignKeys as $table => $foreignKey) {
            $result[] = new ForeignKeyConstraint(['columnNames' => ArrayHelper::getColumn($foreignKey, 'from'), 'foreignTableName' => $table, 'foreignColumnNames' => ArrayHelper::getColumn($foreignKey, 'to'), 'onDelete' => isset($foreignKey[0]['on_delete']) ? $foreignKey[0]['on_delete'] : null, 'onUpdate' => isset($foreignKey[0]['on_update']) ? $foreignKey[0]['on_update'] : null]);
        }
        return $result;
    }
    /**
    ...
    protected function findColumns($table)
    {
        ...
        if (count($table->primaryKey) === 1 && !strncasecmp($table->columns[$table->primaryKey[0]]->dbType, 'int', 3)) {
            $table->sequenceName = '';
            $table->columns[$table->primaryKey[0]]->autoIncrement = \true;
        }
        return \true;
    }
    /**
    ...
    private function loadTableColumnsInfo($tableName)
    {
        ...
        $tableColumns = $this->normalizePdoRowKeyCase($tableColumns, \true);
    ...
    private function loadTableConstraints($tableName, $returnType)
    {
        ...
        foreach ($indexes as $index) {
            ...
            $columns = $this->normalizePdoRowKeyCase($columns, \true);
            ...
            $result['indexes'][] = new IndexConstraint(['isPrimary' => $index['origin'] === 'pk', 'isUnique' => (bool) $index['unique'], 'name' => $index['name'], 'columnNames' => ArrayHelper::getColumn($columns, 'name')]);
            if ($index['origin'] === 'u') {
                $result['uniques'][] = new Constraint(['name' => $index['name'], 'columnNames' => ArrayHelper::getColumn($columns, 'name')]);
