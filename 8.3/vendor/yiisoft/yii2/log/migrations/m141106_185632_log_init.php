...
class m141106_185632_log_init extends Migration
{
        ...
        if ($this->_dbTargets === []) {
            ...
            foreach ($log->targets as $target) {
                if ($target instanceof DbTarget) {
                    $currentTarget = [$target->db, $target->logTable];
                    if (!\in_array($currentTarget, $usedTargets, \true)) {
    ...
    public function up()
    {
        foreach ($this->getDbTargets() as $target) {
            ...
            $this->createTable($target->logTable, ['id' => $this->bigPrimaryKey(), 'level' => $this->integer(), 'category' => $this->string(), 'log_time' => $this->double(), 'prefix' => $this->text(), 'message' => $this->text()], $tableOptions);
 ...
 * @since 2.0.1
 */
\class_alias('Odigos\m141106_185632_log_init', 'm141106_185632_log_init', \false);
