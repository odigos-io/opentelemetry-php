...
final class QueryObject implements \MongoDB\Builder\Type\QueryInterface
{
    ...
    private function __construct(array $queriesOrArrayOfQueries)
    {
        ...
        if (count($queriesOrArrayOfQueries) === 1 && isset($queriesOrArrayOfQueries[0]) && is_array($queriesOrArrayOfQueries[0]) && count($queriesOrArrayOfQueries[0]) > 0 && !str_starts_with((string) array_key_first($queriesOrArrayOfQueries[0]), '$')) {
        ...
        foreach ($queriesOrArrayOfQueries as $fieldPath => $query) {
            if ($query instanceof \MongoDB\Builder\Type\QueryInterface) {
                if ($query instanceof \MongoDB\Builder\Type\OperatorInterface) {
                    ...
                    $seenQueryOperators[$query::NAME] = \true;
                }
                $queries[] = $query;
                continue;
            }
