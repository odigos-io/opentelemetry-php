...
final class VectorSearchStage implements StageInterface, OperatorInterface
{
    ...
    public const PROPERTIES = ['index' => 'index', 'limit' => 'limit', 'path' => 'path', 'queryVector' => 'queryVector', 'exact' => 'exact', 'filter' => 'filter', 'numCandidates' => 'numCandidates'];
     ...
     * Number of nearest neighbors to use during the search. Value must be less than or equal to (<=) 10000. You can't specify a number less than the number of documents to return (limit).
     */
    public function __construct(string $index, int $limit, string $path, PackedArray|BSONArray|array $queryVector, Optional|bool $exact = Optional::Undefined, Optional|QueryInterface|array $filter = Optional::Undefined, Optional|int $numCandidates = Optional::Undefined)
    {
        $this->index = $index;
        $this->limit = $limit;
        $this->path = $path;
