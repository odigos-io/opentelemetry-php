...
final class DriverOptions
{
     ...
     */
    private function __construct(public readonly array $typeMap, public readonly Encoder $builderEncoder, public readonly ?array $autoEncryption, private readonly array $miscOptions, array $driver)
    {
    ...
    public static function fromArray(array $options): self
    {
        ...
        if (!is_array($options[self::KEY_TYPE_MAP])) {
            throw InvalidArgumentException::invalidType(sprintf('"%s" driver option', self::KEY_TYPE_MAP), $options[self::KEY_TYPE_MAP], 'array');
        }
        if (isset($options[self::KEY_BUILDER_ENCODER]) && !$options[self::KEY_BUILDER_ENCODER] instanceof Encoder) {
            throw InvalidArgumentException::invalidType(sprintf('"%s" option', self::KEY_BUILDER_ENCODER), $options[self::KEY_BUILDER_ENCODER], Encoder::class);
        }
        ...
        $autoEncryption = !empty($autoEncryptionOptions) ? AutoEncryptionOptions::fromArray($autoEncryptionOptions)->toArray() : null;
        /** @var array{name?: string, version?: string, platform?: string} $driver $driver */
        $driver = $options[self::KEY_DRIVER] ?? [];
        return new self(typeMap: $options[self::KEY_TYPE_MAP], builderEncoder: $options[self::KEY_BUILDER_ENCODER] ?? new BuilderEncoder(), autoEncryption: $autoEncryption, miscOptions: array_diff_key($options, [self::KEY_TYPE_MAP => 1, self::KEY_BUILDER_ENCODER => 1, self::KEY_AUTO_ENCRYPTION => 1, self::KEY_DRIVER => 1]), driver: $driver);
    }
    ...
    {
        return array_filter(['typeMap' => $this->typeMap, 'builderEncoder' => $this->builderEncoder, 'autoEncryption' => $this->autoEncryption, 'driver' => $this->driver] + $this->miscOptions, static fn($option) => $option !== null);
    }
