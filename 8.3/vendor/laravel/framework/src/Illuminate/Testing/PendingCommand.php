...
class PendingCommand
{
    ...
    {
        $this->test->expectedChoices[$question] = ['expected' => $answers, 'strict' => $strict];
    ...
    {
        return $this->expectsQuestion($question, $search)->expectsChoice($question, $answer, $answers);
    }
    /**
    ...
    {
        $table = (new Table($output = new BufferedOutput()))->setHeaders((array) $headers)->setRows($rows instanceof Arrayable ? $rows->toArray() : $rows)->setStyle($tableStyle);
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsNote($message, 'info'));
        return $this;
    }
    /**
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsNote($message, 'warning'));
        return $this;
    }
    /**
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsNote($message, 'error'));
        return $this;
    }
    /**
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsNote($message, 'alert'));
        return $this;
    }
    /**
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsNote($message, 'intro'));
        return $this;
    }
    /**
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsNote($message, 'outro'));
        return $this;
    }
    /**
    ...
    {
        $this->expectOutputToContainPrompt(new PromptsTable($headers, $rows));
        return $this;
    }
    /**
    ...
    public function run()
    {
        ...
        if ($this->expectedExitCode !== null) {
            $this->test->assertEquals($this->expectedExitCode, $exitCode, "Expected status code {$this->expectedExitCode} but received {$exitCode}.");
        } elseif (!is_null($this->unexpectedExitCode)) {
            $this->test->assertNotEquals($this->unexpectedExitCode, $exitCode, "Unexpected status code {$this->unexpectedExitCode} was received.");
        }
    ...
    public function dd()
    {
        ...
        fclose($streamOutput);
        dd(['exitCode' => $exitCode, 'output' => $output]);
    }
    /**
    ...
    protected function verifyExpectations()
    {
        ...
        if (count($this->test->expectedChoices) > 0) {
            foreach ($this->test->expectedChoices as $question => $answers) {
                ...
                $this->test->{$assertion}($answers['expected'], $answers['actual'], 'Question "' . $question . '" has different options.');
            }
        }
        ...
        if ($output = array_search(\true, $this->test->unexpectedOutput)) {
            $this->test->fail('Output "' . $output . '" was printed.');
        }
        if ($output = array_search(\true, $this->test->unexpectedOutputSubstrings)) {
            $this->test->fail('Output "' . $output . '" was printed.');
        }
    }
    /**
    ...
    protected function mockConsoleOutput()
    {
        ...
        foreach ($this->test->expectedQuestions as $i => $question) {
            $mock->shouldReceive('askQuestion')->once()->ordered()->with(Mockery::on(function ($argument) use ($question) {
                if (isset($this->test->expectedChoices[$question[0]])) {
                    $this->test->expectedChoices[$question[0]]['actual'] = $argument instanceof ChoiceQuestion && !array_is_list($this->test->expectedChoices[$question[0]]['expected']) ? $argument->getChoices() : $argument->getAutocompleterValues();
                }
    ...
    private function createABufferedOutputMock()
    {
        ...
        if ($this->test->expectsOutput === \true && count($this->test->expectedOutput) === 0 && count($this->test->expectedOutputSubstrings) === 0) {
            $mock->shouldReceive('doWrite')->atLeast()->once();
        }
        foreach ($this->test->expectedOutput as $i => $output) {
            $mock->shouldReceive('doWrite')->once()->ordered()->with($output, Mockery::any())->andReturnUsing(function () use ($i) {
                unset($this->test->expectedOutput[$i]);
            });
        }
        foreach ($this->test->expectedOutputSubstrings as $i => $text) {
            $mock->shouldReceive('doWrite')->atLeast()->times(0)->withArgs(fn($output) => str_contains($output, $text))->andReturnUsing(function () use ($i) {
                unset($this->test->expectedOutputSubstrings[$i]);
            });
        }
        foreach ($this->test->unexpectedOutput as $output => $displayed) {
            $mock->shouldReceive('doWrite')->atLeast()->times(0)->ordered()->with($output, Mockery::any())->andReturnUsing(function () use ($output) {
                $this->test->unexpectedOutput[$output] = \true;
            });
        }
        foreach ($this->test->unexpectedOutputSubstrings as $text => $displayed) {
            $mock->shouldReceive('doWrite')->atLeast()->times(0)->withArgs(fn($output) => str_contains($output, $text))->andReturnUsing(function () use ($text) {
                $this->test->unexpectedOutputSubstrings[$text] = \true;
            });
        }
        return $mock;
    }
