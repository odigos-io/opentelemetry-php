...
class HasInDatabase extends Constraint
{
    ...
    {
        return $this->database->table($table)->where($this->data)->exists();
    }
    /**
    ...
    {
        return sprintf("a row in the table [%s] matches the attributes %s.\n\n%s", $table, $this->toString(\JSON_PRETTY_PRINT | \JSON_UNESCAPED_UNICODE), $this->getAdditionalInfo($table));
    }
    /**
    ...
    protected function getAdditionalInfo($table)
    {
        ...
        $similarResults = $query->where(array_key_first($this->data), array_first($this->data))->select(array_keys($this->data))->limit($this->show)->get();
