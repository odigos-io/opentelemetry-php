...
class Kernel implements KernelContract
{
    ...
    public function rerouteSymfonyCommandEvents()
    {
        if (is_null($this->symfonyDispatcher)) {
            ...
            $this->symfonyDispatcher->addListener(ConsoleEvents::COMMAND, function (ConsoleCommandEvent $event) {
                $this->events->dispatch(new CommandStarting($event->getCommand()?->getName() ?? '', $event->getInput(), $event->getOutput()));
            });
            $this->symfonyDispatcher->addListener(ConsoleEvents::TERMINATE, function (ConsoleTerminateEvent $event) {
                $this->events->dispatch(new CommandFinished($event->getCommand()?->getName() ?? '', $event->getInput(), $event->getOutput(), $event->getExitCode()));
            });
        }
        return $this;
    }
    ...
    public function handle($input, $output = null)
    {
        ...
        try {
            if (in_array($input->getFirstArgument(), ['env:encrypt', 'env:decrypt'], \true)) {
    ...
    protected function load($paths)
    {
        ...
        $this->loadedPaths = array_values(array_unique(array_merge($this->loadedPaths, $paths)));
        ...
        $filterCommands = function (SplFileInfo $file) use ($namespace, &$possibleCommands) {
            ...
            return $command instanceof ReflectionClass && $command->isSubClassOf(Command::class) && !$command->isAbstract();
        };
    ...
    {
        return $namespace . str_replace(['/', '.php'], ['\\', ''], Str::after($file->getRealPath(), realpath(app_path()) . \DIRECTORY_SEPARATOR));
    }
    /**
    ...
    {
        if (in_array($command, ['env:encrypt', 'env:decrypt'], \true)) {
            $this->bootstrapWithoutBootingProviders();
        }
        $this->bootstrap();
    ...
    protected function getArtisan()
    {
        if (is_null($this->artisan)) {
            $this->artisan = (new Artisan($this->app, $this->events, $this->app->version()))->resolveCommands($this->commands)->setContainerCommandLoader();
