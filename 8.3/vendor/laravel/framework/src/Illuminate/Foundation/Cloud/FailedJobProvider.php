...
class FailedJobProvider implements FailedJobProviderInterface, CountableFailedJobProvider, PrunableFailedJobProvider
{
    ...
    public function log($connection, $queue, $payload, $exception)
    {
        ...
        $this->events->emit(['_cloud_event' => 'failed_job', 'id' => $id = Str::uuid7($timestamp)->toString(), 'queue' => $processingJobDetails['queue'], 'started_at' => $processingJobDetails['started_at']->toDateTimeString('microsecond'), 'attempts' => $processingJobDetails['attempts'], 'payload' => $payload, 'exception' => (string) mb_convert_encoding($exception, 'UTF-8')]);
    ...
    public function find($id)
    {
        ...
        $response = Http::connectTimeout(10)->timeout(10)->retry(3, 1000, fn($exception) => $exception instanceof ConnectionException)->throw()->get($id);
    ...
    public function forget($id)
    {
        ...
        $this->events->emit(['_cloud_event' => 'failed_job', 'id' => $job->id, 'queue' => $job->queue, 'retried_at' => CarbonImmutable::now('UTC')->toDateTimeString('microsecond')]);
        return \true;
    }
    /**
