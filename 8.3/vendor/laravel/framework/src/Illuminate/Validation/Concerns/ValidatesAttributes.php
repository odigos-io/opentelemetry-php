...
use Odigos\Brick\Math\Exception\MathException as BrickMathException;
use DateTime;
use DateTimeInterface;
use DateTimeZone;
use Odigos\Egulias\EmailValidator\EmailValidator;
use Odigos\Egulias\EmailValidator\Validation\DNSCheckValidation;
use Odigos\Egulias\EmailValidator\Validation\Extra\SpoofCheckValidation;
use Odigos\Egulias\EmailValidator\Validation\MultipleValidationWithAnd;
...
trait ValidatesAttributes
{
    ...
    public function validateDigits($attribute, $value, $parameters)
    {
        ...
        return (is_numeric($value) || is_string($value)) && !preg_match('/[^0-9]/', $value) && strlen((string) $value) == $parameters[0];
    }
    /**
    ...
    public function validateDimensions($attribute, $value, $parameters)
    {
        ...
        $dimensions = method_exists($value, 'dimensions') ? $value->dimensions() : @getimagesize($value->getRealPath());
        if (!$dimensions) {
            return \false;
        }
        ...
        return !($this->failsBasicDimensionChecks($parameters, $width, $height) || $this->failsRatioCheck($parameters, $width, $height) || $this->failsMinRatioCheck($parameters, $width, $height) || $this->failsMaxRatioCheck($parameters, $width, $height));
    }
    /**
    ...
    private function failsMinRatioCheck($parameters, $width, $height)
    {
        ...
        [$minNumerator, $minDenominator] = array_replace([1, 1], array_filter(sscanf($parameters['min_ratio'], '%f/%d')));
    ...
    private function failsMaxRatioCheck($parameters, $width, $height)
    {
        ...
        [$maxNumerator, $maxDenominator] = array_replace([1, 1], array_filter(sscanf($parameters['max_ratio'], '%f/%d')));
    ...
    public function validateEmail($attribute, $value, $parameters)
    {
        ...
        $validations = (new Collection($parameters))->unique()->map(fn($validation) => match (\true) {
    ...
    public function parseTable($table)
    {
        ...
        if (str_contains($table, '\\') && class_exists($table) && is_a($table, Model::class, \true)) {
            $model = new $table();
    ...
    protected function shouldBlockPhpUpload($value, $parameters)
    {
        ...
        return $value instanceof UploadedFile ? in_array(trim(strtolower($value->getClientOriginalExtension())), $phpExtensions) : in_array(trim(strtolower($value->getExtension())), $phpExtensions);
    }
    /**
    ...
    {
        return in_array($value, timezone_identifiers_list(constant(DateTimeZone::class . '::' . Str::upper($parameters[0] ?? 'ALL')), isset($parameters[1]) ? Str::upper($parameters[1]) : null), \true);
    }
    /**
    ...
    protected function ensureExponentWithinAllowedRange($attribute, $value)
    {
        ...
        if (!is_numeric($value) || !Str::contains($stringValue, 'e', ignoreCase: \true)) {
            return $value;
        }
        $scale = (int) (Str::contains($stringValue, 'e') ? Str::after($stringValue, 'e') : Str::after($stringValue, 'E'));
        $withinRange = ($this->ensureExponentWithinAllowedRangeUsing ?? fn($scale) => $scale <= 1000 && $scale >= -1000)($scale, $attribute, $value);
        if (!$withinRange) {
