...
class RedisBroadcaster extends \Illuminate\Broadcasting\Broadcasters\Broadcaster
{
    ...
    public function validAuthenticationResponse($request, $result)
    {
        ...
        return json_encode(['channel_data' => ['user_id' => $broadcastIdentifier, 'user_info' => $result]]);
    }
    /**
    ...
    public function broadcast(array $channels, $event, array $payload = [])
    {
        ...
        try {
            ...
            } elseif ($connection instanceof PredisClusterConnection && $connection->client()->getConnection() instanceof RedisCluster) {
                $randomClusterNodeConnection = new PredisConnection($connection->client()->getClientBy('slot', mt_rand(0, 16383)));
                if ($events = $connection->getEventDispatcher()) {
                    $randomClusterNodeConnection->setEventDispatcher($events);
                }
                $randomClusterNodeConnection->eval($this->broadcastMultipleChannelsScript(), 0, $payload, ...$this->formatChannels($channels));
            } else {
            ...
            throw new BroadcastException(sprintf('Redis error: %s.', $e->getMessage()));
        }
    }
    /**
