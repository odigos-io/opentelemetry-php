...
class CallQueuedHandler
{
    ...
    protected function dispatchThroughMiddleware(Job $job, $command)
    {
        ...
        $lockReleased = \false;
        return (new Pipeline($this->container))->send($command)->through(array_merge(method_exists($command, 'middleware') ? $command->middleware() : [], $command->middleware ?? []))->finally(function ($command) use (&$lockReleased) {
    ...
    {
        return $command instanceof ShouldBeUnique || $command instanceof CallQueuedListener && $command->shouldBeUnique();
    }
    /**
    ...
    {
        return $command instanceof ShouldBeUniqueUntilProcessing || $command instanceof CallQueuedListener && $command->shouldBeUniqueUntilProcessing();
    }
    /**
    ...
    {
        if (!in_array(Batchable::class, class_uses_recursive($class), \true)) {
            return;
        }
        ...
        if (!is_string($batchId) || $batchId === '' || !is_string($job->uuid()) || $job->uuid() === '') {
            return;
        }
