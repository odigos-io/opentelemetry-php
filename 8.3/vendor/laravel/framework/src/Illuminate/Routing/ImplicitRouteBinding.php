...
class ImplicitRouteBinding
{
    ...
    public static function resolveForRoute($container, $route)
    {
        ...
        foreach ($route->signatureParameters(['subClass' => UrlRoutable::class]) as $parameter) {
            ...
            if ($parent instanceof UrlRoutable && !$route->preventsScopedBindings() && ($route->enforcesScopedBindings() || array_key_exists($parameterName, $route->bindingFields()))) {
                $childRouteBindingMethod = $route->allowsTrashedBindings() && $instance::isSoftDeletable() ? 'resolveSoftDeletableChildRouteBinding' : 'resolveChildRouteBinding';
    ...
    {
        foreach ($route->signatureParameters(['backedEnum' => \true]) as $parameter) {
            if (!$parameterName = static::getParameterName($parameter->getName(), $parameters)) {
                continue;
            }
            $parameterValue = $parameters[$parameterName];
            ...
            $backedEnum = $parameterValue instanceof $backedEnumClass ? $parameterValue : $backedEnumClass::tryFrom((string) $parameterValue);
