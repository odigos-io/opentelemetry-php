...
class RouteUrlGenerator
{
    ...
    protected function formatParameters(\Illuminate\Routing\Route $route, $parameters)
    {
        ...
        if (count($requiredRouteParametersWithoutDefaultsOrNamedParameters) !== 0 && count($parameters) !== count($emptyParameters)) {
    ...
    protected function replaceRouteParameters($path, array &$parameters)
    {
        ...
        $path = preg_replace_callback('/\{.*?\}/', function ($match) use (&$parameters) {
            ...
            return !isset($parameters[0]) && !str_ends_with($match[0], '?}') ? $match[0] : Arr::pull($parameters, 0);
        }, $path);
