...
class CronExpressionTimezoneConverter
{
    ...
    {
        $now = Carbon::now();
        $totalOffsetMinutes = intdiv($displayTimezone->getOffset($now) - $eventTimezone->getOffset($now), 60);
    ...
    protected static function shiftField($field, $offset, $mod, $min = 0)
    {
        ...
        $shifted = collect(explode(',', $field))->map(fn($v) => (((int) $v + $offset - $min) % $mod + $mod) % $mod + $min)->sort();
