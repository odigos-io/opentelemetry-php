...
class SesV2Transport extends AbstractTransport implements Stringable
{
    ...
    protected function doSend(SentMessage $message): void
    {
        ...
        try {
            $result = $this->ses->sendEmail(array_merge($options, ['Source' => $message->getEnvelope()->getSender()->toString(), 'Destination' => ['ToAddresses' => (new Collection($message->getEnvelope()->getRecipients()))->map->toString()->values()->all()], 'Content' => ['Raw' => ['Data' => $message->toString()]]]));
        } catch (AwsException $e) {
            $reason = $e->getAwsErrorMessage() ?? $e->getMessage();
            throw new TransportException(sprintf('Request to AWS SES V2 API failed. Reason: %s.', $reason), is_int($e->getCode()) ? $e->getCode() : 0, $e);
        }
