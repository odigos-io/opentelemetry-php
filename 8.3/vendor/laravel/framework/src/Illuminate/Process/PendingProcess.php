...
class PendingProcess
{
     ...
     * @return $this
     */
    public function tty(bool $tty = \true)
    {
        $this->tty = $tty;
        return $this;
    }
    ...
    protected function toSymfonyProcess(array|string|null $command)
    {
        ...
        $process = is_iterable($command) ? new Process($command, null, $this->environment) : Process::fromShellCommandline((string) $command, null, $this->environment);
    ...
    {
        return (new Collection($this->fakeHandlers))->first(fn($handler, $pattern) => $pattern === '*' || Str::is($pattern, $command));
    }
    /**
