...
class PendingRequest
{
     ...
     * @var array
     */
    protected $mergeableOptions = ['cookies', 'form_params', 'headers', 'json', 'multipart', 'query'];
    /**
    ...
    public function attach($name, $contents = '', $filename = null, array $headers = [])
    {
        ...
        $this->asMultipart();
        $file = ['name' => $name, 'contents' => $contents, 'headers' => $headers];
        if ($filename !== null) {
    ...
    {
        $this->options = array_replace_recursive(array_merge_recursive($this->options, Arr::only($options, $this->mergeableOptions)), $options);
        return $this;
    }
    /**
    ...
    public function pool(callable $callback, ?int $concurrency = null)
    {
        ...
        (new EachPromise($promiseGenerator(), ['fulfilled' => function ($result, $key) use (&$results) {
            $results[$key] = $result;
    ...
    public function send(string $method, string $url, array $options = [])
    {
        ...
        if ($this->async) {
            return $this->promise = new LazyPromise(fn() => $this->makePromise($method, $url, $options));
        }
        $shouldRetry = null;
        return retry($this->tries ?? 1, function ($attempt) use ($method, $url, $options, &$shouldRetry) {
            try {
                return tap($this->newResponse($this->sendRequest($method, $url, $options)), function (&$response) use ($attempt, &$shouldRetry) {
                    ...
                    try {
                        $shouldRetry = $this->retryWhenCallback ? call_user_func($this->retryWhenCallback, $response->toException(), $this, $this->request->toPsrRequest()->getMethod()) : \true;
            ...
            $result = $shouldRetry !== null ? $shouldRetry : ($this->retryWhenCallback ? call_user_func($this->retryWhenCallback, $exception, $this, $this->request?->toPsrRequest()->getMethod()) : \true);
            $shouldRetry = null;
            return $result;
        });
    }
    ...
    {
        return (new Collection($data))->flatMap(function ($value, $key) {
            if (is_array($value)) {
    ...
    {
        return array_replace_recursive(array_merge_recursive($this->options, Arr::only($options, $this->mergeableOptions)), ...$options);
    }
    /**
    ...
    protected function newResponse($response)
    {
        return tap(new \Illuminate\Http\Client\Response($response), function (\Illuminate\Http\Client\Response $laravelResponse) {
            ...
            $this->truncateExceptionsAt === \false ? $laravelResponse->dontTruncateExceptions() : $laravelResponse->truncateExceptionsAt($this->truncateExceptionsAt);
        });
    }
    /**
