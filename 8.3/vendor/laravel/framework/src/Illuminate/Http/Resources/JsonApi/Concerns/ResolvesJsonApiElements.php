...
trait ResolvesJsonApiElements
{
     ...
     */
    protected bool $usesRequestQueryString = \true;
    /**
    ...
    protected function resolveResourceObject(JsonApiRequest $request): array
    {
        ...
        return ['id' => $this->resolveResourceIdentifier($request), 'type' => $resourceType, ...(new Collection(['attributes' => $this->resolveResourceAttributes($request, $resourceType), 'relationships' => $this->resolveResourceRelationshipIdentifiers($request), 'links' => $this->resolveResourceLinks($request), 'meta' => $this->resolveResourceMetaInformation($request)]))->filter()->map(fn($value) => (object) $value)];
    }
    /**
    ...
    public function resolveResourceType(JsonApiRequest $request): string
    {
        ...
        return Str::of($morphMap !== $modelClassName ? $morphMap : class_basename($modelClassName))->snake()->pluralStudly();
    }
    /**
    ...
    protected function resolveResourceAttributes(JsonApiRequest $request, string $resourceType): array
    {
        ...
        $sparseFieldset = match ($this->usesRequestQueryString) {
            \true => $request->sparseFields($resourceType),
            default => [],
        };
        $data = (new Collection($data))->mapWithKeys(fn($value, $key) => is_int($key) ? [$value => $this->resource->{$value}] : [$key => $value])->when(!empty($sparseFieldset), fn($attributes) => $attributes->only($sparseFieldset))->transform(fn($value) => value($value, $request))->all();
    ...
    protected function resolveResourceRelationshipIdentifiers(JsonApiRequest $request): array
    {
        ...
        return [...(new Collection($this->filter($this->loadedRelationshipIdentifiers)))->map(function ($relation) {
    ...
    protected function compileResourceRelationships(JsonApiRequest $request): void
    {
        ...
        $sparseIncluded = match (\true) {
            $this->includesPreviouslyLoadedRelationships => array_keys($this->resource->getRelations()),
            default => $request->sparseIncluded(),
        };
        $resourceRelationships = (new Collection($this->toRelationships($request)))->transform(fn($value, $key) => is_int($key) ? new RelationResolver($value) : new RelationResolver($key, $value))->mapWithKeys(fn($relationResolver) => [$relationResolver->relationName => $relationResolver])->filter(fn($value, $key) => in_array($key, $sparseIncluded));
        ...
        $this->loadedRelationshipIdentifiers = (new LazyCollection(function () use ($request, $resourceRelationships) {
            foreach ($resourceRelationships as $relationName => $relationResolver) {
                ...
                yield from $this->compileResourceRelationshipUsingResolver($request, $this->resource, $relationResolver, $relatedModels);
            }
        }))->all();
    }
    /**
     ...
     */
    protected function compileResourceRelationshipUsingResolver(JsonApiRequest $request, mixed $resource, RelationResolver $relationResolver, Collection|Model|null $relatedModels): Generator
    {
        ...
        if ($relatedModels instanceof Collection) {
            ...
            yield $relationName => ['data' => $relatedModels->map(function ($relatedModel) use ($request, $resourceClass, $isUnique) {
                ...
                return transform([$relatedResource->resolveResourceType($request), $relatedResource->resolveResourceIdentifier($request)], function ($uniqueKey) use ($request, $relatedModel, $relatedResource, $isUnique) {
        ...
        if (is_null($relatedModel)) {
            ...
            return;
        } elseif ($relatedModel instanceof Pivot || in_array(AsPivot::class, class_uses_recursive($relatedModel), \true)) {
            yield $relationName => new MissingValue();
            return;
        }
        $relatedResource = rescue(fn() => $relatedModel->toResource($resourceClass), new JsonApiResource($relatedModel));
        yield $relationName => ['data' => transform([$relatedResource->resolveResourceType($request), $relatedResource->resolveResourceIdentifier($request)], function ($uniqueKey) use ($relatedModel, $relatedResource, $request) {
            $this->loadedRelationshipsMap[] = [$relatedResource, ...$uniqueKey, \true];
            $this->compileIncludedNestedRelationshipsMap($request, $relatedModel, $relatedResource);
            return ['id' => $uniqueKey[1], 'type' => $uniqueKey[0]];
        })];
    }
    ...
    {
        (new Collection($resource->toRelationships($request)))->transform(fn($value, $key) => is_int($key) ? new RelationResolver($value) : new RelationResolver($key, $value))->mapWithKeys(fn($relationResolver) => [$relationResolver->relationName => $relationResolver])->filter(fn($value, $key) => in_array($key, array_keys($relation->getRelations())))->each(function ($relationResolver, $key) use ($relation, $request) {
    ...
    public function resolveIncludedResourceObjects(JsonApiRequest $request): Collection
    {
        ...
        $visitedObjects[$this->resource] = [$this->resolveResourceType($request) => \true];
        while ($index < count($this->loadedRelationshipsMap)) {
            ...
            if (is_object($underlyingResource)) {
                ...
                $visitedObjects[$underlyingResource][$type] = \true;
            }
            ...
            $relationsData = $resourceInstance->includePreviouslyLoadedRelationships()->resolve($request);
            array_push($this->loadedRelationshipsMap, ...$resourceInstance->loadedRelationshipsMap ?? []);
            $relations->push(array_filter(['id' => $id, 'type' => $type, '_uniqueKey' => implode(':', $isUnique === \true ? [$id, $type] : [$id, $type, (string) Str::random()]), 'attributes' => Arr::get($relationsData, 'data.attributes'), 'relationships' => Arr::get($relationsData, 'data.relationships'), 'links' => Arr::get($relationsData, 'data.links'), 'meta' => Arr::get($relationsData, 'data.meta')]));
            $index++;
        }
        return $relations;
    }
     ...
     * @return $this
     */
    public function respectFieldsAndIncludesInQueryString(bool $value = \true)
    {
    ...
    {
        $this->includesPreviouslyLoadedRelationships = \true;
        return $this;
    }
}
