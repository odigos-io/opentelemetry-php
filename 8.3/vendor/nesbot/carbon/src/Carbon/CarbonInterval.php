...
use Odigos\Carbon\Constants\UnitValue;
use Odigos\Carbon\Exceptions\BadFluentConstructorException;
use Odigos\Carbon\Exceptions\BadFluentSetterException;
use Odigos\Carbon\Exceptions\InvalidCastException;
use Odigos\Carbon\Exceptions\InvalidFormatException;
use Odigos\Carbon\Exceptions\InvalidIntervalException;
use Odigos\Carbon\Exceptions\OutOfRangeException;
use Odigos\Carbon\Exceptions\ParseErrorException;
use Odigos\Carbon\Exceptions\UnitNotConfiguredException;
...
use Odigos\Carbon\Traits\IntervalRounding;
use Odigos\Carbon\Traits\IntervalStep;
use Odigos\Carbon\Traits\LocalFactory;
use Odigos\Carbon\Traits\MagicParameter;
use Odigos\Carbon\Traits\Mixin;
use Odigos\Carbon\Traits\Options;
use Odigos\Carbon\Traits\ToStringFormat;
use Closure;
use DateInterval;
use DateTime;
...
class CarbonInterval extends DateInterval implements CarbonConverterInterface, UnitValue
{
    ...
    public static function getDateIntervalSpec(DateInterval $interval, bool $microseconds = \false, array $skip = [], bool $withNegatives = \false): string
    {
        ...
        if ($microseconds && $interval->f !== 0.0) {
            $seconds = $withNegatives ? number_format($seconds + $interval->f, 6, '.', '') : \sprintf('%d.%06d', $seconds, abs($interval->f) * 1000000);
        }
