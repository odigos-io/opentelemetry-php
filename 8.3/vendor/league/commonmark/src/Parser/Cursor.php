...
use Odigos\League\CommonMark\Exception\UnexpectedEncodingException;
class Cursor
{
    ...
    public function getRemainder(): string
    {
        ...
        $subString = $this->isMultibyte ? \substr($this->line, $this->byteOffset($position)) : \substr($this->line, $position);
    ...
    public function match(string $regex): ?string
    {
        ...
        if ($this->partiallyConsumedTab) {
            ...
            if ($advance < $charsToTab) {
                $this->advanceBy($advance, \true);
                return $matches[0][0];
            }
            $this->advanceBy($charsToTab, \true);
            $advance -= $charsToTab;
        }
