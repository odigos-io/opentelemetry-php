...
final class CreateResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    {
        $choices = array_map(fn(array $result): \OpenAI\Responses\Chat\CreateResponseChoice => \OpenAI\Responses\Chat\CreateResponseChoice::from($result), $attributes['choices']);
        return new self($attributes['id'] ?? null, $attributes['object'], $attributes['created'], $attributes['model'], $attributes['system_fingerprint'] ?? null, $choices, isset($attributes['usage']) ? \OpenAI\Responses\Chat\CreateResponseUsage::from($attributes['usage']) : null, $meta);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return array_filter(['id' => $this->id, 'object' => $this->object, 'created' => $this->created, 'model' => $this->model, 'system_fingerprint' => $this->systemFingerprint, 'choices' => array_map(static fn(\OpenAI\Responses\Chat\CreateResponseChoice $result): array => $result->toArray(), $this->choices), 'usage' => $this->usage?->toArray()], fn(mixed $value): bool => !is_null($value));
    }
}
