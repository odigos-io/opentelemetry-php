...
class Digest
{
    ...
    {
        $response = $this->_client->get((string) $request->getUri(), [], ['auth' => ['type' => null]]);
    ...
    protected function _generateHeader(Request $request, array $credentials): string
    {
        ...
        if ($this->isSessAlgorithm) {
            ...
            $a1 = hash($this->hashType, $credentials['username'] . ':' . $credentials['realm'] . ':' . $credentials['password']) . ':' . $credentials['nonce'] . ':' . $credentials['cnonce'];
        } else {
        ...
        if (empty($credentials['qop'])) {
            ...
            $ha2 = hash($this->hashType, $a2);
            $response = hash($this->hashType, $ha1 . ':' . $credentials['nonce'] . ':' . $nc . ':' . $credentials['cnonce'] . ':' . $credentials['qop'] . ':' . $ha2);
        }
        $authHeader = 'Digest ';
