...
class SqliteSchemaDialect extends \Cake\Database\Schema\SchemaDialect
{
    ...
    protected function _convertColumn(string $column): array
    {
        ...
        if (Configure::read('ORM.mapJsonTypeForSqlite') === \true && (str_contains($col, \Cake\Database\Schema\TableSchemaInterface::TYPE_JSON) && !str_contains($col, 'jsonb'))) {
    ...
    private function describeColumnQuery(string $tableName): string
    {
        ...
        return sprintf('PRAGMA %s(%s)', $pragma, $this->_driver->quoteIdentifier($tableName));
    }
    /**
     * @inheritDoc
     */
    public function describeColumns(string $tableName): array
    {
        ...
        foreach ($statement->fetchAll('assoc') as $i => $row) {
            $name = $row['name'];
            $field = $this->_convertColumn($row['type']);
            $field += ['name' => $name, 'null' => !$row['notnull'], 'default' => $this->_defaultValue($row['dflt_value'], $row['type']), 'comment' => null, 'length' => null];
            if ($row['pk']) {
                $primary[] = $i;
            }
            $columns[] = $field;
        ...
        if (count($primary) == 1) {
            $offset = $primary[0];
            $columns[$offset]['autoIncrement'] = \true;
    ...
    {
        return sprintf('PRAGMA index_list(%s)', $this->_driver->quoteIdentifier($tableName));
    }
    /**
    ...
    {
        $columnsPattern = implode('\s*,\s*', array_map(fn($column) => '(?:' . $this->possiblyQuotedIdentifierRegex($column) . ')', $columns));
    ...
    private function extractDeferrable(string $tableSql, array $columns): ?string
    {
        ...
        if (preg_match($regex, $tableSql, $matches)) {
            return match ($matches['deferable']) {
                'NOT DEFERRABLE' => \Cake\Database\Schema\ForeignKey::NOT_DEFERRED,
                'DEFERRABLE INITIALLY DEFERRED' => \Cake\Database\Schema\ForeignKey::DEFERRED,
                'DEFERRABLE INITIALLY IMMEDIATE' => \Cake\Database\Schema\ForeignKey::IMMEDIATE,
                default => null,
            };
        }
        return null;
    ...
    public function describeIndexes(string $tableName): array
    {
        ...
        foreach ($statement->fetchAll('assoc') as $row) {
            $indexName = $row['name'];
            $indexSql = sprintf('PRAGMA index_info(%s)', $this->_driver->quoteIdentifier($indexName));
            $columns = [];
            ...
            $indexes[$indexName] = ['name' => $indexName, 'type' => $indexType, 'columns' => $columns, 'length' => []];
        }
    ...
    {
        $sql = sprintf('SELECT id FROM pragma_foreign_key_list(%s) GROUP BY id', $this->_driver->quoteIdentifier($tableName));
        return [$sql, []];
    }
    /**
     * @inheritDoc
    ...
    public function describeForeignKeys(string $tableName): array
    {
        ...
        foreach ($statement->fetchAll('assoc') as $row) {
            $id = $row['id'];
            if (!isset($keys[$id])) {
                $keys[$id] = ['name' => $id, 'type' => \Cake\Database\Schema\TableSchema::CONSTRAINT_FOREIGN, 'columns' => [], 'references' => [$row['table'], []], 'update' => $this->_convertOnClause($row['on_update'] ?? ''), 'delete' => $this->_convertOnClause($row['on_delete'] ?? ''), 'deferrable' => null];
            }
    ...
    public function describeCheckConstraints(string $tableName): array
    {
        ...
        if (preg_match_all($pattern, $createSql, $matches, \PREG_SET_ORDER)) {
            foreach ($matches as $index => $match) {
                ...
                $constraints[] = ['name' => $name, 'type' => \Cake\Database\Schema\TableSchema::CONSTRAINT_CHECK, 'expression' => $expression];
            }
        }
        return $constraints;
    }
    ...
    public function columnDefinitionSql(array $column): string
    {
        ...
        if (!$autoIncrement && isset($column['unsigned']) && $column['unsigned'] === \true && in_array($column['type'], $hasUnsigned, \true)) {
            $out .= ' UNSIGNED';
        }
        $foundType = \false;
        ...
        if (in_array($column['type'], $hasLength, \true) && isset($column['length']) && !$autoIncrement) {
    ...
    public function constraintSql(\Cake\Database\Schema\TableSchema $schema, string $name): string
    {
        ...
        if (isset($data['columns'])) {
            ...
            $aliased = array_map($this->_driver->quoteIdentifier(...), $data['columns']);
        ...
        if ($data['type'] === \Cake\Database\Schema\TableSchema::CONSTRAINT_PRIMARY) {
            ...
            $type = 'FOREIGN KEY';
            $clause = rtrim(sprintf(' REFERENCES %s (%s) ON UPDATE %s ON DELETE %s %s', $this->_driver->quoteIdentifier($data['references'][0]), $this->_convertConstraintColumns($data['references'][1]), $this->_foreignOnClause($data['update']), $this->_foreignOnClause($data['delete']), $data['deferrable'] ?? null));
        } elseif ($data['type'] === \Cake\Database\Schema\TableSchema::CONSTRAINT_CHECK) {
            $type = 'CHECK';
            $columns = $data['expression'];
        }
        return sprintf('CONSTRAINT %s %s (%s)%s', $this->_driver->quoteIdentifier($name), $type, $columns, $clause);
    }
    /**
     * {@inheritDoc}
     *
