...
class Message implements JsonSerializable
{
     ...
     * @var string|bool
     */
    protected string|bool $messageId = \true;
    /**
     ...
     * @var array<string>
     */
    protected array $transferEncodingAvailable = ['7bit', '8bit', 'base64', 'binary', 'quoted-printable'];
    /**
     ...
     */
    protected array $contentTypeCharset = ['ISO-2022-JP-MS' => 'ISO-2022-JP'];
    /**
     ...
     * @var array<string>
     */
    protected array $serializableProperties = ['to', 'from', 'sender', 'replyTo', 'cc', 'bcc', 'subject', 'returnPath', 'readReceipt', 'emailFormat', 'emailPattern', 'domain', 'attachments', 'messageId', 'headers', 'appCharset', 'charset', 'headerCharset', 'textMessage', 'htmlMessage'];
    /**
     * Constructor
     *
    ...
    {
        return $this->setEmailSingle('readReceipt', $email, $name, 'Disposition-Notification-To requires only 1 email address.');
    }
    /**
    ...
    public function setTransferEncoding(?string $encoding)
    {
        if ($encoding !== null) {
            ...
            if (!in_array($encoding, $this->transferEncodingAvailable, \true)) {
                throw new InvalidArgumentException(sprintf('Transfer encoding not available. Can be : %s.', implode(', ', $this->transferEncodingAvailable)));
            }
        }
    ...
    public function getHeaders(array $include = []): array
    {
        ...
        if ($include === array_values($include)) {
            $include = array_fill_keys($include, \true);
        }
    ...
    {
        if (!in_array($format, $this->emailFormatAvailable, \true)) {
    ...
    public function setAttachments(array $attachments)
    {
        ...
        foreach ($attachments as $name => $fileInfo) {
            ...
            if (!isset($fileInfo['file'])) {
                ...
                throw new InvalidArgumentException(sprintf('File must be a filepath or UploadedFileInterface instance. Found `%s` instead.', gettype($fileInfo['file'])));
            }
            if (!isset($fileInfo['mimetype']) && isset($fileInfo['file']) && is_string($fileInfo['file']) && function_exists('mime_content_type')) {
    ...
    {
        $name ??= 0;
        $this->addAttachments([$name => ['file' => $path, 'mimetype' => $mimetype, 'contentId' => $contentId, 'contentDisposition' => $contentDisposition]]);
        return $this;
    }
    /**
     * Add attachments
    ...
    {
        if ($this->boundary === null && ($this->attachments || $this->emailFormat === static::MESSAGE_BOTH)) {
    ...
    protected function generateMessage(): array
    {
        ...
        if ($this->emailFormat === static::MESSAGE_TEXT || $this->emailFormat === static::MESSAGE_BOTH) {
            if ($multiPart) {
                $msg[] = '--' . $textBoundary;
        ...
        if ($this->emailFormat === static::MESSAGE_HTML || $this->emailFormat === static::MESSAGE_BOTH) {
            if ($multiPart) {
                $msg[] = '--' . $textBoundary;
    ...
    protected function attachFiles(?string $boundary = null): array
    {
        ...
        foreach ($this->attachments as $filename => $fileInfo) {
            ...
            $hasDisposition = !isset($fileInfo['contentDisposition']) || $fileInfo['contentDisposition'];
    ...
    {
        $simpleMethods = ['from', 'sender', 'to', 'replyTo', 'readReceipt', 'returnPath', 'cc', 'bcc', 'messageId', 'domain', 'subject', 'attachments', 'emailFormat', 'emailPattern', 'charset', 'headerCharset'];
    ...
    public function setBody(array $content)
    {
        foreach ($content as $type => $text) {
            if (!in_array($type, $this->emailFormatAvailable, \true)) {
    ...
    public function getContentTransferEncoding(): string
    {
        ...
        if (in_array($charset, $this->charset8bit, \true)) {
            return '8bit';
        }
        return '7bit';
    }
    ...
    public function __serialize(): array
    {
        ...
        array_walk_recursive($array, function (&$item): void {
            if ($item instanceof SimpleXMLElement) {
                $item = json_decode((string) json_encode((array) $item), \true);
            }
        });
        /** @var array */
        return $array;
