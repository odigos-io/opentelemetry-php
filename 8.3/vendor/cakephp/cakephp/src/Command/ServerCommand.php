...
class ServerCommand extends \Cake\Command\Command
{
    ...
    {
        $command = sprintf('%s -S %s:%d -t %s', (string) env('PHP', 'php'), $this->_host, $this->_port, escapeshellarg($this->_documentRoot));
        if ($this->_iniPath) {
    ...
    {
        return sprintf('%s php-server -a -l %s:%d -r %s', (string) env('FRANKENPHP', 'frankenphp'), $this->_host, $this->_port, escapeshellarg($this->_documentRoot));
    }
    /**
    ...
    {
        $parser->setDescription([static::getDescription(), "<warning>[WARN] Don't use this in a production environment</warning>"])->addOption('host', ['short' => 'H', 'help' => 'ServerHost'])->addOption('port', ['short' => 'p', 'help' => 'ListenPort'])->addOption('ini_path', ['short' => 'I', 'help' => 'php.ini path'])->addOption('document_root', ['short' => 'd', 'help' => 'DocumentRoot'])->addOption('frankenphp', ['boolean' => \true, 'short' => 'f', 'help' => "Use frankenphp instead of PHP's built-in server"]);
        return $parser;
    }
}
