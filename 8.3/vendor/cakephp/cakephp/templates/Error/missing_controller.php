...
if ($incompleteInflection) {
    ?>
    Your routing resulted in <em><?php 
    echo h($originalClass);
    ?></em> as a controller name.
<?php 
} else {
    ?>
    <em><?php 
    echo h($pluginDot . $class);
    ?>Controller</em> could not be found.
<?php 
}
$this->end();
?>
...
if ($incompleteInflection) {
    ?>
    <p>The controller name <em><?php 
    echo h($originalClass);
    ?></em> has not been properly inflected, and
    could not be resolved to a controller that exists in your application.</p>

    <p>Ensure that your URL <strong><?php 
    echo h($this->request->getUri()->getPath());
    ?></strong> is
...
<?php 
    ...
    <p class="error">
        ...
        Create the class <em><?php 
    echo h($class);
    ?>Controller</em> below in file: <?php 
    echo h($path);
    ?>
    </p>
