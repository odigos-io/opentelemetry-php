...
class Queue implements LaravelHook
{
    ...
    protected function hookLater(): bool
    {
        return hook(QueueContract::class, 'later', pre: function (QueueContract $queue, array $params, string $class, string $function, ?string $filename, ?int $lineno) {
            ...
            $attributes = [TraceAttributes::CODE_FUNCTION_NAME => sprintf('%s::%s', $class, $function), TraceAttributes::CODE_FILE_PATH => $filename, TraceAttributes::CODE_LINE_NUMBER => $lineno, 'messaging.message.delivery_timestamp' => $estimateDeliveryTimestamp];
            /** @psalm-suppress ArgumentTypeCoercion */
            $span = $this->instrumentation->tracer()->spanBuilder(vsprintf('%s %s', [
