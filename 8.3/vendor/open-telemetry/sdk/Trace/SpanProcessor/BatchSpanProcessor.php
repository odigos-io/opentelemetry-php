...
class BatchSpanProcessor implements SpanProcessorInterface
{
    ...
    public function __construct(private readonly SpanExporterInterface $exporter, private readonly ClockInterface $clock, int $maxQueueSize = self::DEFAULT_MAX_QUEUE_SIZE, int $scheduledDelayMillis = self::DEFAULT_SCHEDULE_DELAY, int $exportTimeoutMillis = self::DEFAULT_EXPORT_TIMEOUT, int $maxExportBatchSize = self::DEFAULT_MAX_EXPORT_BATCH_SIZE, private readonly bool $autoFlush = \true, ?MeterProviderInterface $meterProvider = null)
    {
        ...
        $meter->createObservableUpDownCounter('otel.trace.span_processor.spans', '{spans}', 'The number of sampled spans received by the span processor')->observe(function (ObserverInterface $observer): void {
        ...
        $meter->createObservableUpDownCounter('otel.trace.span_processor.queue.limit', '{spans}', 'The queue size limit')->observe(function (ObserverInterface $observer): void {
            $observer->observe($this->maxQueueSize, self::ATTRIBUTES_PROCESSOR);
        });
        $meter->createObservableUpDownCounter('otel.trace.span_processor.queue.usage', '{spans}', 'The current queue usage')->observe(function (ObserverInterface $observer): void {
            $queued = $this->queue->count() * $this->maxExportBatchSize + count($this->batch);
            $pending = $this->queueSize - $queued;
    ...
    public static function builder(SpanExporterInterface $exporter): \OpenTelemetry\SDK\Trace\SpanProcessor\BatchSpanProcessorBuilder
    {
        return new \OpenTelemetry\SDK\Trace\SpanProcessor\BatchSpanProcessorBuilder($exporter);
    }
