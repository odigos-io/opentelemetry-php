...
class HackTypecheckerTest extends TestCase
{
    ...
    public function testTypechecks($recurse = \true)
    {
        ...
        $output = [];
        $exit_code = null;
        exec('hh_server --check ' . escapeshellarg(__DIR__ . '/../../') . ' 2>&1', $output, $exit_code);
        if ($exit_code === self::SERVER_ALREADY_RUNNING_CODE) {
            $this->assertTrue($recurse, 'Typechecker still running after running hh_client stop');
