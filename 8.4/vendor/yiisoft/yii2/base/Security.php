...
class Security extends \yii\base\Component
{
     ...
     * derivation salt.
     */
    public $allowedCiphers = ['AES-128-CBC' => [16, 16], 'AES-192-CBC' => [16, 24], 'AES-256-CBC' => [16, 32]];
    /**
    ...
    {
        return $this->encrypt($data, \true, $password, null);
    }
    /**
    ...
    {
        return $this->decrypt($data, \true, $password, null);
    }
    /**
    ...
    public function hkdf($algo, $inputKey, $salt = null, $info = null, $length = 0)
    {
        ...
        $test = @hash_hmac($algo, '', '', \true);
        if (!$test) {
            throw new \yii\base\InvalidArgumentException('Failed to generate HMAC with hash algorithm: ' . $algo);
        }
        ...
        if (!is_int($length) || $length < 0 || $length > 255 * $hashLength) {
            throw new \yii\base\InvalidArgumentException('Invalid length');
        }
        $blocks = $length !== 0 ? ceil($length / $hashLength) : 1;
        if ($salt === null) {
            $salt = str_repeat("\x00", $hashLength);
        }
        $prKey = hash_hmac($algo, $inputKey, $salt, \true);
        $hmac = '';
        $outputKey = '';
        for ($i = 1; $i <= $blocks; $i++) {
            $hmac = hash_hmac($algo, $hmac . $info . chr($i), $prKey, \true);
            $outputKey .= $hmac;
        }
        if ($length !== 0) {
    ...
    public function pbkdf2($algo, $password, $salt, $iterations, $length = 0)
    {
        ...
        if (!is_int($iterations) || $iterations < 1) {
            throw new \yii\base\InvalidArgumentException('Invalid iterations');
        }
        if (is_string($length) && preg_match('{^\d{1,16}$}', $length)) {
            $length = (int) $length;
        }
        ...
        $outputKey = '';
        for ($j = 1; $j <= $blocks; $j++) {
            $hmac = hash_hmac($algo, $salt . pack('N', $j), $password, \true);
            $xorsum = $hmac;
            for ($i = 1; $i < $iterations; $i++) {
                $hmac = hash_hmac($algo, $hmac, $password, \true);
                $xorsum ^= $hmac;
            }
            $outputKey .= $xorsum;
        }
    ...
    public function generatePasswordHash($password, $cost = null)
    {
        ...
        if (!is_string($hash) || strlen($hash) !== 60) {
            throw new \yii\base\Exception('Unknown error occurred while generating hash.');
        }
        return $hash;
    }
    /**
