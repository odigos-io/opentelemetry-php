...
final class RetrieveJobResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    {
        return new self($attributes['id'], $attributes['object'], $attributes['model'], $attributes['created_at'], $attributes['finished_at'], $attributes['fine_tuned_model'], isset($attributes['hyperparameters']) ? \OpenAI\Responses\FineTuning\RetrieveJobResponseHyperparameters::from($attributes['hyperparameters']) : null, $attributes['organization_id'], $attributes['result_files'], $attributes['status'], $attributes['validation_file'], $attributes['training_file'], $attributes['trained_tokens'], isset($attributes['error']) && isset($attributes['error']['code']) && isset($attributes['error']['param']) && isset($attributes['error']['message']) ? \OpenAI\Responses\FineTuning\RetrieveJobResponseError::from($attributes['error']) : null, $meta);
    }
    /**
     * {@inheritDoc}
     */
