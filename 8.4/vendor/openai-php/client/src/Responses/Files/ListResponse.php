...
final class ListResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    {
        $data = array_map(fn(array $result): \OpenAI\Responses\Files\RetrieveResponse => \OpenAI\Responses\Files\RetrieveResponse::from($result, $meta), $attributes['data']);
    ...
    {
        return ['object' => $this->object, 'data' => array_map(static fn(\OpenAI\Responses\Files\RetrieveResponse $response): array => $response->toArray(), $this->data), 'first_id' => $this->firstId, 'last_id' => $this->lastId, 'has_more' => $this->hasMore];
    }
}
