...
final class FileSystem
{
    ...
    public static function writeAtomic(string $file, string $content, ?int $mode = 0666): void
    {
        ...
        $tmp = $file . '.' . uniqid('', more_entropy: \true) . '.tmp';
        try {
    ...
    {
        [$stem] = explode('.', $name, 2);
        return $name !== '' && $name !== '.' && $name !== '..' && !preg_match('#[\x00-\x1F<>:"|?*\\\\/]#', $name) && !str_ends_with($name, '.') && !str_ends_with($name, ' ') && !preg_match('#^(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])$#i', $stem);
