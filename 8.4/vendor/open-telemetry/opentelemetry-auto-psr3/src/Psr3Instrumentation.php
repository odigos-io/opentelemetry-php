...
class Psr3Instrumentation
{
    ...
    private const MODES = [self::MODE_INJECT, self::MODE_EXPORT];
    ...
    public static function register(): void
    {
        ...
        $pre = static function (LoggerInterface $object, array $params, string $class, string $function): array {
            ...
            if (self::$cache[$id] === \true && $function !== 'log') {
            ...
            switch (self::$mode) {
                ...
                case self::MODE_EXPORT:
                    ...
                    $instrumentation ??= new CachedInstrumentation('io.opentelemetry.contrib.php.psr3', null, 'https://opentelemetry.io/schemas/1.32.0');
                    if ($function === 'log') {
                        $level = $params[0];
                        $body = $params[1] ?? '';
                        $context = $params[2] ?? [];
