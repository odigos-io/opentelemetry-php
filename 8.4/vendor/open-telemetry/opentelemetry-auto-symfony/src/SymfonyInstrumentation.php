...
final class SymfonyInstrumentation
{
    ...
    public static function register(): void
    {
        ...
        hook(HttpKernel::class, 'handle', pre: static function (HttpKernel $kernel, array $params, string $class, string $function, ?string $filename, ?int $lineno) use ($instrumentation): array {
            ...
            if (!is_callable($controller, \true, $controllerName)) {
                $controllerName = 'sub-request';
            }
            $name = $type === HttpKernelInterface::SUB_REQUEST ? sprintf('%s %s', $method, $controllerName) : $method;
            ...
            $scope->detach();
            $span->recordException($exception, [TraceAttributes::EXCEPTION_ESCAPED => \true]);
        ...
        hook(HttpKernel::class, 'terminate', post: static function (HttpKernel $kernel, array $params, ?\Throwable $exception): void {
            ...
            if (null !== $request) {
                ...
                if ('' !== $routeName) {
                    /** @psalm-suppress ArgumentTypeCoercion */
                    $span->updateName(sprintf('%s %s', $request->getMethod(), $routeName))->setAttribute(TraceAttributes::HTTP_ROUTE, $routeName);
                }
            }
            if (null !== $exception) {
