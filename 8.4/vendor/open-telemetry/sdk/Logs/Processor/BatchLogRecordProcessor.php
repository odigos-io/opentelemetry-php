...
class BatchLogRecordProcessor implements LogRecordProcessorInterface
{
    ...
    public function __construct(private readonly LogRecordExporterInterface $exporter, private readonly ClockInterface $clock, int $maxQueueSize = self::DEFAULT_MAX_QUEUE_SIZE, int $scheduledDelayMillis = self::DEFAULT_SCHEDULE_DELAY, int $exportTimeoutMillis = self::DEFAULT_EXPORT_TIMEOUT, int $maxExportBatchSize = self::DEFAULT_MAX_EXPORT_BATCH_SIZE, private readonly bool $autoFlush = \true, ?MeterProviderInterface $meterProvider = null)
    {
        ...
        $meter->createObservableUpDownCounter('otel.logs.log_processor.logs', '{logs}', 'The number of log records received by the processor')->observe(function (ObserverInterface $observer): void {
        ...
        $meter->createObservableUpDownCounter('otel.logs.log_processor.queue.limit', '{logs}', 'The queue size limit')->observe(function (ObserverInterface $observer): void {
            $observer->observe($this->maxQueueSize, self::ATTRIBUTES_PROCESSOR);
        });
        $meter->createObservableUpDownCounter('otel.logs.log_processor.queue.usage', '{logs}', 'The current queue usage')->observe(function (ObserverInterface $observer): void {
            $queued = $this->queue->count() * $this->maxExportBatchSize + count($this->batch);
            $pending = $this->queueSize - $queued;
