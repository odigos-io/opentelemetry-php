 ...
 */
class UuidV6 extends \Symfony\Component\Uid\Uuid implements \Symfony\Component\Uid\TimeBasedUidInterface
{
    ...
    {
        return \Symfony\Component\Uid\BinaryUtil::hexToDateTime('0' . substr($this->uid, 0, 8) . substr($this->uid, 9, 4) . substr($this->uid, 15, 3));
    }
    ...
    {
        $uuid = $this->uid;
        $time = \Symfony\Component\Uid\BinaryUtil::hexToNumericString('0' . substr($uuid, 0, 8) . substr($uuid, 9, 4) . substr($uuid, 15, 3));
        if ('-' === $time[0]) {
        ...
        return new \Symfony\Component\Uid\UuidV7(substr_replace(\sprintf('%012s-7%s-%s%s-%s%06s', \PHP_INT_SIZE >= 8 ? dechex($ms) : bin2hex(\Symfony\Component\Uid\BinaryUtil::fromBase($ms, \Symfony\Component\Uid\BinaryUtil::BASE10)), substr($uuid, -6, 3), $uuid[19], substr($uuid, -3), substr($uuid, -12, 6), $time), '-', 8, 0));
    }
    public static function generate(?\DateTimeInterface $time = null, ?\Symfony\Component\Uid\Uuid $node = null): string
    {
        $uuidV1 = \Symfony\Component\Uid\UuidV1::generate($time, $node);
