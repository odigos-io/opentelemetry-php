...
class Stream extends \Laravel\Prompts\Prompt
{
    ...
    public function lines(): array
    {
        ...
        foreach ($lines as $line) {
            $finalLines = array_merge($finalLines, $this->ansiWordwrap($line, $this->maxWidth));
        }
        return $finalLines;
    }
    ...
    protected function fadeOut(int $steps = 10): array
    {
        if (!static::terminal()->supportsTrueColor()) {
            return [fn(string $text) => $text, fn(string $text) => $this->dim($text)];
        }
        $fg = static::terminal()->foregroundColor();
        $bg = static::terminal()->backgroundColor();
        return array_map(function (int $step) use ($fg, $bg, $steps) {
            $factor = 1 - $step / $steps;
            $r = (int) ($bg[0] + ($fg[0] - $bg[0]) * $factor);
            $g = (int) ($bg[1] + ($fg[1] - $bg[1]) * $factor);
            $b = (int) ($bg[2] + ($fg[2] - $bg[2]) * $factor);
            return fn(string $text) => "\x1b[38;2;{$r};{$g};{$b}m{$text}\x1b[0m";
        }, range(0, $steps - 1));
    }
}
