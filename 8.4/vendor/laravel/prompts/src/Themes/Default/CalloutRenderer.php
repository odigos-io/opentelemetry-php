...
class CalloutRenderer extends \Laravel\Prompts\Themes\Default\Renderer
{
    ...
    public function __invoke(Callout $prompt): string
    {
        ...
        return match ($prompt->type) {
            'error' => $this->box($this->red($this->truncate('⚠ ' . $prompt->label, $prompt->terminal()->cols() - 6)), $message, color: 'red', info: $prompt->info),
            'warning' => $this->box($this->yellow($this->truncate('⚠ ' . $prompt->label, $prompt->terminal()->cols() - 6)), $message, color: 'yellow', info: $prompt->info),
            default => $this->box($this->cyan($this->truncate($prompt->label, $prompt->terminal()->cols() - 6)), $message, info: $prompt->info),
        };
    }
    /**
    ...
    protected function resolvePart(string|ElementContract $part): string|array
    {
        ...
        if ($part instanceof Heading) {
            return $this->bold($this->autoFormat(implode('', $part->content())));
        }
