...
class TestResponse implements ArrayAccess
{
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->isSuccessful(), $this->statusMessageWithDetails('>=200, <300', $this->getStatusCode()));
        return $this;
    }
    /**
    ...
    public function assertSuccessfulPrecognition()
    {
        ...
        PHPUnit::withResponse($this)->assertTrue($this->headers->has('Precognition-Success'), 'Header [Precognition-Success] not present on response.');
        PHPUnit::withResponse($this)->assertSame('true', $this->headers->get('Precognition-Success'), 'The Precognition-Success header was found, but the value is not `true`.');
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->isClientError(), $this->statusMessageWithDetails('>=400, < 500', $this->getStatusCode()));
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->isServerError(), $this->statusMessageWithDetails('>=500, < 600', $this->getStatusCode()));
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->isRedirect(), $this->statusMessageWithDetails('201, 301, 302, 303, 307, 308', $this->getStatusCode()));
        if (!is_null($uri)) {
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->isRedirect(), $this->statusMessageWithDetails('201, 301, 302, 303, 307, 308', $this->getStatusCode()));
        PHPUnit::withResponse($this)->assertTrue(Str::contains($this->headers->get('Location'), $uri), 'Redirect location [' . $this->headers->get('Location') . '] does not contain [' . $uri . '].');
        return $this;
    }
    /**
     ...
     * @return $this
     */
    public function assertRedirectToSignedRoute($name = null, $parameters = [], $absolute = \true)
    {
        if (!is_null($name)) {
            $uri = route($name, $parameters);
        }
        ...
        PHPUnit::withResponse($this)->assertTrue($request->hasValidSignature($absolute), 'The response is not a redirect to a signed route.');
        if (!is_null($name)) {
            $expectedUri = rtrim($request->fullUrlWithQuery(['signature' => null, 'expires' => null]), '?');
            PHPUnit::withResponse($this)->assertEquals(app('url')->to($uri), $expectedUri);
        }
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->headers->has($headerName), "Header [{$headerName}] not present on response.");
        $actual = $this->headers->get($headerName);
        if (!is_null($value)) {
            PHPUnit::withResponse($this)->assertEqualsIgnoringCase($value, $this->headers->get($headerName), "Header [{$headerName}] was found, but value [{$actual}] does not match [{$value}].");
        }
        return $this;
    }
    /**
    ...
    public function assertHeaderContains($headerName, $value)
    {
        PHPUnit::withResponse($this)->assertTrue($this->headers->has($headerName), "Header [{$headerName}] not present on response.");
        $actual = $this->headers->get($headerName, '');
        PHPUnit::withResponse($this)->assertTrue(Str::contains($actual, $value), "Header [{$headerName}] was found, but [{$actual}] does not contain [{$value}].");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertFalse($this->headers->has($headerName), "Unexpected header [{$headerName}] is present on response.");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertEquals(app('url')->to($uri), app('url')->to($this->headers->get('Location', '')));
        return $this;
    }
    /**
    ...
    public function assertDownload($filename = null)
    {
        ...
        if (trim($contentDisposition[0]) !== 'attachment') {
            PHPUnit::withResponse($this)->fail('Response does not offer a file download.' . \PHP_EOL . 'Disposition [' . trim($contentDisposition[0]) . '] found in header, [attachment] expected.');
        }
        if (!is_null($filename)) {
            if (isset($contentDisposition[1]) && trim(explode('=', $contentDisposition[1])[0]) !== 'filename') {
                PHPUnit::withResponse($this)->fail('Unsupported Content-Disposition header provided.' . \PHP_EOL . 'Disposition [' . trim(explode('=', $contentDisposition[1])[0]) . '] found in header, [filename] expected.');
            }
            ...
            if (!isset($contentDisposition[1])) {
                ...
                PHPUnit::withResponse($this)->assertSame($filename, isset(explode('=', $contentDisposition[1])[1]) ? trim(explode('=', $contentDisposition[1])[1], " \"'") : '', $message);
                return $this;
            }
        } else {
            PHPUnit::withResponse($this)->assertTrue(\true);
            return $this;
        }
    }
    /**
     ...
     * @return $this
     */
    public function assertCookie($cookieName, $value = null, $encrypted = \true, $unserialize = \false)
    {
        PHPUnit::withResponse($this)->assertNotNull($cookie = $this->getCookie($cookieName, $encrypted && !is_null($value), $unserialize), "Cookie [{$cookieName}] not present on response.");
        if (!$cookie || is_null($value)) {
            return $this;
        }
        $cookieValue = $cookie->getValue();
        PHPUnit::withResponse($this)->assertEquals($value, $cookieValue, "Cookie [{$cookieName}] was found, but value [{$cookieValue}] does not match [{$value}].");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertNotNull($cookie = $this->getCookie($cookieName, \false), "Cookie [{$cookieName}] not present on response.");
        $expiresAt = Carbon::createFromTimestamp($cookie->getExpiresTime(), date_default_timezone_get());
        PHPUnit::withResponse($this)->assertTrue($cookie->getExpiresTime() !== 0 && $expiresAt->lessThan(Carbon::now()), "Cookie [{$cookieName}] is not expired, it expires at [{$expiresAt}].");
        return $this;
    }
    /**
    ...
    public function assertCookieNotExpired($cookieName)
    {
        PHPUnit::withResponse($this)->assertNotNull($cookie = $this->getCookie($cookieName, \false), "Cookie [{$cookieName}] not present on response.");
        $expiresAt = Carbon::createFromTimestamp($cookie->getExpiresTime(), date_default_timezone_get());
        PHPUnit::withResponse($this)->assertTrue($cookie->getExpiresTime() === 0 || $expiresAt->greaterThan(Carbon::now()), "Cookie [{$cookieName}] is expired, it expired at [{$expiresAt}].");
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertNull($this->getCookie($cookieName, \false), "Cookie [{$cookieName}] is present on response.");
        return $this;
    }
    /**
     ...
     */
    public function getCookie($cookieName, $decrypt = \true, $unserialize = \false)
    {
        foreach ($this->headers->getCookies() as $cookie) {
            if ($cookie->getName() === $cookieName) {
                ...
                return new Cookie($cookie->getName(), $decryptedValue, $cookie->getExpiresTime(), $cookie->getPath(), $cookie->getDomain(), $cookie->isSecure(), $cookie->isHttpOnly(), $cookie->isRaw(), $cookie->getSameSite(), $cookie->isPartitioned());
            }
        }
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertTrue($this->baseResponse instanceof StreamedResponse || $this->baseResponse instanceof StreamedJsonResponse, 'Expected the response to be streamed, but it wasn\'t.');
        return $this;
    }
    /**
    ...
    {
        PHPUnit::withResponse($this)->assertTrue(!$this->baseResponse instanceof StreamedResponse && !$this->baseResponse instanceof StreamedJsonResponse, 'Response was unexpectedly streamed.');
        return $this;
    }
    /**
    ...
    {
        $this->decodeResponseJson()->assertStructure($structure, $responseData, \true);
        return $this;
    }
    /**
    ...
    public function assertJsonValidationErrors($errors, $responseKey = 'errors')
    {
        ...
        $errorMessage = $jsonErrors ? 'Response has the following JSON validation errors:' . \PHP_EOL . \PHP_EOL . json_encode($jsonErrors, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE) . \PHP_EOL : 'Response does not have JSON validation errors.';
        foreach ($errors as $key => $value) {
            ...
            foreach (Arr::wrap($value) as $expectedMessage) {
                ...
                if ($errorMissing) {
                    PHPUnit::withResponse($this)->fail("Failed to find a validation error in the response for key and message: '{$key}' => '{$expectedMessage}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
                }
            }
        }
        return $this;
    ...
    public function assertOnlyJsonValidationErrors($errors, $responseKey = 'errors')
    {
        ...
        $jsonErrors = Arr::get($this->json(), $responseKey) ?? [];
        $expectedErrorKeys = (new Collection($errors))->map(fn($value, $key) => is_int($key) ? $value : $key)->all();
        $unexpectedErrorKeys = Arr::except($jsonErrors, $expectedErrorKeys);
        PHPUnit::withResponse($this)->assertTrue(count($unexpectedErrorKeys) === 0, 'Response has unexpected validation errors: ' . (new Collection($unexpectedErrorKeys))->keys()->map(fn($key) => "'{$key}'")->join(', '));
        return $this;
    }
    /**
    ...
    public function assertJsonValidationErrorFor($key, $responseKey = 'errors')
    {
        $jsonErrors = Arr::get($this->json(), $responseKey) ?? [];
        $errorMessage = $jsonErrors ? 'Response has the following JSON validation errors:' . \PHP_EOL . \PHP_EOL . json_encode($jsonErrors, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE) . \PHP_EOL : 'Response does not have JSON validation errors.';
        PHPUnit::withResponse($this)->assertArrayHasKey($key, $jsonErrors, "Failed to find a validation error in the response for key: '{$key}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
        return $this;
    }
    /**
    ...
    public function assertJsonMissingValidationErrors($keys = null, $responseKey = 'errors')
    {
        ...
        if (is_null($keys) && count($errors) > 0) {
            PHPUnit::withResponse($this)->fail('Response has unexpected validation errors: ' . \PHP_EOL . \PHP_EOL . json_encode($errors, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE));
        }
        foreach (Arr::wrap($keys) as $key) {
            PHPUnit::withResponse($this)->assertFalse(isset($errors[$key]), "Found unexpected validation error for key: '{$key}'");
        }
        return $this;
    }
    /**
    ...
    public function assertJsonIsArray($key = null)
    {
        ...
        PHPUnit::withResponse($this)->assertTrue(is_array($data) && str_starts_with($encodedData, '[') && str_ends_with($encodedData, ']'));
        return $this;
    }
    /**
     ...
     * @return $this
     */
    public function assertInvalid($errors = null, $errorBag = 'default', $responseKey = 'errors')
    {
        if ($this->baseResponse->headers->get('Content-Type') === 'application/json') {
        ...
        $errorMessage = $sessionErrors ? 'Response has the following validation errors in the session:' . \PHP_EOL . \PHP_EOL . json_encode($sessionErrors, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE) . \PHP_EOL : 'Response does not have validation errors in the session.';
        foreach (Arr::wrap($errors) as $key => $value) {
            PHPUnit::withResponse($this)->assertArrayHasKey($resolvedKey = is_int($key) ? $value : $key, $sessionErrors, "Failed to find a validation error in session for key: '{$resolvedKey}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
            foreach (Arr::wrap($value) as $message) {
                if (!is_int($key)) {
                    ...
                    if (!$hasError) {
                        PHPUnit::withResponse($this)->fail("Failed to find a validation error for key and message: '{$key}' => '{$message}'" . \PHP_EOL . \PHP_EOL . $errorMessage);
                    }
                }
            }
        }
    ...
    public function assertSessionHas($key, $value = null)
    {
        ...
        if (is_null($value)) {
            PHPUnit::withResponse($this)->assertTrue($this->session()->has($key), "Session is missing expected key [{$key}].");
        } elseif ($value instanceof Closure) {
    ...
    public function assertSessionHasInput($key, $value = null)
    {
        ...
        if (is_null($value)) {
            PHPUnit::withResponse($this)->assertTrue($this->session()->hasOldInput($key), "Session is missing expected key [{$key}].");
        } elseif ($value instanceof Closure) {
    ...
    public function assertSessionHasNoErrors()
    {
        ...
        PHPUnit::withResponse($this)->assertFalse($hasErrors, 'Session has unexpected errors: ' . \PHP_EOL . \PHP_EOL . json_encode((function () use ($hasErrors) {
            $errors = [];
    ...
    public function assertSessionMissing($key, $value = null)
    {
        ...
        if (is_null($value)) {
            PHPUnit::withResponse($this)->assertFalse($this->session()->has($key), "Session has unexpected key [{$key}].");
        } elseif ($value instanceof Closure) {
    ...
    public function streamedContent()
    {
        ...
        if (!$this->baseResponse instanceof StreamedResponse && !$this->baseResponse instanceof BinaryFileResponse) {
    ...
    {
        return $this->responseHasView() ? isset($this->original->gatherData()[$offset]) : isset($this->json()[$offset]);
    }
    /**
    ...
    {
        return $this->responseHasView() ? $this->viewData($offset) : $this->json()[$offset];
    }
    /**
