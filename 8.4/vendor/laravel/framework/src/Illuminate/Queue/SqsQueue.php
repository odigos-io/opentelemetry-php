...
class SqsQueue extends \Illuminate\Queue\Queue implements QueueContract, ClearableQueue
{
    ...
    {
        $response = $this->sqs->getQueueAttributes(['QueueUrl' => $this->getQueue($queue), 'AttributeNames' => ['ApproximateNumberOfMessages', 'ApproximateNumberOfMessagesDelayed', 'ApproximateNumberOfMessagesNotVisible']]);
    ...
    {
        $response = $this->sqs->getQueueAttributes(['QueueUrl' => $this->getQueue($queue), 'AttributeNames' => ['ApproximateNumberOfMessagesDelayed']]);
    ...
    {
        $response = $this->sqs->getQueueAttributes(['QueueUrl' => $this->getQueue($queue), 'AttributeNames' => ['ApproximateNumberOfMessagesNotVisible']]);
    ...
    {
        return $this->enqueueUsing($job, $this->createPayload($job, $queue ?: $this->default, $data), $queue, null, function ($payload, $queue) use ($job) {
    ...
    {
        return $this->enqueueUsing($job, $this->createPayload($job, $queue ?: $this->default, $data, $delay), $queue, $delay, function ($payload, $queue, $delay) use ($job) {
    ...
    protected function getQueueableOptions($job, $queue, $payload, $delay = null): array
    {
        ...
        if ($isFifo) {
            $messageDeduplicationId = match (\true) {
