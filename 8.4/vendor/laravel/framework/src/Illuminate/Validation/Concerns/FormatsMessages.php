...
trait FormatsMessages
{
    ...
    protected function getMessage($attribute, $rule)
    {
        ...
        $customMessage = $this->getCustomMessageFromTranslator(in_array($rule, $this->sizeRules) ? [$customKey . ".{$this->getAttributeType($attribute)}", $customKey] : $customKey);
    ...
    protected function getCustomMessageFromTranslator($keys)
    {
        foreach (Arr::wrap($keys) as $key) {
            ...
            $message = $this->getWildcardCustomMessages(Arr::dot((array) $this->translator->get('validation.custom')), $shortKey, $key);
            if ($message !== $key) {
                return $message;
            }
        }
    ...
    protected function getAttributeType($attribute)
    {
        ...
        return match (\true) {
            ...
            $this->getValue($attribute) instanceof UploadedFile, $this->getValue($attribute) instanceof File => 'file',
            default => 'string',
        };
    }
    /**
    ...
    {
        return $this->replaceIndexOrPositionPlaceholder($message, $attribute, 'position', fn($segment) => $segment + 1);
    }
    /**
    ...
    protected function replaceOrdinalPositionPlaceholder($message, $attribute)
    {
        ...
        return $this->replaceIndexOrPositionPlaceholder($message, $attribute, 'ordinal-position', fn($segment) => Number::ordinal($segment + 1));
    }
    /**
    ...
    protected function replaceIndexOrPositionPlaceholder($message, $attribute, $placeholder, ?Closure $modifier = null)
    {
        ...
        foreach ($segments as $segment) {
            if (is_numeric($segment)) {
                ...
                $message = str_ireplace(':' . $this->numberToIndexOrPositionWord($numericIndex) . '-' . $placeholder, $modifier((int) $segment), $message);
                $numericIndex++;
            }
        }
        return $message;
    ...
    {
        return [1 => 'first', 2 => 'second', 3 => 'third', 4 => 'fourth', 5 => 'fifth', 6 => 'sixth', 7 => 'seventh', 8 => 'eighth', 9 => 'ninth', 10 => 'tenth'][(int) $value] ?? 'other';
    }
    /**
