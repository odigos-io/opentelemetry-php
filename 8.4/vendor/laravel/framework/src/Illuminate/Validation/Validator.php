...
class Validator implements ValidatorContract
{
     ...
     * @var string[]
     */
    protected $fileRules = ['Between', 'Dimensions', 'Encoding', 'Extensions', 'File', 'Image', 'Max', 'Mimes', 'Mimetypes', 'Min', 'Size'];
    /**
    ...
    public function parseData(array $data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $key = str_replace(['.', '*'], ['__dot__' . static::$placeholderHash, '__asterisk__' . static::$placeholderHash], $key);
            $newData[$key] = $value;
        }
        return $newData;
    }
    ...
    {
        return str_replace(['__dot__' . static::$placeholderHash, '__asterisk__' . static::$placeholderHash], ['.', '*'], $value);
    }
    /**
    ...
    public function validated()
    {
        ...
        foreach ($this->getRules() as $key => $rules) {
            ...
            if ($this->excludeUnvalidatedArrayKeys && (in_array('array', $rules) || in_array('list', $rules)) && $value !== null && !empty(preg_grep('/^' . preg_quote($key, '/') . '\.+/', array_keys($this->getRules())))) {
                continue;
            }
    ...
    protected function validateUsingCustomRule($attribute, $value, $rule)
    {
        ...
        if ($rule instanceof ValidatorAwareRule) {
            if ($attribute !== $originalAttribute) {
                $this->addCustomAttributes([$attribute => $this->customAttributes[$originalAttribute] ?? $originalAttribute]);
            }
    ...
    {
        return (new Collection($this->messages()->toArray()))->map(fn($message, $key) => explode('.', $key)[0])->unique()->flip()->all();
    }
    /**
    ...
    {
        return (new Collection($this->rules))->mapWithKeys(fn($value, $key) => [static::decodeAttributeWithPlaceholder($key) => $value])->all();
    }
    /**
    ...
    {
        $rules = (new Collection($rules))->map(function ($value) {
