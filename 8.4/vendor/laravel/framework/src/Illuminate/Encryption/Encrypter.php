...
class Encrypter implements EncrypterContract, StringEncrypter
{
     ...
     * @var array
     */
    private static $supportedCiphers = ['aes-128-cbc' => ['size' => 16, 'aead' => \false], 'aes-256-cbc' => ['size' => 32, 'aead' => \false], 'aes-128-gcm' => ['size' => 16, 'aead' => \true], 'aes-256-gcm' => ['size' => 32, 'aead' => \true]];
    /**
    ...
    public function encrypt(#[\SensitiveParameter] $value, $serialize = \true)
    {
        ...
        $mac = self::$supportedCiphers[strtolower($this->cipher)]['aead'] ? '' : $this->hash($iv, $value, $this->key);
    ...
    public function decrypt($payload, $unserialize = \true)
    {
        ...
        foreach ($this->getAllKeys() as $key) {
            if ($this->shouldValidateMac() && !$foundValidMac = $foundValidMac || $this->validMacForKey($payload, $key)) {
                continue;
            }
    ...
    protected function getJsonPayload($payload)
    {
        ...
        $payload = json_decode(base64_decode($payload), \true);
    ...
    protected function validPayload($payload)
    {
        ...
        return strlen(base64_decode($payload['iv'], \true)) === openssl_cipher_iv_length(strtolower($this->cipher));
    }
    /**
    ...
    public static function appearsEncrypted($value)
    {
        ...
        $decoded = base64_decode($value, \true);
        if ($decoded === \false) {
            return \false;
        }
        $payload = json_decode($decoded, \true);
