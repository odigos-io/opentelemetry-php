...
class AboutCommand extends Command
{
    ...
    public function handle()
    {
        ...
        (new Collection(static::$data))->map(fn($items) => (new Collection($items))->map(function ($value) {
            if (is_array($value)) {
                return [$value];
            }
            if (is_string($value)) {
            ...
            return (new Collection($this->laravel->call($value)))->map(fn($value, $key) => [$key, $value])->values()->all();
        })->flatten(1))->sortBy(function ($data, $key) {
            $index = array_search($key, ['Environment', 'Cache', 'Drivers']);
            return $index === \false ? 99 : $index;
        })->filter(function ($data, $key) {
            return $this->option('only') ? in_array($this->toSearchKeyword($key), $this->sections()) : \true;
    ...
    protected function displayJson($data)
    {
        $output = $data->flatMap(function ($data, $section) {
            return [(new Stringable($section))->snake()->value() => $data->mapWithKeys(fn($item, $key) => [$this->toSearchKeyword($item[0]) => value($item[1], \true)])];
        });
    ...
    protected function gatherApplicationInformation()
    {
        ...
        static::addToSection('Drivers', fn() => array_filter(['Broadcasting' => config('broadcasting.default'), 'Cache' => function ($json) {
            ...
            if (config('logging.channels.' . $logChannel . '.driver') === 'stack') {
                ...
                return value(static::format(value: $logChannel, console: fn($value) => '<fg=yellow;options=bold>' . $value . '</> <fg=gray;options=bold>/</> ' . $secondary->implode(', '), json: fn() => $secondary->all()), $json);
            } else {
                $logs = $logChannel;
            }
            return $logs;
            ...
            if (in_array(config('mail.mailers.' . $mailMailer . '.transport'), ['failover', 'roundrobin'])) {
                ...
                return value(static::format(value: $mailMailer, console: fn($value) => '<fg=yellow;options=bold>' . $value . '</> <fg=gray;options=bold>/</> ' . $secondary->implode(', '), json: fn() => $secondary->all()), $json);
            }
            return $mailMailer;
            ...
            if (config('queue.connections.' . $queueConnection . '.driver') === 'failover') {
                ...
                return value(static::format(value: $queueConnection, console: fn($value) => '<fg=yellow;options=bold>' . $value . '</> <fg=gray;options=bold>/</> ' . $secondary->implode(', '), json: fn() => $secondary->all()), $json);
            }
            return $queueConnection;
        }, 'Scout' => config('scout.driver'), 'Session' => config('session.driver')]));
        static::addToSection('Storage', fn() => [...$this->determineStoragePathLinkStatus($formatStorageLinkedStatus)]);
    ...
    {
        return (new Collection(config('filesystems.links', [])))->mapWithKeys(function ($target, $link) use ($formatStorageLinkedStatus) {
    ...
    {
        return (new Collection(explode(',', $this->option('only') ?? '')))->filter()->map(fn($only) => $this->toSearchKeyword($only))->all();
    }
    /**
    ...
    public static function format($value, ?Closure $console = null, ?Closure $json = null)
    {
        return function ($isJson) use ($value, $console, $json) {
            if ($isJson === \true && $json instanceof Closure) {
