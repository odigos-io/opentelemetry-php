...
class RouteListCommand extends Command
{
     ...
     * @var array
     */
    protected $verbColors = ['ANY' => 'red', 'GET' => 'blue', 'HEAD' => '#6C7280', 'OPTIONS' => '#6C7280', 'POST' => 'yellow', 'PUT' => 'yellow', 'PATCH' => 'yellow', 'DELETE' => 'red'];
    /**
    ...
    {
        $routes = (new Collection($this->router->getRoutes()))->map(fn($route) => $this->getRouteInformation($route))->filter()->all();
    ...
    protected function sortRoutes($sort, array $routes)
    {
        ...
        return (new Collection($routes))->sortBy($sort)->toArray();
    }
    /**
    ...
    protected function displayRoutes(array $routes)
    {
        ...
        $this->output->writeln($this->option('json') ? $this->asJson($routes) : $this->forCli($routes));
    }
    /**
    ...
    {
        return in_array($route->getControllerClass(), ['\Illuminate\Routing\RedirectController', '\Illuminate\Routing\ViewController'], \true);
    }
    /**
    ...
    {
        $routes = $routes->map(fn($route) => array_merge($route, ['action' => $this->formatActionForCli($route), 'method' => $route['method'] == 'GET|HEAD|POST|PUT|PATCH|DELETE|OPTIONS' ? 'ANY' : $route['method'], 'uri' => $route['domain'] ? $route['domain'] . '/' . ltrim($route['uri'], '/') : $route['uri']]));
        ...
        return $routes->map(function ($route) use ($maxMethod, $terminalWidth) {
            ...
            $dots = str_repeat('.', max($terminalWidth - mb_strlen($method . $spaces . $uri . $action) - 6 - ($action ? 1 : 0), 0));
            ...
            $method = (new Stringable($method))->explode('|')->map(fn($method) => sprintf('<fg=%s>%s</>', $this->verbColors[$method] ?? 'default', $method))->implode('<fg=#6C7280>|</>');
            return [sprintf('  <fg=white;options=bold>%s</> %s<fg=white>%s</><fg=#6C7280>%s %s</>', $method, $spaces, preg_replace('#({[^}]+})#', '<fg=yellow>$1</>', $uri), $dots, str_replace('   ', ' › ', $action ?? '')), $this->output->isVerbose() && !empty($middleware) ? "<fg=#6C7280>{$middleware}</>" : null];
        })->flatten()->filter()->prepend('')->push('')->push($routeCount)->push('')->toArray();
    }
    /**
    ...
    {
        return [['json', null, InputOption::VALUE_NONE, 'Output the route list as JSON'], ['method', null, InputOption::VALUE_OPTIONAL, 'Filter the routes by method'], ['action', null, InputOption::VALUE_OPTIONAL, 'Filter the routes by action'], ['name', null, InputOption::VALUE_OPTIONAL, 'Filter the routes by name'], ['domain', null, InputOption::VALUE_OPTIONAL, 'Filter the routes by domain'], ['middleware', null, InputOption::VALUE_OPTIONAL, 'Filter the routes by middleware'], ['path', null, InputOption::VALUE_OPTIONAL, 'Only show routes matching the given path pattern'], ['except-path', null, InputOption::VALUE_OPTIONAL, 'Do not display the routes matching the given path pattern'], ['reverse', 'r', InputOption::VALUE_NONE, 'Reverse the ordering of the routes'], ['sort', null, InputOption::VALUE_OPTIONAL, 'The column (domain, method, uri, name, action, middleware, definition) to sort by', 'uri'], ['except-vendor', null, InputOption::VALUE_NONE, 'Do not display routes defined by vendor packages'], ['only-vendor', null, InputOption::VALUE_NONE, 'Only display routes defined by vendor packages']];
    }
}
