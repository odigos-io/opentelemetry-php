...
class DownCommand extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $this->components->info($wasAlreadyDown ? 'Maintenance mode options updated.' : 'Application is now in maintenance mode.');
            ...
            $this->components->error(sprintf('Failed to enter maintenance mode: %s.', $e->getMessage()));
            return 1;
        }
    }
    /**
    ...
    {
        return ['except' => $this->excludedPaths(), 'redirect' => $this->redirectPath(), 'retry' => $this->getRetryTime(), 'refresh' => $this->option('refresh'), 'secret' => $this->getSecret(), 'status' => (int) ($this->option('status') ?? 503), 'template' => $this->option('render') ? $this->prerenderView() : null];
    }
    /**
