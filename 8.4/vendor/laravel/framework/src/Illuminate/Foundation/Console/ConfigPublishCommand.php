...
class ConfigPublishCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $name = (string) (is_null($this->argument('name')) ? select(label: 'Which configuration file would you like to publish?', options: (new Collection($config))->map(fn(string $path) => basename($path, '.php'))) : $this->argument('name'));
    ...
    protected function getBaseConfigurationFiles()
    {
        ...
        foreach (Finder::create()->files()->name('*.php')->in(__DIR__ . '/../../../../config') as $file) {
            ...
            $config[$name] = $shouldMergeConfiguration === \true && file_exists($stubPath = __DIR__ . '/../../../../config-stubs/' . $name . '.php') ? $stubPath : $file->getRealPath();
        }
