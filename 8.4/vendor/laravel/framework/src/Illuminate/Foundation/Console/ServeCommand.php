...
class ServeCommand extends Command
{
     ...
     * @var string[]
     */
    public static $passthroughVariables = ['APP_ENV', 'HERD_PHP_81_INI_SCAN_DIR', 'HERD_PHP_82_INI_SCAN_DIR', 'HERD_PHP_83_INI_SCAN_DIR', 'HERD_PHP_84_INI_SCAN_DIR', 'HERD_PHP_85_INI_SCAN_DIR', 'IGNITION_LOCAL_SITES_PATH', 'LARAVEL_SAIL', 'PATH', 'PHP_IDE_CONFIG', 'SYSTEMROOT', 'XDEBUG_CONFIG', 'XDEBUG_MODE', 'XDEBUG_SESSION'];
    /** {@inheritdoc} */
    #[\Override]
    ...
    protected function getHostAndPort()
    {
        if (preg_match('/(\[.*\]):?([0-9]+)?/', $this->input->getOption('host'), $matches) !== \false) {
            return [$matches[1] ?? $this->input->getOption('host'), $matches[2] ?? null];
        }
    ...
    protected function flushOutputBuffer()
    {
        ...
        $lines->map(fn($line) => trim($line))->filter()->each(function ($line) {
            ...
            if ((new Stringable($line))->contains(' Accepted')) {
                ...
                $this->requestsPool[$requestPort] = [$this->getDateFromLine($line), $this->requestsPool[$requestPort][1] ?? \false, microtime(\true)];
                ...
                if (empty($this->requestsPool[$requestPort]) || count($this->requestsPool[$requestPort] ?? []) !== 3) {
                    $this->requestsPool[$requestPort] = [$this->getDateFromLine($line), \false, microtime(\true)];
                }
    ...
    {
        $regex = !windows_os() && is_int($this->phpServerWorkers) ? '/^\[\d+]\s\[([a-zA-Z0-9: ]+)\]/' : '/^\[([^\]]+)\]/';
    ...
    {
        return [['host', null, InputOption::VALUE_OPTIONAL, 'The host address to serve the application on', Env::get('SERVER_HOST', '127.0.0.1')], ['port', null, InputOption::VALUE_OPTIONAL, 'The port to serve the application on', Env::get('SERVER_PORT')], ['tries', null, InputOption::VALUE_OPTIONAL, 'The max number of ports to attempt to serve from', 10], ['no-reload', null, InputOption::VALUE_NONE, 'Do not reload the development server on .env file changes']];
    }
}
