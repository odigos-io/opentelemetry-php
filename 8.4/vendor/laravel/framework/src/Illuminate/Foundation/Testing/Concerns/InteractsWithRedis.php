...
trait InteractsWithRedis
{
    ...
    public function setUpRedis()
    {
        ...
        foreach (static::redisDriverProvider() as $driver) {
            if (Env::get('REDIS_CLUSTER_HOSTS_AND_PORTS')) {
                $config = ['options' => ['cluster' => 'redis', 'prefix' => 'test_'], 'clusters' => ['default' => array_map(static fn($hostAndPort) => ['host' => explode(':', $hostAndPort)[0], 'port' => explode(':', $hostAndPort)[1]], explode(',', Env::get('REDIS_CLUSTER_HOSTS_AND_PORTS')))]];
            } else {
        ...
        try {
            ...
            if ($host === '127.0.0.1' && $port === 6379 && Env::get('REDIS_HOST') === null) {
                static::$connectionFailedOnceWithDefaultsSkip = \true;
                $this->markTestSkipped('Trying default host/port failed, please set environment variable REDIS_HOST & REDIS_PORT to enable ' . __CLASS__);
            }
        }
    ...
    {
        if (static::$connectionFailedOnceWithDefaultsSkip === \true) {
            return;
        }
