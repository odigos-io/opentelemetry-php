...
trait DatabaseTruncation
{
    ...
    protected function truncateTablesForAllConnections(): void
    {
        ...
        (new Collection($this->connectionsToTruncate()))->each(function ($name) use ($database) {
    ...
    protected function truncateTablesForConnection(ConnectionInterface $connection, ?string $name): void
    {
        ...
        (new Collection($this->getAllTablesForConnection($connection, $name)))->when($this->tablesToTruncate($connection, $name), function (Collection $tables, array $tablesToTruncate) {
    ...
    {
        return property_exists($this, 'tablesToTruncate') && is_array($this->tablesToTruncate) ? $this->tablesToTruncate[$connectionName] ?? $this->tablesToTruncate : null;
    }
    /**
    ...
    protected function exceptTables(ConnectionInterface $connection, ?string $connectionName): array
    {
        ...
        return property_exists($this, 'exceptTables') && is_array($this->exceptTables) ? array_merge($this->exceptTables[$connectionName] ?? $this->exceptTables, [$migrationsTable]) : [$migrationsTable];
    }
    /**
