...
trait InteractsWithDatabase
{
    ...
    protected function assertDatabaseHas($table, array $data = [], $connection = null)
    {
        ...
        if ($table instanceof Model) {
            $data = [$table->getKeyName() => $table->getKey(), ...$data];
        }
    ...
    public function expectsDatabaseQueryCount($expected, $connection = null)
    {
        ...
        $this->beforeApplicationDestroyed(function () use (&$actual, $expected, $connectionInstance) {
            $this->assertSame($expected, $actual, "Expected {$expected} database queries on the [{$connectionInstance->getName()}] connection. {$actual} occurred.");
        });
        return $this;
    }
    /**
    ...
    public function castAsJson($value, $connection = null)
    {
        ...
        return $db->raw($db->getQueryGrammar()->compileJsonValueCast($value));
    }
    /**
