...
class BehaviorRegistry extends ObjectRegistry implements EventDispatcherInterface
{
    ...
    {
        throw new MissingBehaviorException(['class' => $class . 'Behavior', 'plugin' => $plugin]);
    }
    /**
    ...
    {
        deprecationWarning('5.3.0', sprintf('Calling behavior methods on the table instance is deprecated.' . '  Use `$table->getBehavior(\'YourBehavior\')->%s()` instead.', $method));
        $method = strtolower($method);
