...
class SelectLoader
{
    ...
    protected function _buildQuery(array $options): SelectQuery
    {
        ...
        $fetchQuery = $query->select($options['fields'])->where($options['conditions'])->eagerLoaded(\true)->enableHydration($shouldHydrate)->setConnectionRole($selectQuery->getConnectionRole());
