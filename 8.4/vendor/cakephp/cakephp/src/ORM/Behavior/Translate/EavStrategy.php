...
class EavStrategy implements \Cake\ORM\Behavior\Translate\TranslateStrategyInterface
{
     ...
     */
    protected array $_defaultConfig = ['fields' => [], 'translationTable' => 'I18n', 'defaultLocale' => null, 'referenceName' => null, 'allowEmptyTranslations' => \true, 'onlyTranslated' => \false, 'strategy' => 'subquery', 'tableLocator' => null, 'validator' => \false];
    /**
     * Constructor
     *
    ...
    public function __construct(Table $table, array $config = [])
    {
        ...
        $this->table = $table;
        $this->translationTable = $this->getTableLocator()->get($this->_config['translationTable'], ['allowFallbackClass' => \true]);
    ...
    protected function setupAssociations(): void
    {
        ...
        foreach ($fields as $field) {
            ...
            if (!$tableLocator->exists($name)) {
                $fieldTable = $tableLocator->get($name, ['className' => $table, 'alias' => $name, 'table' => $this->translationTable->getTable(), 'allowFallbackClass' => \true]);
            } else {
            ...
            $this->table->hasOne($name, ['targetTable' => $fieldTable, 'foreignKey' => 'foreign_key', 'joinType' => $filter ? SelectQuery::JOIN_TYPE_INNER : SelectQuery::JOIN_TYPE_LEFT, 'conditions' => $conditions, 'propertyName' => $field . '_translation']);
        }
        ...
        $this->table->hasMany($targetAlias, ['className' => $table, 'foreignKey' => 'foreign_key', 'strategy' => $strategy, 'conditions' => $conditions, 'propertyName' => '_i18n', 'dependent' => \true]);
    }
    /**
    ...
    public function beforeFind(EventInterface $event, SelectQuery $query, ArrayObject $options): void
    {
        ...
        $conditions = function (string $field, string $locale, SelectQuery $query, array $select) {
            return function (SelectQuery $q) use ($field, $locale, $query, $select) {
                ...
                if ($query->isAutoFieldsEnabled() || in_array($field, $select, \true) || in_array($this->table->aliasField($field), $select, \true)) {
                    $q->select(['id', 'content']);
                }
                return $q;
            };
        ...
        foreach ($fields as $field) {
            ...
            if ($changeFilter) {
                $filter = $options['filterByCurrentLocale'] ? SelectQuery::JOIN_TYPE_INNER : SelectQuery::JOIN_TYPE_LEFT;
                $contain[$name]['joinType'] = $filter;
            }
        }
        $query->contain($contain);
        $query->formatResults(fn(CollectionInterface $results) => $this->rowMapper($results, $locale), SelectQuery::PREPEND);
    }
    /**
    ...
    public function beforeSave(EventInterface $event, EntityInterface $entity, ArrayObject $options): void
    {
        ...
        $values = $entity->extract($this->_config['fields'], \true);
        ...
        if ($key) {
            ...
            $preexistent = $this->translationTable->find()->select(['id', 'field'])->where(['field IN' => $fields, 'locale' => $locale, 'foreign_key' => $key, 'model' => $model])->all()->indexBy('field');
        }
        $modified = [];
        ...
        foreach ($new as $field => $content) {
            $new[$field] = new $entityClass(compact('locale', 'field', 'content', 'model'), ['useSetters' => \false, 'markNew' => \true]);
        }
    ...
    public function groupTranslations(CollectionInterface $results): CollectionInterface
    {
        return $results->map(function ($row) {
            ...
            foreach ($grouped->combine('field', 'content', 'locale') as $locale => $keys) {
                $translation = new $entityClass($keys + ['locale' => $locale], ['markNew' => \false, 'useSetters' => \false, 'markClean' => \true]);
    ...
    protected function bundleTranslatedFields(EntityInterface $entity): void
    {
        ...
        foreach ($translations as $lang => $translation) {
            foreach ($fields as $field) {
                ...
                $contents[] = new $entityClass(['content' => $translation->get($field)], ['useSetters' => \false]);
            }
        }
        if (!$find) {
            return;
    ...
    protected function findExistingTranslations(array $ruleSet): array
    {
        ...
        $query = $association->find()->select(['id', 'num' => 0])->where(current($ruleSet))->disableHydration();
        unset($ruleSet[0]);
