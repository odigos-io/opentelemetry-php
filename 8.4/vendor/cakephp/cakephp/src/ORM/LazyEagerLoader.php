...
class LazyEagerLoader
{
    ...
    protected function _getQuery(array $entities, array $contain, \Cake\ORM\Table $source): SelectQuery
    {
        ...
        $query = $source->find()->select((array) $primaryKey)->where(function (QueryExpression $exp, SelectQuery $q) use ($primaryKey, $keys, $source) {
    ...
    protected function _injectResults(array $entities, SelectQuery $query, array $associations, \Cake\ORM\Table $source): array
    {
        ...
        $results = $query->all()->indexBy(fn(EntityInterface $e) => implode(';', $e->extract($primaryKey)))->toArray();
