...
class EagerLoader
{
     ...
     */
    protected array $_containOptions = ['associations' => 1, 'foreignKey' => 1, 'conditions' => 1, 'fields' => 1, 'sort' => 1, 'matching' => 1, 'queryBuilder' => 1, 'finder' => 1, 'joinType' => 1, 'strategy' => 1, 'negateMatch' => 1, 'includeFields' => 1];
    /**
     ...
     * @var bool
     */
    protected bool $_autoFields = \true;
    /**
     ...
     * @return $this
     */
    public function enableAutoFields(bool $enable = \true)
    {
    ...
    public function setMatching(string $associationPath, ?Closure $builder = null, array $options = [])
    {
        ...
        $sharedOptions = ['negateMatch' => \false, 'matching' => \true] + $options;
        $contains = [];
        $nested =& $contains;
    ...
    public function normalized(\Cake\ORM\Table $repository): array
    {
        ...
        foreach ($this->_containments as $alias => $options) {
            $contain[$alias] = $this->_normalizeContain($repository, $alias, $options, ['root' => '']);
        }
        ...
        foreach ($associations as $table => $options) {
            ...
            if (is_array($options)) {
                ...
                if (isset($options['config'], $options['associations'])) {
                    ...
                    $associations = $this->_reformatContain($options['associations'], $pointer[$table] ?? []);
                    // Merge config with associations, ensuring config options stay as options
                    $options = $options['config'] + $associations;
                } else {
                    $options = $this->_reformatContain($options, $pointer[$table] ?? []);
                }
            }
    ...
    public function attachAssociations(SelectQuery $query, \Cake\ORM\Table $repository, bool $includeFields): void
    {
        ...
        do {
            foreach ($attachable as $alias => $loadable) {
                $config = $loadable->getConfig() + ['aliasPath' => $loadable->aliasPath(), 'propertyPath' => $loadable->propertyPath(), 'includeFields' => $includeFields];
    ...
    protected function _normalizeContain(\Cake\ORM\Table $parent, string $alias, array $options, array $paths): \Cake\ORM\EagerLoadable
    {
        ...
        if (isset($options['matching']) && $options['matching'] === \true) {
    ...
    private function mergeJoins(array $a, array $b): array
    {
        foreach ($b as $alias => $loadable) {
            if (isset($a[$alias])) {
                assert(\false, sprintf('You cannot join with `%s` because it conflicts with the existing `%s` join.' . ' The existing join will be lost.', $loadable->aliasPath(), $a[$alias]->aliasPath()));
            }
        }
        return $a + $b;
    }
    ...
    public function loadExternal(SelectQuery $query, iterable $results): iterable
    {
        ...
        foreach ($external as $meta) {
            ...
            $callback = $instance->eagerLoader($config + ['query' => $query, 'contain' => $contain, 'keys' => $keys, 'nestKey' => $meta->aliasPath()]);
    ...
    public function associationsMap(\Cake\ORM\Table $table): array
    {
        ...
        $map = $this->_buildAssociationsMap($map, $this->_matching->normalized($table), \true);
    ...
    protected function _buildAssociationsMap(array $map, array $level, bool $matching = \false): array
    {
        foreach ($level as $assoc => $meta) {
            ...
            $map[] = ['alias' => $assoc, 'instance' => $instance, 'canBeJoined' => $canBeJoined, 'entityClass' => $instance->getTarget()->getEntityClass(), 'nestKey' => $canBeJoined ? $assoc : $meta->aliasPath(), 'matching' => $forMatching ?? $matching, 'targetProperty' => $meta->targetProperty()];
