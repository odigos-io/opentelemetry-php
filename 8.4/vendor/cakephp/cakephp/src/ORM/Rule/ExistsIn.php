    ...
    public function __invoke(EntityInterface $entity, array $options): bool
    {
        if (is_string($this->_repository)) {
            ...
            if (!$table->hasAssociation($this->_repository)) {
                throw new DatabaseException(sprintf('ExistsIn rule for `%s` is invalid. `%s` is not associated with `%s`.', implode(', ', $this->_fields), $this->_repository, $options['repository']::class));
            }
        ...
        $primary = array_map(fn(string $key) => $target->aliasField($key) . ' IS', $bindingKey);
