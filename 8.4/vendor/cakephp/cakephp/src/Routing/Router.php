...
class Router
{
    ...
    protected static function _applyUrlFilters(array $url): array
    {
        ...
        foreach (static::$_urlFilters as $filter) {
            try {
                ...
                $message = sprintf('URL filter defined in %s on line %s could not be applied. The filter failed with: %s', $ref->getFileName(), $ref->getStartLine(), $e->getMessage());
    ...
    public static function url(UriInterface|array|string|null $url = null, bool $full = \false): string
    {
        ...
        if (is_array($url)) {
            ...
            if (isset($url['_https'])) {
                $url['_scheme'] = $url['_https'] === \true ? 'https' : 'http';
            }
            if (isset($url['_full']) && $url['_full'] === \true) {
                $full = \true;
            }
            if (isset($url['#'])) {
                $frag = '#' . $url['#'];
            ...
            $url = (string) $url;
            if (str_starts_with($url, 'javascript:') || str_starts_with($url, 'mailto:') || str_starts_with($url, 'tel:') || str_starts_with($url, 'sms:') || str_starts_with($url, '#') || str_starts_with($url, '?') || str_starts_with($url, '//') || str_contains($url, '://')) {
                return $url;
            }
    ...
    public static function fullBaseUrl(?string $base = null): string
    {
        ...
        if ($base !== null) {
            ...
            if (!$base && !empty(static::$_requestContext['_host'])) {
                $base = sprintf('%s://%s', static::$_requestContext['_scheme'], static::$_requestContext['_host']);
        ...
        static::$_requestContext = ['_scheme' => $parts['scheme'] ?? null, '_host' => $parts['host'] ?? null, '_port' => $parts['port'] ?? null] + static::$_requestContext;
        return static::$_fullBaseUrl;
    }
    /**
    ...
    public static function reverseToArray(ServerRequest|array $params): array
    {
        ...
        unset($params['pass'], $params['_matchedRoute'], $params['_name']);
        if (!$route && $template) {
     ...
     */
    public static function extensions(array|string|null $extensions = null, bool $merge = \true): array
    {
    ...
    protected static function unwrapShortString(array $url): array
    {
        foreach (['plugin', 'prefix', 'controller', 'action'] as $key) {
            if (array_key_exists($key, $url)) {
                throw new InvalidArgumentException("`{$key}` cannot be used when defining route targets with a string route path.");
            }
        }
    ...
    public static function parseRoutePath(string $url): array
    {
        ...
        $defaults = ['controller' => $matches['controller'], 'action' => $matches['action']];
        if ($matches['plugin'] !== '') {
            ...
            foreach ($paramsArray as $param) {
                if (str_contains($param, '=')) {
                    if (!preg_match('/(?<key>.+?)=(?<value>.*)/', $param, $paramMatches)) {
                        throw new InvalidArgumentException("Could not parse a key=value from `{$param}` in route path `{$url}`.");
                    }
                    $paramKey = $paramMatches['key'];
                    if (!preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/', $paramKey)) {
                        throw new InvalidArgumentException("Param key `{$paramKey}` is not valid in route path `{$url}`.");
                    }
