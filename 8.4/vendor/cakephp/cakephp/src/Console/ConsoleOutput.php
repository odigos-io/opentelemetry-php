...
class ConsoleOutput
{
    ...
    public function __construct($stream = 'php://stdout')
    {
        ...
        $this->_output = $stream;
        if (\DIRECTORY_SEPARATOR === '\\' && !str_contains(strtolower(php_uname('v')), 'windows 10') && !str_contains(strtolower((string) env('SHELL')), 'bash.exe') && !env('ANSICON') && env('ConEmuANSI') !== 'ON' || function_exists('posix_isatty') && !posix_isatty($this->_output) || env('NO_COLOR') !== null) {
