...
class FileLog extends \Cake\Log\Engine\BaseLog
{
    ...
    protected array $_defaultConfig = [
        ...
        'mask' => null,
        'dirMask' => 0777,
        'formatter' => ['className' => DefaultFormatter::class],
    ];
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!is_dir($this->_path)) {
            mkdir($this->_path, $this->_config['dirMask'] ^ umask(), \true);
        }
    ...
    protected function _getFilename(string $level): string
    {
        ...
        if ($this->_file) {
            ...
            $filename = 'error.log';
        } elseif (in_array($level, $debugTypes, \true)) {
            $filename = 'debug.log';
        } else {
            $filename = $level . '.log';
        }
    ...
    protected function _rotateFile(string $filename): ?bool
    {
        ...
        if (!is_file($filePath) || filesize($filePath) < $this->_size) {
            return null;
        }
