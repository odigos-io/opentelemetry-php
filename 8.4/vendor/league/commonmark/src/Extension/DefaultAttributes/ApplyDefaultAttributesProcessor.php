    ...
    private static function wrapCallable(string $class, string $name, $value, \Closure $callback): \Closure
    {
        return static function (Node $node) use ($callback, $class, $name, $value) {
            try {
                ...
                throw new InvalidConfigurationException(\sprintf('The "%s" attribute configured for %s is %s, which PHP also treats as a callable, so it was ' . 'called with the node and failed: %s. If it was meant to be a literal attribute value instead, ' . 'set "default_attributes/strict_callables" to true.', $name, $class, \is_string($value) ? \sprintf('the string "%s"', $value) : 'an array', $e->getMessage()), 0, $e);
            }
        };
    }
}
